...
skimmed_aoc_flag_missing <- skim(df) %>%
  filter(n_missing > 0.5*nrow(df)) %>%
  pull(skim_variable)

#tells us if any of the variables have more variable groups than 10% of the total rows 
skimmed_aoc_flag_deep <- skim(df) %>%
  filter(factor.n_unique > 0.1*nrow(df)) %>%
  pull(skim_variable)

#variables that only have one group (not useful)
skimmed_aoc_flag_shallow <- skim(df) %>%
...
```{r}
#reorder
dfmatrix$Var2 <- factor(dfmatrix$Var2, levels = dfmatrix$Var2[order(dfmatrix$Freq)])
#plot
dfmatrix %>%
  filter(Var2 != "effect_10") %>% 
  ggplot(aes(y = Var2, x = Freq)) +
  geom_col(color = "white") +
  labs(x = "Relative Predictor Power for Toxicity",
       y = "Variable",
       title = "Mutual Information Output") +
  dark_theme_bw()
```
...
fulldata <- df %>%
  dplyr::select(organism.group, max.size.ingest.mm, acute.chronic_f, 
          lvl2_f, bio.org, dose.um3.mL.master, dose.mg.L.master,
          dose.particles.mL.master, dose.surface.area.um2.mL.master,
          dose.specific.surface.area.um2.mg.mL, polymer, effect_10)


full_data_model <- glm(effect_10 ~ ., data = fulldata, family = binomial, na.action=na.fail)
AICc(full_data_model)

allfits <- dredge(full_data_model, extra = "r.squaredLR")

allfits_withorg <- allfits %>%
  filter(lvl2_f == "+", bio.org == "+", acute.chronic_f == "+", polymer == "+", organism.group == "+", !is.na(max.size.ingest.mm))

write.csv(allfits_withorg, "fitsnolvl1.csv")

summary(glm(effect_10 ~ organism.group+ max.size.ingest.mm+ acute.chronic_f+ 
          lvl2_f+ bio.org+ dose.surface.area.um2.mL.master + polymer, data = fulldata, family = binomial, na.action=na.fail))

explainer_glm_model <- DALEX::explain(glm(effect_10 ~ organism.group+ max.size.ingest.mm+ acute.chronic_f+ 
          lvl2_f+ bio.org+ dose.surface.area.um2.mL.master + polymer, data = fulldata, family = binomial, na.action=na.fail))
...
# Fit the model
model.null <- glm(effect_10 ~ 1, data = train.data, family = binomial) 
# Summarize the final selected model
summary(model.null)
AIC(model.null)

# Fit the model
model.polymer <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
               bio.org +
               polymer, data = train.data, family = binomial) 

# Summarize the final selected model
summary(model.polymer)


# Fit the model
model.volume <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
               bio.org +
               dose.um3.mL.master, data = train.data, family = binomial) 

# Summarize the final selected model
summary(model.volume)

# Fit the model
model.mass <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
               bio.org +
               dose.mg.L.master, data = train.data, family = binomial) 

# Summarize the final selected model
summary(model.mass)

# Fit the model
model.surfacearea <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
               bio.org +
               dose.surface.area.um2.mL.master, data = train.data, family = binomial) 

summary(model.surfacearea)


# Fit the model
model.count <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
...
# Fit the model
model <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
...
model.null <- gam(effect_10 ~ 1, data = train.data, family = binomial) 

summary(model.null)

probabilities <- model.null %>% predict(test.data, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, "1", "0")
# Model accuracy
mean(predicted.classes==test.data$effect_10)

model.dose <- glm(effect_10 ~ organism.group + 
               max.size.ingest.mm + 
               acute.chronic_f + 
               lvl1_f + 
               lvl2_f + 
...
probabilities <- model.dose %>% predict(test.data, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, "1", "0")
# Model accuracy
mean(predicted.classes==test.data$effect_10)
