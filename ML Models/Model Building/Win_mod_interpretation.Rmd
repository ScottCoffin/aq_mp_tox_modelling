...
org_endpoint_comb <- df %>%
  dplyr::select(-dose.um3.mL.master, -dose.mg.L.master, -dose.particles.mL.master, -dose.surface.area.um2.mL.master, -sa_vol_ratio, -effect_10) %>%
  distinct()

for(row in 1:nrow(org_endpoint_comb)){
  slice <- org_endpoint_comb[row,]
  doses <- inner_join(df_expand, slice) %>%
    dplyr::select(dose.um3.mL.master, dose.mg.L.master, dose.particles.mL.master, dose.surface.area.um2.mL.master, sa_vol_ratio, prob) %>%
