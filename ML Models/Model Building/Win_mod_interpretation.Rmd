...
#Look at the data and mutual information

```{r include = FALSE}
skim(aoc_z)

```
variables that shouldn't get used in the model because
They are too deep (too many unique classes): doi, authors, 
...
```{r include=F}
discretized <- discretize(aoc_z)
matrix <- discretized %>%
  mutual_info_matrix(names(.), normalized = T)

```

```{r include=F}
mutual_info_heatmap(matrix)
```
