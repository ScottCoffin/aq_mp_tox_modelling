...
```
# Data summary
First let's build a manuscript-ready table of all parameters and their completeness using skimr
```{r}
require(modelsummary)
#datasummary_skim(aoc_z)

full_skim <- skim(aoc_z)
write.csv(full_skim, "Simulated Distributions/BigDataManuscript/full_skim.csv")
```

# Data Selection

**Important**: this dataset has ONLY been filtered for 'red' criteria for technical quality.
Goal is to determine relative feature importance, with effect "y/n" as response variable.

```{r}
df_intermediate <- aoc_z %>% 
  ...
  dplyr::select(c(
    ### DOSE METRICS ####
                  ...
                  dose.um3.mL.master,
                  dose.specific.surface.area.um2.mg.mL,
    ...
    ### Particle characteristics ####
                  ...
                  ingestible, #dependent on species mouth size and particle size
                  translocatable, #translocatable, non-translocatable, or nanoparticle
    ### Effect data ####
                  bio.org, #use tissue and above for lower tiers, organism and above for higher tiers
                  effect, #convert to response variable then remove below
  ...
  #mutate_if(~is.numeric(.) && (.) > 0, log10) %>% #scale data for better model
...
#ensure completeness
skim(df_intermediate)
```

### Publication-Ready Table
```{r}
skim_df <- skim(df_intermediate)
write.csv(skim_df, "Simulated Distributions/BigDataManuscript/skim_df.csv")
```
### Normalize data
```{r}
...
df <- df_intermediate %>% 
  mutate_if(is.numeric, returnOrderNormedData) #normalization function
...
roc <- plot(mp_classif_rf, 
  ...
  labs(title = "ROC Curves - All Models",  
       subtitle = paste("AUC:",
          paste("rf = ",round(mp_classif_rf$measures$auc,2), 
                paste("svm = ",round(mp_classif_svm$measures$auc,2)),
          paste("nnet = ",round(mp_classif_nnet$measures$auc,2))
          )
          )) +
...
vi_classif_rf <- model_parts(explainer_classif_rf, loss_function = loss_root_mean_square, B = 50)
...
pp <- ppr(as.numeric(effect_10) ~ ., data = train, nterms = 22)  
```


The idea is that if we randomly permute the values of an important feature in the training data, the training performance would degrade (since permuting the values of a feature effectively destroys any relationship between that feature and the target variable). This of course assumes that the model has been properly tuned (e.g., using cross-validation) and is not over fitting. The permutation approach uses the difference between some baseline performance measure (e.g., training R2 or RMSE) and the same performance measure obtained after permuting the values of a particular feature in the training data (Note: the model is NOT refit to the training data after randomly permuting the values of a feature). To use the permutation approach, specify method = "permute" in the call to vi() or vip(). Note that using method = "permute" requires specifying a few additional arguments; see ?vi_permute for details.

```{r eval=FALSE, include=FALSE}
# Plot VI scores
set.seed(2021)  # for reproducibility
p1 <- vip(pp, method = "permute", target = "effect_10", metric = "RMSE",
          pred_wrapper = function(pp, test)) +
           ggtitle("PPR")

p2 <- vip(nn, method = "permute", target = "effect_10", metric = "RMSE",
...
set.seed(2021)  # for reproducibility
vip(pp,
    method = "permute", target = "effect_10", metric = "rmse", nsim = 20,
...
t1 <- rpart(effect_10 ~ .,
       ...
       #     control = rpart.control(minbucket = 100, cp=0.008), #requires that there be at least 19 cases (responded + nonrespondents) in the final grouping of variable values of the terminal node of the tre..
            data = train)
...
t1 <- rpart(effect_10 ~ dose.surface.area.um2.mL.master +
              lvl1_f +
              size.length.um.used.for.conversions +
              max.size.ingest.mm +
              shape +
              ...
              #effect.metric +
              #dose.um3.mL.master +
              bio.org + organism.group,
            method = "class", #classification because response is discrete
            control = rpart.control(minbucket = 5, cp=0.008), #requires that there be at least 19 cases (responded + nonrespondents) in the final grouping of variable values of the terminal node of the tre..
            data = train)
print(t1, digits=4)
```
...
interpretable_tree <- prp(t1, main="Tree for Effect",
