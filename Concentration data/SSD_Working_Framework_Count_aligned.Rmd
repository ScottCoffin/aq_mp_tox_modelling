...
tier1_2_small <- aoc_final_small %>% 
         ...
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(size.length.um.used.for.conversions %in% (1:upper.tissue.trans.size.um)) 
...
gt(ERM_small) %>% 
  tab_header(title = "1-100 µm Thresholds (Aligned to 1 - 5000 µm)", subtitle = "Thresholds presented in particles/L") %>% 
