...
endpoints_tier3_4_small <- tier3_4_small %>% 
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tiers 3 & 4, Endpoint Representation", subtitle = "1µm < 100µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
...
#       scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
#                          labels = trans_format("log10", scales::math_format(10^.x))) + 
#   labs(title = "Tier 3/4, 10-100 µm", x = "particles/L") +
#       fill.type + #user-selected
#       color.type + #user-selected
#       theme.type #user theme
# 
...
a <- tier1_2_large %>% 
  select(doi, article, authors, risk.13, org_f, species_f, lvl1_f, lvl2_f, lvl3_f, bio_f, effect.metric, shape_f, poly_f, size.length.um.used.for.conversions, size.width.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_v.particles.mL, af.time_noNA, af.noec, dose_new) %>% 
  rename("Endpoint_Score" = risk.13,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Broad_Endpoint_Category" = lvl1_f,
         "Specific_Endpoint_Category" = lvl2_f,
         "Measured_Endpoint" = lvl3_f,
         "Biological_Level" = bio_f,
         "Particle_Length" = size.length.um.used.for.conversions, 
         "Particle_Width" = size.width.um.used.for.conversions, 
         "Dose_Volume_Aligned_particles_mL" = EC_env_v.particles.mL, 
         "Acute_Chronic_AF" = af.time_noNA, 
         "Convert_to_NOEC" = af.noec, 
...
c <- tier1_2_small %>% 
  select(doi, article, authors, risk.13, org_f, species_f, lvl1_f, lvl2_f, lvl3_f, bio_f, effect.metric, shape_f, poly_f, size.length.um.used.for.conversions, size.width.um.used.for.conversions,dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_sa.particles.mL, af.time_noNA, af.noec, dose_new) %>% 
  rename("Endpoint_Score" = risk.13,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Broad_Endpoint_Category" = lvl1_f,
