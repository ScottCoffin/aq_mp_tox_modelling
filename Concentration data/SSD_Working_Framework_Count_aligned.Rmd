...
For the example of a 10 um PE fragment, the length was reported ($L_i = 10 \mu m$), however the width was not reported by the authors, so a default value of 0.77 is used, which is the average for marine surface water ($R_i = 0.77$). 
...
For the example of a 10 um PE fragment, the length was reported ($a = 0.5* 10 \mu m$), however the width was not reported by the authors, so a default value of 0.77 is used ($b = 0.5 * 0.77 * 10 \mu m$), which is the average for marine surface water, and height is assumed to be 0.67 times the width ($c = 0.5 * 0.77 * 0.67 * 10 \mu m$) (Kooi et al 2021). Surface area is calculated:
```{r}
...
For the example of a 10 um polyethylene fragment, the volume was calculated above (~ $207 um^3$), and the density of polyethylene was is estimated to be 0.935 g/cm^3. $\mu_{x,mono}$ is calculated:
...
mu_m.mono <- massfnx(R = 0.77,  L = 10, p = 0.935)
...
species_size_f <- aoc_intermediate %>% 
  ...
  geom_bar() +
  scale_fill_manual(values = cal_palette(name = "superbloom1", type = "continuous", n = 34)) +
  labs(title = "Filtered Data by Species") +
  xlab("Length (um)") +
  ylab("# of Species")+
  theme.type 
...
endpoint_size_f <- aoc_intermediate %>% 
  ...
  geom_bar() +
  scale_fill_manual(values = cal_palette(name = "superbloom3", type = "continuous", n = 6)) +
  labs(title = "Filtered Data by General Endpoint") +
  ylab("# of Endpoints")+
  xlab("Length (um)") +
  theme.type 
...
ROS_size <- aoc_intermediate %>% 
  filter(lvl2_f %in% "Oxidative Stress") %>% 
  ggplot(aes(x = size.length.um.used.for.conversions, fill = lvl2_f)) +
  geom_histogram(bins = 25) +
   scale_fill_manual(name = "endpoint", values = cal_palette(name = "superbloom3", type = "continuous", n = 9)) +
  xlab("Length (um)") +
  ylab("# of observations")+
  labs(title = "Filtered Data by General Endpoint") +
  theme.type +
...
aoc_final_small <- aoc_intermediate  %>% 
  ...
  mutate(x2M = 10) %>% #set to 10um for upper limit
...
tier1_2_large <- aoc_final_large %>% 
         ...
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(size.length.um.used.for.conversions %in% (1:5000)) 
...
tier1_2_small <- aoc_final_small %>% 
         ...
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(size.length.um.used.for.conversions %in% (1:10)) 
...
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
  ...
  labs(title = "Microplastics Species Sensitivity Distribution",
      ...
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 3/4, 1 < 10 µm", x = "particles/L") +
      fill.type + #user-selected
      color.type + #user-selected
      theme.type #user theme
...
gt(ERM_small) %>% 
  tab_header(title = "1-10 µm Thresholds (Aligned)", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Particle, Volume,'Surface Area', Mass, 'Specific Surface Area'), 
             n_sigfig = 2,
             use_seps = TRUE) %>% 
    data_color(
    columns = names(ERM_small)[2:6],
    colors = scales::col_numeric(
      palette = paletteer::paletteer_d(palette = "ggsci::teal_material") %>% as.character(),
...
gt(ERM_large) %>% 
  tab_header(title = "1-5,000 µm Thresholds (Aligned)", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Particle, Volume,'Surface Area', Mass, 'Specific Surface Area'), 
             n_sigfig = 2,
             use_seps = TRUE) %>% 
    data_color(
    columns = names(ERM_large)[2:6],
    colors = scales::col_numeric(
      palette = paletteer::paletteer_d(palette = "ggsci::teal_material") %>% as.character(),
      domain = full_val_range),
    alpha = 0.75) %>% 
