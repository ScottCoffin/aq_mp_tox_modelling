...
##--- environmental calculations ---###
  ...
  mutate(x_LL_v = volumefnx_poly(width = x1D_set * R.ave, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M * R.ave, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
...
##--- environmental calculations ---###
  ...
  mutate(x_LL_m = massfnx_poly(width = x1D_set * R.ave, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M * R.ave, # upper width
...
##--- environmental calculations ---###
  ...
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         ...
                         b = 0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_LL_m = massfnx_poly(width = x1D_set * R.ave,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M * R.ave, #ingestible
...
write.xlsx(list_of_datasets, file = "./Concentration data/Threshold_Manuscript_Figs/SSD_rawdata_912021_lessConservative.xlsx")
...
aoc_final_large_lower <- aoc_intermediate_alphas  %>% 
  ...
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, #all calculations below occur for length. Width is R.ave * length, so correcting here makes width the max size ingest below
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
...
##--- environmental calculations ---###
  ...
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         ...
                         c = 0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, #CONSERVATIVE: x2D_set, #length is upper default limit by definition (accounts for fibers)
                         b = 0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
...
##--- environmental calculations ---###
  ...
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, #length-limited
                                  #length = x2D_set, #upper definiton (accouunts for fibers)
                                 width = x2M * R.ave)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  ...
  mutate(x_UL_m = massfnx_poly(width = x2M * R.ave, #ingestible
                               length = x2M, #length-limited
                               #length = x2D_set, #upper definition
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
...
aoc_final_large_upper <- aoc_intermediate_alphas  %>% 
  ...
  filter(Group != "Algae") %>% 
  # define upper size WIDTH for ingestion (based on average width:length ratio)
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ R.ave * x2D_set, #all calculations below occur for length. Width is R.ave * length, so correcting here makes width the max size ingest below
                         (max.size.ingest.um * R.ave) < x2D_set ~ (R.ave * max.size.ingest.um),
                         (max.size.ingest.um * R.ave) > x2D_set ~ (x2D_set * R.ave)
...
##--- environmental calculations ---###
  ...
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         ...
                         c = 0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         b = 0.5 * R.ave * x2M, #width-limited
                         c = 0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible width
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
...
##--- environmental calculations ---###
  ...
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2D_set, #upper definiton (accouunts for fibers)
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
...
####---- TISSUE TRANSLOCATION ------#####
#### DEFAULT ####
filtered.data.small.default <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))
...
# get thresholds
small.default.t1 <- SSD_function_t1(filtered.data = filtered.data.small.default, hcxlcl = 5)
small.default.t2 <- SSD_function_t2(filtered.data = filtered.data.small.default, hcx = 5)
small.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.default, hcx = 5)
small.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.default, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.small.lower <- aoc_final_small_lower %>% 
         mutate(dose_new =  EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))
# get thresholds
small.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.small.lower, hcxlcl = 5)
small.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.small.lower, hcx = 5)
small.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower, hcx = 5)
small.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower, hcx = 10)

#### upper ####
#data filtering
filtered.data.small.upper <- aoc_final_small_upper %>% 
         mutate(dose_new =  EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))
# get thresholds
small.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.small.upper, hcxlcl = 5)
small.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.small.upper, hcx = 5)
small.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper, hcx = 5)
small.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper, hcx = 10)

####---- Food Dilution ------#####
#### DEFAULT ####
filtered.data.large.default <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))
...
# get thresholds
large.default.t1 <- SSD_function_t1(filtered.data = filtered.data.large.default, hcxlcl = 5)
large.default.t2 <- SSD_function_t2(filtered.data = filtered.data.large.default, hcx = 5)
large.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.default, hcx = 5)
large.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.default, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.large.lower <- aoc_final_large_lower %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))
# get thresholds
large.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.large.lower, hcxlcl = 5)
large.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.large.lower, hcx = 5)
large.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower, hcx = 5)
large.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower, hcx = 10)

#### upper ####
#data filtering
filtered.data.large.upper <- aoc_final_large_upper %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))
# get thresholds
large.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.large.upper, hcxlcl = 5)
large.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.large.upper, hcx = 5)
large.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper, hcx = 5)
large.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper, hcx = 10)
...
```{r eval=FALSE, include=FALSE}
library(openxlsx)
write.xlsx(t1_2_SA, file="sensitivityAnalysis_8-31-2021.xlsx", sheetName="Tier 1&2_translocation", row.names=TRUE)
write.xlsx(t3_4_SA, file="sensitivityAnalysis_8-31-2021.xlsx", sheetName="Tier 3&4_translocation", row.names=TRUE, append = TRUE)
write.xlsx(t1_2_volume, file="sensitivityAnalysis_8-31-2021.xlsx", sheetName="Tier 1&2_food dilution", row.names=TRUE, append = TRUE)
write.xlsx(t3_4_volume, file="sensitivityAnalysi_8-31-2021s.xlsx", sheetName="Tier 1&2_food dilution", row.names=TRUE, append = TRUE)
