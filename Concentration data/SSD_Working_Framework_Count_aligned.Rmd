---
title: "Aquatic Environment Thresholds for Marine Species"
author: "Scott Coffin & Leah Thornton Hampton"
date: "7/1/2021"
output:   
  html_document:
    code_folding: hide
    theme: journal
    toc: yes
    toc_float: yes
    toc_depth: 6
    number_sections: true
  word_document:
    toc: yes
---
*MARINE SPECIES ONLY*
```{r Libraries, include = FALSE}

library(tidyverse)
library(calecopal)
library(ssdtools)
library(DT)
library(plotly)
library(gridExtra)
library(grid)
library(wesanderson)
library(ggdark)
library(broom)
library(knitr)
library(kableExtra)
library(viridis)
library(ggrepel)
library(scales)
library(gt)
library(ggsci)
library(openxlsx)
library(ggpubr)

```

```{r Set Particle Size, include = FALSE}
###define sizes for filtering and alignment##
# smaller size bin
small_tier_lower_size <- 1 #um
small_tier_upper_size <- 5000 #um #size to align to
upper.tissue.trans.size.um <- 83 #um #set size for filtering data and x2M
# larger size bin
large_tier_lower_size <- 1 #um
large_tier_upper_size <- 5000 #um
```

```{r Aesthetics, include = FALSE}

#Theme
theme.type <- theme_bw(base_size = 15) +
              theme(plot.title = element_text(hjust = 0.5),
                    plot.subtitle = element_text(hjust = 0.5))

#Fill
fill.type <-  scale_fill_nejm()

#Color
color.type <- scale_color_nejm()

#Manually assign colors to taxa for SSD plots
group.colors <- c(Algae = "#BC3C29FF", Cnidaria = "#0072B5FF", Crustacea = "#E18727FF", Echinoderm = "#20854EFF", Fish = "#7876B1FF", Mollusca = "#6F99ADFF", Rotifera = "#EE4C97FF")

```

```{r Data Import, include = FALSE}
#Load aoc_z into dataframe. This file is generated from RDA_Maker.R
#Source("Tox Data/RDA_Maker.R")
aoc_z <- readRDS(file = "Tox Data/aoc_z.Rda") %>%
#Remove 26C temperature treatment data from Jaimukar et al. 2018
filter(!(article == 42 & media.temp == 26))

# aoc_z <- readRDS(file = "Tox Data/aoc_z.RDS") %>%    #Remove leachate and additive/chemical transfer experiments
#   tidyr::replace_na(list(chem.exp.typ.nominal = "Particle Only")) %>%
#   dplyr::filter(leachate.only != "Y") %>%
#   mutate(chem.exp.typ.nominal_f = factor(case_when(chem.exp.typ.nominal == "Particle Only" ~ "Particle Only",
#                                                    chem.exp.typ.nominal == "co.exp" ~ "Chemical Co-Exposure",
#                                                    chem.exp.typ.nominal == "sorbed" ~ "Chemical Transfer"))) %>%
#   dplyr::filter(chem.exp.typ.nominal_f == "Particle Only") #rShiny file
```

#Framework Summary

![Agreed-upon framework](www/Treshold_Framework.png)

The figure above displays the working threshold framework for the ambient threshold group for the Microplastics Health Effects Workshop. 

The current framework for generating SSDs includes the following parameters:

Data pertaining to only aquatic organisms are included (marine and freshwater).  

All taxa are included with the exception of bacterium and plants.  

HONEC (Highest Observed No Effect Concentration) are excluded.  

Assessment factors are applied to convert all other effect metrics into NOECs:

![](www/AF_EM.png)

Reference: Wigger et al. 2020 (doi: 10.1002/ieam.4214)  

Assessment factors are applied to convert acute data into chronic:  

![](www/AF_chronic.png)

Green rows are from Wigger et al. 2020. Grey rows were agreed upon by the ambient threshold working group.

Reference: Wigger et al. 2020 (doi: 10.1002/ieam.4214)

Note: Values may be slightly different than those presented in the shiny app as concentrations are converted from particles/mL to particles/L ahead of calculations.

# ERM/SSD Calculations
The following script utilizes the above equations to calculate *a priori* effect thresholds for each of the ERM of interest for each species in the database, then calculates species sensitivity distributions using each ERM.

*Data Filtering by Quality*
```{r}
## First filter data with global filters
aoc_intermediate <- aoc_z %>% 
  filter(!environment %in% c("Terrestrial", "Not Reported"),#, "Marine"),
                             #, "Freshwater"),
         org_f != "Bacterium",
         org_f != "Plant",
         #Group != "Algae",
    effect.metric != "HONEC",
    tier_zero_tech_f == "Red Criteria Passed",
         tier_zero_risk_f == "Red Criteria Passed" #All thresholds must pass technical and risk red criteria
         ) %>% 
  filter(risk.13 != 0) %>%  #Drop studies that received a score of 0 for endpoints criteria (this also drops studies that have not yet been scored) - KEEP THIS AFTER THE RED CRITERIA FILTERS  
  mutate(max.size.ingest.um = 1000 * max.size.ingest.mm)  #makes it less confusing below
#### Ecologically Relevant Metric calculations ####

#examine available data as histogram
Group_size <- aoc_intermediate %>% 
  ggplot(aes(x = size.length.um.used.for.conversions, fill = Group)) +
  geom_histogram(bins = 25) +
  scale_fill_manual(values = cal_palette(name = "superbloom1", type = "continuous", n = 9)) +
  labs(title = "Filtered Data by Group") +
  ylab("# of observations")+
  xlab("Length (um)") +
  theme.type +
  scale_x_log10()

endpoint_size <- aoc_intermediate %>% 
  ggplot(aes(x = size.length.um.used.for.conversions, fill = lvl1_f)) +
  geom_histogram(bins = 25) +
   scale_fill_manual(name = "General Endpoint", values = cal_palette(name = "superbloom3", type = "continuous", n = 9)) +
  xlab("Particle Length (um)") +
  ylab("# of observations")+
  labs(title = "Filtered Data by General Endpoint") +
  theme.type +
  scale_x_log10()

#arrange histograms
grid.arrange(Group_size, endpoint_size)
```
```{r}
#examine available data as bar plots
species_size_f <- aoc_intermediate %>% 
  distinct(Species, size_f) %>% 
  ggplot(aes(x = size_f, fill = Species)) +
  geom_bar() +
  scale_fill_manual(values = cal_palette(name = "superbloom1", type = "continuous", n = 35)) +
  labs(title = "Number of Species by Particle Length",
       subtitle = "Red criteria passed only, aquatic species (subcell to population organization)") +
  xlab("Length (um)") +
  ylab("# of Species")+
  theme.type 
  
  
endpoint_size_f <- aoc_intermediate %>% 
   distinct(lvl1_f, size_f) %>% 
  ggplot(aes(x = size_f, fill = lvl1_f)) +
  geom_bar() +
  scale_fill_manual(name = "General Endpoint", values = cal_palette(name = "superbloom3", type = "continuous", n = 6)) +
  labs(title = "Filtered Data by General Endpoint",
       subtitle = "Red criteria passed only, aquatic species (subcell to population organization)") +
  ylab("# of Endpoints")+
  xlab("Particle Length (um)") +
  theme.type 

#arrange histograms
grid.arrange(species_size_f, endpoint_size_f)
```
Do these size bins make sense for the endpoints of interest? I.e.food dilution and ROS.
```{r}
# determine if ROS is shown for any studies (no filtering)
ROS_size_allData <- aoc_z %>% 
  filter(lvl2_f %in% "Oxidative Stress") %>% 
  ggplot(aes(x = size.length.um.used.for.conversions, fill = lvl2_f)) +
  geom_histogram(bins = 25) +
  xlab("Length (um)") +
  ylab("# of observations")+
  labs(title = "All Data by Oxidative Stress",
       subtitle = "Includes all data - no filters for quality") +
  theme.type +
  scale_x_log10()

# determine if ROS is shown for organismal studies with high quality
ROS_size_filteredData <- aoc_intermediate %>%
   #filter(#risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
    #     bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
  filter(lvl2_f %in% "Oxidative Stress") %>% 
  ggplot(aes(x = size.length.um.used.for.conversions, fill = lvl2_f)) +
  geom_histogram(bins = 25) +
  xlab("Length (um)") +
  ylab("# of observations")+
  labs(title = "Data by Oxidative Stress",
       subtitle = "Includes only high-quality data") +
  theme.type +
  scale_x_log10()

grid.arrange(ROS_size_allData, ROS_size_filteredData)
```

To account for polydisperse studies, a separate equation must be used (see SI).

### Data Alignments

#### Equations
```{r}
###function to derive correction factor (CF) from Koelmans et al (equation 2)
CFfnx = function(a, #default alpha from Koelmans et al (2020)
                 x2D, #set detault values to convert ranges to (1-5,000 um) #5mm is upper defuault 
                 x1D, #1 um is lower default size
                 x2M, x1M){
  CF = (x2D^(1-a)-x1D^(1-a))/(x2M^(1-a)-x1M^(1-a)) 
  return(CF)}

#### equations for mu_x_poly (note that there are three depending on certain alphas for limits of equation)
#generalizable if a.x =2 or not
mux.polyfnx_generalizable = Vectorize(function(a.x, x_UL, x_LL){
  if(a.x == 1){ # in case a.x = 1
    mux.poly = (x_UL - x_LL)/(log(x_UL/x_LL))
    return(mux.poly)}
  if(a.x == 2){ # in case a.x = 2
     mux.poly = (log10(x_UL/x_LL))/(x_LL^(-1) - x_UL^-1)
     return(mux.poly)}
  else{ #in case alpha is not 2 or 1
    mux.poly = ((1-a.x)/(2-a.x)) * ((x_UL^(2-a.x) - x_LL^(2-a.x))/(x_UL^(1-a.x) - x_LL^(1-a.x)))
    return(mux.poly)}
  },
  vectorize.args = "a.x") # if Vectorize isn't here, the if else won't work
## ^^ Note that the above generalizable function doesn't play well with mutate(case_when), likely due to some bug with dplyr. I don't have a solution to this, so a special equation will need to be used when those values are used...

#in case alpha is not 1 or 2
mux.polyfnx = function(a.x, x_UL, x_LL){
    mux.poly = ((1-a.x)/(2-a.x)) * ((x_UL^(2-a.x) - x_LL^(2-a.x))/(x_UL^(1-a.x) - x_LL^(1-a.x)))
    return(mux.poly)}

##### If alpha does equal 2 #####
mux.polyfnx2 = function(a.x, x_UL,x_LL){
  mux.poly = (log(x_UL/x_LL))/(x_LL^(-1) - x_UL^-1)
  return(mux.poly)}

##### If alpha equals 1 #####
mux.polyfnx1 = function(a.x, x_UL, x_LL){
     mux.poly = (x_UL - x_LL)/(log(x_UL/x_LL)) #natural log
    return(mux.poly)}

### Calculating max ingestible parameters ###
## function to calcualte min and max ingestible surface area ##
SAfnx = function(a, # a = 0.5 * length
                 b, # b = 0.5 * width
                 c # c = 0.5 * height (note that hieght is 0.67 * width)
){
  SA = 4*pi*(((a*b)^1.6 + (a*c)^1.6 + (b*c)^1.6) / 3)^(1/1.6)
  return(SA)}

## max ingestible volume ##

volumefnx = function(R, L){
  volume = 0.111667 * pi * R^2 * L^3 #assumes height = 0.67 * Width, and Width:Length ratio is 'R' (compartment-specific)
  return(volume)}

volumefnx_poly = function(width, length){
  height = width #0.67 * width
  volume = (4/3) * pi * (length/2) * (width/2) * (height/2) #assumes height = 0.67 * Width 
  return(volume)}

#max ingestible mass (only used for mu_mono calculations)
massfnx = function(R, L, p){
  mass = p * #density (g/cm^3)
    0.111667 * pi * R^2 * L^3 * # volume (um^3): assumes height = 0.67 * Width, and Width:Length ratio is 'R' (compartment-specific)
    1/1e12 * 1e6 #correction factor
  return(mass)}

massfnx_poly = function(width, length, p){
  height = width #0.67 * width
  volume = (4/3) * pi * (length/2) * (width/2) * (height/2) #assumes height = 0.67 * Width 
  mass = p * #density (g/cm^3)
    volume * # volume (um^3): assumes height = 0.67 * Width, and Width:Length ratio is 'R' (compartment-specific)
    1/1e12 * 1e6 #correction factor
  return(mass)}

#max ingestible specific surface area
SSAfnx = function(sa, #surface area, calcaulted elsewhere
                  m){ #mass, calculated elsewhere
  SSA = sa/m
    return(SSA)}

#max ingestible specific surface area
SSA.inversefnx = function(sa, #surface area, calcaulted elsewhere
                  m){ #mass, calculated elsewhere
  SSA.inverse = m / sa
    return(SSA.inverse)}

## parametrization ##
# Define params for correction #
alpha.marine = 2.07 #table s4 for marine surface water. length
alpha.freshwater = 2.64 #table s4 for freshwater surface water. length
# define parameters for power law coefficients
a.sa.marine = 1.5 #marine surface area power law
a.sa.freshwater = 2.00 #freshwater surface area power law
a.v.marine = 1.48 #a_V for marine surface water volume
a.v.freshwater = 1.68 #a_V for Freshwater surface water volume
a.m.marine = 1.32 # upper limit fora_m for mass for marine surface water in table S4
a.m.freshwater = 1.65 # upper limit fora_m for mass for freshwater surface water in table S4 
a.ssa.marine = 1.98 # A_S1/SA for marine surface water
a.ssa.freshwater = 2.71 # A_1/SSA for freshwater surface water

#define additional parameters for calculations based on averages in the environment
R.ave.marine = 0.77 #average width to length ratio for microplastics in marine enviornment
p.ave.marine = 1.10 #average density in marine surface water
R.ave.freshwater = 0.67 #average width to length ratio for microplastics in surface freshwater
p.ave.freshwater = 1.04 #average density in freshwater surface water

#join alpha values for each data point
aoc_intermediate_alphas <- aoc_intermediate %>% 
  mutate(alpha = alpha.marine) %>% 
  mutate(a.sa = a.sa.marine) %>% 
   mutate(a.v =  a.v.marine) %>% 
   mutate(a.m =  a.m.marine) %>% 
   mutate(a.ssa = a.ssa.marine) %>% 
   mutate(R.ave = R.ave.marine) %>% 
   mutate(p.ave = p.ave.marine)
```

##### Tissue Translocation 
```{r}
##### SMALL TIER DATA ####
### Set size limits for tier
x2D_set = small_tier_upper_size #upper size range (default)
x1D_set = small_tier_lower_size #lower size range (default)
x1M_set = small_tier_lower_size #lower size range for measured


# calculate ERM for each species
aoc_final_small <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>%  #set to 10um for upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper translocatable
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, #length
                         b = 0.5 * x1D_set, #0.5 * R.ave * x1D_set, #width
                         c = 0.5 * x1D_set  #0.5 * R.ave * 0.67 * x1D_set #height
                         )) %>%  
  #calculate upper translocatable surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, 
                         b = 0.5 * x2M, #width #0.5 * R.ave * x2M, 
                         c = 0.5 * x2M #heigth #0.5 * R.ave * 0.67 * x2M
                         )) %>%  
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
  
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable volume 
  mutate(x_LL_v = volumefnx_poly(width = x1D_set, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
##--- environmental calculations ---###
  #calculate lower translocatable mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               length = x2M, # upper width
                               p = p.ave)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
  ##### specific surface area ERM ####
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower translocatable 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper translocatable SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```
##### Ingestion
```{r Data Alignments-2}
##### LARGE TIER DATA ####
### Set size limits for tier
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

# Quick Build
## SSD Functions
```{r}
#### define function for SSD generation for tier 1 ####
SSD_function_t1 <- function(filtered.data, hcxlcl){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hc5lcl <- c(preds$lcl[hcxlcl]) #CI95
#values to extract
print(hc5lcl)
}

# Leave-one-out by study function
sensitivity_t1 <- Vectorize(function(x, y) {
  train <- filtered.data[!filtered.data$doi %in% x,]
   SSD_function_t1(train, hcxlcl = y)
})

#### define function for SSD generation for tier 2 ####
SSD_function_t2 <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hc <- c(preds$est[hcx]) #HC5
hc.lower.95 <- c(preds$lcl[hcx])
hc.upper.95 <- c(preds$ucl[hcx])
#values to extract
print(hc)
}

#### define function for SSD generation for tier 2 ####
SSD_function_t2_uncertainty <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hc <- c(preds$est[hcx]) #HC5
hc.lower.95 <- c(preds$lcl[hcx])
hc.upper.95 <- c(preds$ucl[hcx])
#values to extract
#print(hc5)
print(c(hc, hc.lower.95, hc.upper.95))
}

#### define function for SSD generation for tiers 3 and 4 ####
SSD_function_t3_4_uncertainty <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  #filter specific things for tiers 3 and 4
   filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.50))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hc <- c(preds$est[hcx]) #HC5
hc.lower.95 <- c(preds$lcl[hcx])
hc.upper.95 <- c(preds$ucl[hcx])
#values to extract
#print(hc5)
print(c(hc, hc.lower.95, hc.upper.95))
}

#### define function for SSD generation for tiers 3 and 4 ####
SSD_function_t3_4 <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  #filter specific things for tiers 3 and 4
   filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.50))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hc <- c(preds$est[hcx]) #HC5
hc.lower.95 <- c(preds$lcl[hcx])
hc.upper.95 <- c(preds$ucl[hcx])
#values to extract
print(hc)
}
```


## Calculate Thresholds
```{r include=FALSE}
####---- Food Dilution ------#####
#### base Endpoints ###
filtered.data.large.base_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.base_t3.4 <- filtered.data.large.base_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.base.t1 <- SSD_function_t1(filtered.data = filtered.data.large.base_t1.2, hcxlcl = 5)
large.base.t2 <- SSD_function_t2_uncertainty(filtered.data = filtered.data.large.base_t1.2, hcx = 5)
large.base.t3 <- SSD_function_t3_4_uncertainty(filtered.data = filtered.data.large.base_t3.4, hcx = 5)
large.base.t4 <- SSD_function_t3_4_uncertainty(filtered.data = filtered.data.large.base_t3.4, hcx = 10)

####---- Tissue Translocation ------#####
#### base endpoints ###
filtered.data.small.base_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.base_t3.4 <- filtered.data.small.base_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.base.t1 <- SSD_function_t1(filtered.data = filtered.data.small.base_t1.2, hcxlcl = 5)
small.base.t2 <- SSD_function_t2_uncertainty(filtered.data = filtered.data.small.base_t1.2, hcx = 5)
small.base.t3 <- SSD_function_t3_4_uncertainty(filtered.data = filtered.data.small.base_t3.4, hcx = 5)
small.base.t4 <- SSD_function_t3_4_uncertainty(filtered.data = filtered.data.small.base_t3.4, hcx = 10)
```
## Summary Table

```{r}
# arrange
base_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Food Dilution (base Endpoints)" = c(large.base.t1, large.base.t2[1], large.base.t3[1], large.base.t4[1]),
  "Tissue Translocation (base Endpoints)" = c(small.base.t1, small.base.t2[1], small.base.t3[1], small.base.t4[1]),
  "Food.lower.95" = c(large.base.t1, large.base.t2[2], large.base.t3[2], large.base.t4[2]),
  "Food.upper.95" = c(large.base.t1, large.base.t2[3], large.base.t3[3], large.base.t4[3]),
  "tissue.lower.95" = c(small.base.t1, small.base.t2[2], small.base.t3[2], small.base.t4[2]),
  "tissue.upper.95" = c(small.base.t1, small.base.t2[3], small.base.t3[3], small.base.t4[3])) %>% 
  mutate_if(is.numeric, funs(format(., scientific = TRUE, digits = 3))) %>% 
  #concat
  mutate("Food Dilution (base Endpoints; mean with 95% CI's)" = paste(`Food Dilution (base Endpoints)`, "(", Food.lower.95, "to", Food.upper.95, ")"),
         "Tissue Translocation (base Endpoints; mean with 95% CI's)" = paste(`Tissue Translocation (base Endpoints)`, "(", tissue.lower.95, "to", tissue.upper.95, ")")) %>% 
  dplyr::select(c("Tier", "Food Dilution (base Endpoints; mean with 95% CI's)","Tissue Translocation (base Endpoints; mean with 95% CI's)" ))

#print
kable(base_thresholds)
```


# Verbose Build
### Assign Environment  
```{r}
aoc_final_large_marine <- aoc_final_large# %>% 
#  filter(environment == "Marine")
```


## ERM = Unaligned
```{r}
ERM <- "Unaligned"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = dose.particles.mL.master / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions,large_tier_lower_size, large_tier_upper_size)) 

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = dose.particles.mL.master / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, small_tier_lower_size, upper.tissue.trans.size.um))
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build
##### Data Collapse
```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

#add unique ID for multiplot
pred_tier1_2_large_1q_unaligned <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_unaligned <- SSD_tier1_2_large_1q

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "black") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group), size = 2) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  #   labs(title = "Microplastics Species Sensitivity Distribution",
  #        subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
  #        title = "Tier 1 & 2, 1-5000 µm", 
        x = "Particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier1_2_large_1q_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_small_1q_unaligned <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_unaligned <- SSD_tier1_2_small_1q

#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier1_2_small_1q_ggplot)
```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_u <- tier1_2_large_1q_lcl
tier2_large_u <- tier1_2_large_1q_hc5
tier1_small_u <- tier1_2_small_1q_lcl
tier2_small_u <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
```

### Tiers 3 & 4

#### Data Filtering

```{r}
#Food Dilution
tier3_4_large <- aoc_final_large_marine %>%
         filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
         mutate(dose_new = dose.particles.mL.master / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, large_tier_lower_size, large_tier_upper_size)) #%>% 
 # filter(environment == "Marine")

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier3_4_small <- aoc_final_small %>%
         filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
         mutate(dose_new = dose.particles.mL.master / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, small_tier_lower_size, upper.tissue.trans.size.um))  
  # filter(environment == "Marine")  
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build
##### Data Collapse
```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution Median
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

#Translocation Median
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

#add unique ID for multiplot
pred_tier3_4_large_median_unaligned <- pred_tier3_4_large_median
SSD_tier3_4_large_median_unaligned <- SSD_tier3_4_large_median

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #        subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1-5000 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier3_4_large_median_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_small_median_unaligned <- pred_tier3_4_small_median
SSD_tier3_4_small_median_unaligned <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #        subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier3_4_small_median_ggplot)
```

#### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier3_large_u <- tier3_4_large_median_hc5
tier4_large_u <- tier3_4_large_median_hc10
tier3_small_u <- tier3_4_small_median_hc5
tier4_small_u <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)
```

## ERM = Particles
```{r}
ERM <- "Particles"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r size category selection2, include=TRUE}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = EC_env_p.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, large_tier_lower_size, large_tier_upper_size))# %>% 
 # filter(environment == "Marine")

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = EC_env_p.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, small_tier_lower_size, upper.tissue.trans.size.um))  #%>% 
#  filter(environment == "Marine")
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build
##### Data Collapse
```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

#add unique ID for multiplot
pred_tier1_2_large_1q_particles <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_particles <- SSD_tier1_2_large_1q

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1-5000 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier1_2_large_1q_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_small_1q_particles <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_particles <- SSD_tier1_2_small_1q


#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier1_2_small_1q_ggplot)
```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_p <- tier1_2_large_1q_lcl
tier2_large_p <- tier1_2_large_1q_hc5
tier1_small_p <- tier1_2_small_1q_lcl
tier2_small_p <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
```

### Tiers 3 & 4

Tier 3 and 4 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria

#### Data Filtering

```{r size category selection3 - tiers 3/4, include=TRUE}

#Food Dilution
tier3_4_large <-  aoc_final_large_marine %>% 
  filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
         mutate(dose_new = EC_env_p.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, large_tier_lower_size, large_tier_upper_size))# %>% 
  #filter(environment == "Marine")

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier3_4_small <- aoc_final_small %>% 
  filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
         mutate(dose_new = EC_env_p.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, small_tier_lower_size, upper.tissue.trans.size.um))  
  # filter(environment == "Marine") 
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build
##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution Median
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

#Translocation Median
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_large_median_particles <- pred_tier3_4_large_median
SSD_tier3_4_large_median_particles <- SSD_tier3_4_large_median

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 3/4, 1-5000 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier3_4_large_median_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_small_median_particles <- pred_tier3_4_small_median
SSD_tier3_4_small_median_particles <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 3/4, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier3_4_small_median_ggplot)
```

##### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria  

```{r include=TRUE}
#save unique ID for ERM
tier3_large_p <- tier3_4_large_median_hc5
tier4_large_p <- tier3_4_large_median_hc10
tier3_small_p <- tier3_4_small_median_hc5
tier4_small_p <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

```

## ERM = Volume

```{r}
ERM <- "Volume"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r size category selection4, include=TRUE}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) 

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Study list for manuscript
study_list_fooddilution_1_2 <-tier1_2_large %>% 
  select(doi, authors, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, polydispersity, size.length.min.um.used.for.conversions, size.length.max.um.used.for.conversions,
         size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, 
         dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_v.particles.mL, dose_new,
tech.a1, tech.a2, tech.a3, tech.a4, tech.a5, tech.a6,
tech.1, tech.2, tech.3, tech.4, tech.5, tech.6,
tech.7, tech.8, tech.9, tech.10, tech.11, tech.12,
risk.b1, risk.13, risk.14, risk.15, risk.16, risk.17,
risk.18, risk.19, risk.20) %>% 
  rename("DOI" = doi,
         "First Author" = authors,
         "Particle Morphology" = shape_f,
         "Effect Metric" = effect.metric,
         "Exposure Duration" = acute.chronic_f,
         "Polymer" = poly_f,
         "Mixture or Single Size" = polydispersity,
         "Minimum Length (polydisperse only)" = size.length.min.um.used.for.conversions,
         "Maximum Length (polydisperse only)" = size.length.max.um.used.for.conversions,
         "Original Dose (Particles/mL)" = dose.particles.mL.master, 
         "Original Dose (Particles/mL), Reported or Converted" = dose.particles.mL.master.converted.reported,
         "Original Dose (mg/L)" = dose.mg.L.master,
         "Original Dose (mg/L), Reported or Converted" = dose.mg.L.master.converted.reported,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Endpoint Category" = lvl1_f,
         "Measured Endpoint" = lvl3_f,
         "Biological Level of Organization" = bio_f,
         "Particle Size (µm)" = size.length.um.used.for.conversions, 
         "Aligned Dose, Volume (Particles/mL)" = EC_env_v.particles.mL, 
         "Aligned Dose, Volume, Assessment Factors Applied (Particles/mL)" = dose_new,
         "Test Medium Vehice Reported" = tech.a1, 
         "Administration Route Reported" = tech.a2, 
        "Test Species Reported" = tech.a3, 
      "Sample Size Reported" = tech.a4, 
      "Control Group Reported" = tech.a5, 
    "Exposure Duration Reported" = tech.a6,
  "Particle Size" = tech.1, 
  "Particle Shape" = tech.2, 
"Polymer Type" = tech.3, 
"Source of MP" = tech.4, 
"Concentration Reporting" = tech.5, 
"Chemical Purity" = tech.6,
"Contamination Prevention" = tech.7, 
"Verification of Background Contamination" = tech.8, 
"Verification of Exposure" = tech.9, 
"Homogeneity of Exposure" = tech.10, 
"Exposure Validation" = tech.11, 
"Replication" = tech.12,
"Number of MP Treament Groups" = risk.b1, 
"Endpoints" = risk.13, 
"Presence of Natural (food) Particles" = risk.14, 
"Reporting Effect Thresholds" = risk.15, 
"Quality of Dose Response Relationship" = risk.16, 
"Concentration Range Tested" = risk.17,
"Aging and Biofouling" = risk.18, 
"Diversity of MP Tested" = risk.19, 
"Exposure Time" = risk.20)

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  #volume
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))#  %>% 
 # filter(environment == "Marine")
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build

##### Data Collapse Comparison - Food Dilution - Tiers 1 & 2

```{r, include = TRUE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10}

#Create data frame with all data collapsing methods
data_collapse <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Mean = mean(dose_new),
            Median = median(dose_new),
            Minimum = min(dose_new),
            Quartile = quantile(dose_new, 0.25)) %>% 
  ungroup()

#Output dataframe from loop iterations
SSD_List <- NULL;
Prediction_List <- NULL;

#Loop to generate figures SSD for each data collapsing method
for (i in 1:4)

{ #Open loop

Conc_Type <- case_when(i == 1 ~ "Mean",
                       i == 2 ~ "Median",
                       i == 3 ~ "Minimum",
                       i == 4 ~ "Quartile")

#Give generic name to data collapsing method
data_collapse$Conc <- data_collapse[[Conc_Type]]

data_collapse_select <- data_collapse %>% 
  select(Group, Species, Conc) 
    
###### Modelling ####
distribution <- ssd_fit_dists(data_collapse_select, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(distribution) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof <- as.data.frame(ssd_gof(distribution)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
prediction <- predict(distribution, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD <- data_collapse_select[order(data_collapse_select$Conc), ]
SSD$frac <- ppoints(data_collapse_select$Conc, 0.5)

lcl <- c(prediction$lcl[5]) #CI95
hc5 <- c(prediction$est[5]) #HC5
prediction$est_format <-format(prediction$est, digits = 3, scientific = TRUE)

#Save the results of each iteration needed for plots
tmp_SSD <- SSD %>% 
  mutate(method = paste(Conc_Type))
SSD_List <- rbind(SSD_List, tmp_SSD)

tmp_pred <- prediction %>% 
  mutate(method = paste(Conc_Type))
Prediction_List <- rbind(Prediction_List, tmp_pred)

} #Close loop

FoodDilution_Tier1_2_DataCollapse <- ggplot(Prediction_List, aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", color = "method", fill = "method"), alpha = 0.2) +
      geom_line(aes_string(y = "percent/100", color = "method")) +
      # geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(x = "particles/L", fill = "Data Collapse Method", color = "Data Collapse Method") +
          fill.type + 
          color.type + 
          theme.type 

plot(FoodDilution_Tier1_2_DataCollapse)

```

##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)


#add unique ID for multiplot
pred_tier1_2_large_1q_volume <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_volume <- SSD_tier1_2_large_1q

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
      scale_color_manual(values = group.colors) +
      labs(
          # title = "Tier 1/2, 1-5000 µm", 
          x = "particles/L") +
          fill.type + 
          theme.type 

plot(tier1_2_large_1q_ggplot)

#Save to unique name for export
FoodDilution_Tier1_2_SSD <- tier1_2_large_1q_ggplot

```

Is body size related to effect threshold for Volume?
```{r}

size.volume <- aoc_final_large_marine %>%  
  #filter(effect.metric %in% c("EC50","LC50")) %>% 
  filter(lvl1_f %in% c("Fitness"))

summary(lm(data = size.volume, EC_env_v.particles.mL ~ body.length.cm))


body_size_volume <- size.volume %>%  
  #filter(effect.metric %in% c("EC50","LC50")) %>% 
  ggplot(aes(x = EC_env_v.particles.mL, y = body.length.cm, color = Group)) +
  #geom_smooth() +
  geom_point() + 
  scale_x_log10(limits = c(1e-2, 1e10)) + 
  scale_y_log10() + 
  ylab("Species Body Length (cm)") +
  xlab("Critical Effect Concentration (particles/L; volume aligned)") +
  #dark_theme_bw()
  theme_bw(base_size = 14)

body_size_volume
```
Is body size related to effect threshold for Surface Area?
```{r}
size.sa <- aoc_final_large_marine %>%  
  #filter(effect.metric %in% c("EC50","LC50")) %>% 
  filter(lvl1_f %in% c("Fitness"))

summary(lm(data = size.sa, EC_env_sa.particles.mL ~ body.length.cm))


body_size_surface_area <- size.sa %>%  
  #filter(effect.metric %in% c("EC50","LC50")) %>% 
  ggplot(aes(x = EC_env_sa.particles.mL, y = body.length.cm, color = Group)) +
  #geom_smooth() +
  geom_point() + 
  scale_x_log10(limits = c(1e-2, 1e10)) + 
  scale_y_log10() + 
  ylab("Species Body Length (cm)") +
  xlab("Critical Effect Concentration (particles/L; surface area aligned)") +
  #dark_theme_bw()
  theme_bw(base_size = 14)

body_size_surface_area 
```
###### Manuscript Figure
```{r}
body_size_figs <- ggarrange(body_size_surface_area, body_size_volume,
                            labels = c("A", "B"),
                            legend = "bottom",
                            common.legend = TRUE)
body_size_figs


ggsave(filename = "body_size_figs_SIx.jpeg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       plot = body_size_figs, width = 12, height = 8, units = "in")
```


##### Oxidative Stress (1 - 83 µm) 

```{r include=TRUE, warning=FALSE}
###### --modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)


#add unique ID for multiplot
pred_tier1_2_small_1q_volume <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_volume <- SSD_tier1_2_small_1q

#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(
      # title = "Tier 1 & 2, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier1_2_small_1q_ggplot)
```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_v <- tier1_2_large_1q_lcl
tier2_large_v <- tier1_2_large_1q_hc5
tier1_small_v <- tier1_2_small_1q_lcl
tier2_small_v <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
```

### Tiers 3 & 4

Tier 3 and 4 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria

#### Data Filtering

```{r}

#Food Dilution
tier3_4_large <-  aoc_final_large_marine %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) 

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Study list for manuscript
study_list_fooddilution_3_4 <-tier3_4_large %>% 
    select(doi, authors, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, polydispersity, size.length.min.um.used.for.conversions, size.length.max.um.used.for.conversions,
         size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, 
         dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_v.particles.mL, dose_new,
tech.a1, tech.a2, tech.a3, tech.a4, tech.a5, tech.a6,
tech.1, tech.2, tech.3, tech.4, tech.5, tech.6,
tech.7, tech.8, tech.9, tech.10, tech.11, tech.12,
risk.b1, risk.13, risk.14, risk.15, risk.16, risk.17,
risk.18, risk.19, risk.20) %>% 
  rename("DOI" = doi,
         "First Author" = authors,
         "Particle Morphology" = shape_f,
         "Effect Metric" = effect.metric,
         "Exposure Duration" = acute.chronic_f,
         "Polymer" = poly_f,
         "Mixture or Single Size" = polydispersity,
         "Minimum Length (polydisperse only)" = size.length.min.um.used.for.conversions,
         "Maximum Length (polydisperse only)" = size.length.max.um.used.for.conversions,
         "Original Dose (Particles/mL)" = dose.particles.mL.master, 
         "Original Dose (Particles/mL), Reported or Converted" = dose.particles.mL.master.converted.reported,
         "Original Dose (mg/L)" = dose.mg.L.master,
         "Original Dose (mg/L), Reported or Converted" = dose.mg.L.master.converted.reported,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Endpoint Category" = lvl1_f,
         "Measured Endpoint" = lvl3_f,
         "Biological Level of Organization" = bio_f,
         "Particle Size (µm)" = size.length.um.used.for.conversions, 
         "Aligned Dose, Volume (Particles/mL)" = EC_env_v.particles.mL, 
         "Aligned Dose, Volume, Assessment Factors Applied (Particles/mL)" = dose_new,
         "Test Medium Vehice Reported" = tech.a1, 
         "Administration Route Reported" = tech.a2, 
        "Test Species Reported" = tech.a3, 
      "Sample Size Reported" = tech.a4, 
      "Control Group Reported" = tech.a5, 
    "Exposure Duration Reported" = tech.a6,
  "Particle Size" = tech.1, 
  "Particle Shape" = tech.2, 
"Polymer Type" = tech.3, 
"Source of MP" = tech.4, 
"Concentration Reporting" = tech.5, 
"Chemical Purity" = tech.6,
"Contamination Prevention" = tech.7, 
"Verification of Background Contamination" = tech.8, 
"Verification of Exposure" = tech.9, 
"Homogeneity of Exposure" = tech.10, 
"Exposure Validation" = tech.11, 
"Replication" = tech.12,
"Number of MP Treament Groups" = risk.b1, 
"Endpoints" = risk.13, 
"Presence of Natural (food) Particles" = risk.14, 
"Reporting Effect Thresholds" = risk.15, 
"Quality of Dose Response Relationship" = risk.16, 
"Concentration Range Tested" = risk.17,
"Aging and Biofouling" = risk.18, 
"Diversity of MP Tested" = risk.19, 
"Exposure Time" = risk.20)

#Translocation
tier3_4_small <- aoc_final_small %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))  
  # filter(environment == "Marine") 
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build

##### Data Collapse Comparison - Food Dilution - Tiers 3 & 4

```{r, include = TRUE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10}

#Create data frame with all data collapsing methods
data_collapse <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Mean = mean(dose_new),
            Median = median(dose_new),
            Minimum = min(dose_new),
            Quartile = quantile(dose_new, 0.25)) %>% 
  ungroup()

#Output dataframe from loop iterations
SSD_List <- NULL;
Prediction_List <- NULL;

#Loop to generate figures SSD for each data collapsing method
for (i in 1:4)

{ #Open loop

Conc_Type <- case_when(i == 1 ~ "Mean",
                       i == 2 ~ "Median",
                       i == 3 ~ "Minimum",
                       i == 4 ~ "Quartile")

#Give generic name to data collapsing method
data_collapse$Conc <- data_collapse[[Conc_Type]]

data_collapse_select <- data_collapse %>% 
  select(Group, Species, Conc) 
    
###### Modelling ####
distribution <- ssd_fit_dists(data_collapse_select, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(distribution) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof <- as.data.frame(ssd_gof(distribution)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
prediction <- predict(distribution, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD <- data_collapse_select[order(data_collapse_select$Conc), ]
SSD$frac <- ppoints(data_collapse_select$Conc, 0.5)

lcl <- c(prediction$lcl[5]) #CI95
hc5 <- c(prediction$est[5]) #HC5
prediction$est_format <-format(prediction$est, digits = 3, scientific = TRUE)

#Save the results of each iteration needed for plots
tmp_SSD <- SSD %>% 
  mutate(method = paste(Conc_Type))
SSD_List <- rbind(SSD_List, tmp_SSD)

tmp_pred <- prediction %>% 
  mutate(method = paste(Conc_Type))
Prediction_List <- rbind(Prediction_List, tmp_pred)

} #Close loop

FoodDilution_Tier3_4_DataCollapse <- ggplot(Prediction_List, aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", color = "method", fill = "method"), alpha = 0.2) +
      geom_line(aes_string(y = "percent/100", color = "method")) +
      # geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(x = "particles/L", fill = "Data Collapse Method", color = "Data Collapse Method") +
          fill.type + 
          color.type + 
          theme.type 

plot(FoodDilution_Tier3_4_DataCollapse)

```

##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution Median
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.5))

#Translocation Median
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_large_median_volume <- pred_tier3_4_large_median
SSD_tier3_4_large_median_volume <- SSD_tier3_4_large_median

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
      scale_color_manual(values = group.colors) +
      labs(
      # title = "Tier 3/4, 1-5000 µm", 
      x = "particles/L") +
      fill.type + 
      theme.type 

plot(tier3_4_large_median_ggplot)

#Save to unique name for export
FoodDilution_Tier3_4_SSD <- tier3_4_large_median_ggplot

```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_small_median_volume <- pred_tier3_4_small_median
SSD_tier3_4_small_median_volume <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
      # title = "Tier 3/4, 1 < 83 µm", 
      x = "particles/L") +
      fill.type + #user-selected
      color.type + #user-selected
      theme.type #user theme

plot(tier3_4_small_median_ggplot)
```

##### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria  

```{r include=TRUE}
#save unique ID for ERM
tier3_large_v <- tier3_4_large_median_hc5
tier4_large_v <- tier3_4_large_median_hc10
tier3_small_v <- tier3_4_small_median_hc5
tier4_small_v <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

```

## ERM = Surface Area
```{r}
ERM <- "Surface Area"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r size category selection6, include=TRUE}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))# %>% 
 # filter(environment == "Marine")

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) 
  # filter(environment == "Marine")  
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Study list for manuscript
study_list_translocation_1_2 <-tier1_2_small %>% 
  select(doi, authors, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, polydispersity, size.length.min.um.used.for.conversions, size.length.max.um.used.for.conversions,
         size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, 
         dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_sa.particles.mL, dose_new, tech.a1, tech.a2, tech.a3, tech.a4, tech.a5, tech.a6,
tech.1, tech.2, tech.3, tech.4, tech.5, tech.6,
tech.7, tech.8, tech.9, tech.10, tech.11, tech.12,
risk.b1, risk.13, risk.14, risk.15, risk.16, risk.17,
risk.18, risk.19, risk.20) %>% 
  rename("DOI" = doi,
         "First Author" = authors,
         "Particle Morphology" = shape_f,
         "Effect Metric" = effect.metric,
         "Exposure Duration" = acute.chronic_f,
         "Polymer" = poly_f,
         "Mixture or Single Size" = polydispersity,
         "Minimum Length (polydisperse only)" = size.length.min.um.used.for.conversions,
         "Maximum Length (polydisperse only)" = size.length.max.um.used.for.conversions,
         "Original Dose (Particles/mL)" = dose.particles.mL.master, 
         "Original Dose (Particles/mL), Reported or Converted" = dose.particles.mL.master.converted.reported,
         "Original Dose (mg/L)" = dose.mg.L.master,
         "Original Dose (mg/L), Reported or Converted" = dose.mg.L.master.converted.reported,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Endpoint Category" = lvl1_f,
         "Measured Endpoint" = lvl3_f,
         "Biological Level of Organization" = bio_f,
         "Particle Size (µm)" = size.length.um.used.for.conversions, 
         "Aligned Dose, Surface Area (Particles/mL)" = EC_env_sa.particles.mL, 
         "Aligned Dose, Surface Area, Assessment Factors Applied (Particles/mL)" = dose_new, "Test Medium Vehice Reported" = tech.a1, 
         "Administration Route Reported" = tech.a2, 
        "Test Species Reported" = tech.a3, 
      "Sample Size Reported" = tech.a4, 
      "Control Group Reported" = tech.a5, 
    "Exposure Duration Reported" = tech.a6,
  "Particle Size" = tech.1, 
  "Particle Shape" = tech.2, 
"Polymer Type" = tech.3, 
"Source of MP" = tech.4, 
"Concentration Reporting" = tech.5, 
"Chemical Purity" = tech.6,
"Contamination Prevention" = tech.7, 
"Verification of Background Contamination" = tech.8, 
"Verification of Exposure" = tech.9, 
"Homogeneity of Exposure" = tech.10, 
"Exposure Validation" = tech.11, 
"Replication" = tech.12,
"Number of MP Treament Groups" = risk.b1, 
"Endpoints" = risk.13, 
"Presence of Natural (food) Particles" = risk.14, 
"Reporting Effect Thresholds" = risk.15, 
"Quality of Dose Response Relationship" = risk.16, 
"Concentration Range Tested" = risk.17,
"Aging and Biofouling" = risk.18, 
"Diversity of MP Tested" = risk.19, 
"Exposure Time" = risk.20)

```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build

##### Data Collapse Comparison - Translocation - Tiers 1 & 2

```{r, include = TRUE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10}

#Create data frame with all data collapsing methods
data_collapse <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Mean = mean(dose_new),
            Median = median(dose_new),
            Minimum = min(dose_new),
            Quartile = quantile(dose_new, 0.25)) %>% 
  ungroup()

#Output dataframe from loop iterations
SSD_List <- NULL;
Prediction_List <- NULL;

#Loop to generate figures SSD for each data collapsing method
for (i in 1:4)

{ #Open loop

Conc_Type <- case_when(i == 1 ~ "Mean",
                       i == 2 ~ "Median",
                       i == 3 ~ "Minimum",
                       i == 4 ~ "Quartile")

#Give generic name to data collapsing method
data_collapse$Conc <- data_collapse[[Conc_Type]]

data_collapse_select <- data_collapse %>% 
  select(Group, Species, Conc) 
    
###### Modelling ####
distribution <- ssd_fit_dists(data_collapse_select, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(distribution) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof <- as.data.frame(ssd_gof(distribution)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
prediction <- predict(distribution, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD <- data_collapse_select[order(data_collapse_select$Conc), ]
SSD$frac <- ppoints(data_collapse_select$Conc, 0.5)

lcl <- c(prediction$lcl[5]) #CI95
hc5 <- c(prediction$est[5]) #HC5
prediction$est_format <-format(prediction$est, digits = 3, scientific = TRUE)

#Save the results of each iteration needed for plots
tmp_SSD <- SSD %>% 
  mutate(method = paste(Conc_Type))
SSD_List <- rbind(SSD_List, tmp_SSD)

tmp_pred <- prediction %>% 
  mutate(method = paste(Conc_Type))
Prediction_List <- rbind(Prediction_List, tmp_pred)

} #Close loop

Translocation_Tier1_2_DataCollapse <- ggplot(Prediction_List, aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", color = "method", fill = "method"), alpha = 0.2) +
      geom_line(aes_string(y = "percent/100", color = "method")) +
      # geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(x = "particles/L", fill = "Data Collapse Method", color = "Data Collapse Method") +
          fill.type + 
          color.type + 
          theme.type 

plot(Translocation_Tier1_2_DataCollapse)

```

##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (10 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_large_1q_surfacearea <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_surfacearea <- SSD_tier1_2_large_1q

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
        # title = "Tier 1 & 2, 1-5000 µm", 
        x = "particles/L") +
        fill.type + 
        color.type + 
        theme.type 

plot(tier1_2_large_1q_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_small_1q_surfacearea <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_surfacearea <- SSD_tier1_2_small_1q

#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
      scale_color_manual(values = group.colors) +
      labs(
           # title = "Tier 1 & 2, 1 < 83 µm", 
           x = "particles/L") +
           fill.type + 
           theme.type 

plot(tier1_2_small_1q_ggplot)

#Save to unique name for export
Translocation_Tier1_2_SSD <- tier1_2_small_1q_ggplot

```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_sa <- tier1_2_large_1q_lcl
tier2_large_sa <- tier1_2_large_1q_hc5
tier1_small_sa <- tier1_2_small_1q_lcl
tier2_small_sa <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)

```

### Tiers 3 & 4

Tier 3 and 4 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria

#### Data Filtering

```{r size category selection7 - tiers 3/4, include=TRUE}

#Food Dilution
tier3_4_large <-  aoc_final_large_marine %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))# %>% 
  #filter(environment == "Marine")

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier3_4_small <- aoc_final_small %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))#  %>% 
 # filter(environment == "Marine")
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Study list for manuscript
study_list_translocation_3_4 <-tier3_4_small %>% 
  select(doi, authors, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, polydispersity, size.length.min.um.used.for.conversions, size.length.max.um.used.for.conversions,
         size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, 
         dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_sa.particles.mL, dose_new, tech.a1, tech.a2, tech.a3, tech.a4, tech.a5, tech.a6,
tech.1, tech.2, tech.3, tech.4, tech.5, tech.6,
tech.7, tech.8, tech.9, tech.10, tech.11, tech.12,
risk.b1, risk.13, risk.14, risk.15, risk.16, risk.17,
risk.18, risk.19, risk.20) %>% 
  rename("DOI" = doi,
         "First Author" = authors,
         "Particle Morphology" = shape_f,
         "Effect Metric" = effect.metric,
         "Exposure Duration" = acute.chronic_f,
         "Polymer" = poly_f,
         "Mixture or Single Size" = polydispersity,
         "Minimum Length (polydisperse only)" = size.length.min.um.used.for.conversions,
         "Maximum Length (polydisperse only)" = size.length.max.um.used.for.conversions,
         "Original Dose (Particles/mL)" = dose.particles.mL.master, 
         "Original Dose (Particles/mL), Reported or Converted" = dose.particles.mL.master.converted.reported,
         "Original Dose (mg/L)" = dose.mg.L.master,
         "Original Dose (mg/L), Reported or Converted" = dose.mg.L.master.converted.reported,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Endpoint Category" = lvl1_f,
         "Measured Endpoint" = lvl3_f,
         "Biological Level of Organization" = bio_f,
         "Particle Size (µm)" = size.length.um.used.for.conversions, 
         "Aligned Dose, Surface Area (Particles/mL)" = EC_env_sa.particles.mL, 
         "Aligned Dose, Surface Area, Assessment Factors Applied (Particles/mL)" = dose_new, "Test Medium Vehice Reported" = tech.a1, 
         "Administration Route Reported" = tech.a2, 
        "Test Species Reported" = tech.a3, 
      "Sample Size Reported" = tech.a4, 
      "Control Group Reported" = tech.a5, 
    "Exposure Duration Reported" = tech.a6,
  "Particle Size" = tech.1, 
  "Particle Shape" = tech.2, 
"Polymer Type" = tech.3, 
"Source of MP" = tech.4, 
"Concentration Reporting" = tech.5, 
"Chemical Purity" = tech.6,
"Contamination Prevention" = tech.7, 
"Verification of Background Contamination" = tech.8, 
"Verification of Exposure" = tech.9, 
"Homogeneity of Exposure" = tech.10, 
"Exposure Validation" = tech.11, 
"Replication" = tech.12,
"Number of MP Treament Groups" = risk.b1, 
"Endpoints" = risk.13, 
"Presence of Natural (food) Particles" = risk.14, 
"Reporting Effect Thresholds" = risk.15, 
"Quality of Dose Response Relationship" = risk.16, 
"Concentration Range Tested" = risk.17,
"Aging and Biofouling" = risk.18, 
"Diversity of MP Tested" = risk.19, 
"Exposure Time" = risk.20)
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build

##### Data Collapse Comparison - Translocation - Tiers 3 & 4

```{r, include = TRUE, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 10}

#Create data frame with all data collapsing methods
data_collapse <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Mean = mean(dose_new),
            Median = median(dose_new),
            Minimum = min(dose_new),
            Quartile = quantile(dose_new, 0.25)) %>% 
  ungroup()

#Output dataframe from loop iterations
SSD_List <- NULL;
Prediction_List <- NULL;

#Loop to generate figures SSD for each data collapsing method
for (i in 1:4)

{ #Open loop

Conc_Type <- case_when(i == 1 ~ "Mean",
                       i == 2 ~ "Median",
                       i == 3 ~ "Minimum",
                       i == 4 ~ "Quartile")

#Give generic name to data collapsing method
data_collapse$Conc <- data_collapse[[Conc_Type]]

data_collapse_select <- data_collapse %>% 
  select(Group, Species, Conc) 
    
###### Modelling ####
distribution <- ssd_fit_dists(data_collapse_select, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(distribution) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof <- as.data.frame(ssd_gof(distribution)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
prediction <- predict(distribution, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD <- data_collapse_select[order(data_collapse_select$Conc), ]
SSD$frac <- ppoints(data_collapse_select$Conc, 0.5)

lcl <- c(prediction$lcl[5]) #CI95
hc5 <- c(prediction$est[5]) #HC5
prediction$est_format <-format(prediction$est, digits = 3, scientific = TRUE)

#Save the results of each iteration needed for plots
tmp_SSD <- SSD %>% 
  mutate(method = paste(Conc_Type))
SSD_List <- rbind(SSD_List, tmp_SSD)

tmp_pred <- prediction %>% 
  mutate(method = paste(Conc_Type))
Prediction_List <- rbind(Prediction_List, tmp_pred)

} #Close loop

Translocation_Tier3_4_DataCollapse <- ggplot(Prediction_List, aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", color = "method", fill = "method"), alpha = 0.2) +
      geom_line(aes_string(y = "percent/100", color = "method")) +
      # geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(x = "particles/L", fill = "Data Collapse Method", color = "Data Collapse Method") +
          fill.type + 
          color.type + 
          theme.type 

plot(Translocation_Tier3_4_DataCollapse)

```

##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#First quartile large particles
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

#First quartile small particles
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_large_median_surfacearea <- pred_tier3_4_large_median
SSD_tier3_4_large_median_surfacearea <- SSD_tier3_4_large_median

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 3/4, 1-5000 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier3_4_large_median_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_small_median_surfacearea <- pred_tier3_4_small_median
SSD_tier3_4_small_median_surfacearea <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      scale_color_manual(values = group.colors) +
      labs(
          # title = "Tier 3/4, 1 < 83 µm", 
          x = "particles/L") +
          fill.type + 
          theme.type 

plot(tier3_4_small_median_ggplot)

#Save to unique name for export
Translocation_Tier3_4_SSD <- tier3_4_small_median_ggplot

```

##### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria  

```{r include=TRUE}
#save unique ID for ERM
tier3_large_sa <- tier3_4_large_median_hc5
tier4_large_sa <- tier3_4_large_median_hc10
tier3_small_sa <- tier3_4_small_median_hc5
tier4_small_sa <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

```

## ERM = Mass
```{r}
ERM <- "Mass"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r size category selection8, include=TRUE}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = EC_env_m.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))# %>% 
#  filter(environment == "Marine")

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = EC_env_m.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))  
  # filter(environment == "Marine") 
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build
##### Data Collapse
```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_large_1q_mass <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_mass <- SSD_tier1_2_large_1q

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 1 & 2, 1-5000 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier1_2_large_1q_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_small_1q_mass <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_mass <- SSD_tier1_2_small_1q

#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 1 & 2, 1 < 83 µm", 
          x = "particles/L") +
          fill.type + 
          color.type +
          theme.type 

plot(tier1_2_small_1q_ggplot)
```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_m <- tier1_2_large_1q_lcl
tier2_large_m <- tier1_2_large_1q_hc5
tier1_small_m <- tier1_2_small_1q_lcl
tier2_small_m <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)

```

### Tiers 3 & 4

Tier 3 and 4 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria

#### Data Filtering

```{r size category selection9 - tiers 3/4, include=TRUE}

#Food Dilution
tier3_4_large <-  aoc_final_large_marine %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_m.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))# %>% 
  #filter(environment == "Marine")

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier3_4_small <- aoc_final_small %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_m.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))  
  # filter(environment == "Marine") 
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build

##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution Median
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

#Translocation Median
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (10 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### --modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_large_median_mass <- pred_tier3_4_large_median
SSD_tier3_4_large_median_mass <- SSD_tier3_4_large_median

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #      subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 3/4, 1-5000 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier3_4_large_median_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_small_median_mass <- pred_tier3_4_small_median
SSD_tier3_4_small_median_mass <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #            subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 3/4, 1 < 83 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier3_4_small_median_ggplot)
```

##### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria  

```{r include=TRUE}
#save unique ID for ERM
tier3_large_m <- tier3_4_large_median_hc5
tier4_large_m <- tier3_4_large_median_hc10
tier3_small_m <- tier3_4_small_median_hc5
tier4_small_m <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

```

## ERM = Specific Surface Area
```{r}
ERM <- "Specific Surface Area"
```

### Tiers 1 & 2

Tier 1 and 2 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

#### Data Filtering

```{r size category selection6, include=TRUE}
#Food Dilution
tier1_2_large <- aoc_final_large_marine %>% 
         mutate(dose_new = EC_env_ssa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, large_tier_lower_size, large_tier_upper_size)) 
  # filter(environment == "Marine") 

# Count observations and studies
tier1_2_large.obs <- tier1_2_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier1_2_small <- aoc_final_small %>% 
         mutate(dose_new = EC_env_ssa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, small_tier_lower_size, upper.tissue.trans.size.um)) 
  # filter(environment == "Marine")  
  
# Count observations and studies
tier1_2_small.obs <- tier1_2_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_large)

#small
endpoints_tier1_2_small <- tier1_2_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier1_2_small)
```

#### SSD Build
##### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution First Quartile
tier1_2_large_1q <- tier1_2_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

#Translocation First Quartile
tier1_2_small_1q <- tier1_2_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))

```

##### Food Dilution (10 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_large_1q <- ssd_fit_dists(tier1_2_large_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_large_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_large_1q <- as.data.frame(ssd_gof(dists_tier1_2_large_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_large_1q <- predict(dists_tier1_2_large_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_large_1q <- tier1_2_large_1q[order(tier1_2_large_1q$Conc), ]
SSD_tier1_2_large_1q$frac <- ppoints(tier1_2_large_1q$Conc, 0.5)

tier1_2_large_1q_lcl <- c(pred_tier1_2_large_1q$lcl[5]) #CI95
tier1_2_large_1q_hc5 <- c(pred_tier1_2_large_1q$est[5]) #HC5
pred_tier1_2_large_1q$est_format <-format(pred_tier1_2_large_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_large_1q_specificsurfacearea <- pred_tier1_2_large_1q
SSD_tier1_2_large_1q_specificsurfacearea <- SSD_tier1_2_large_1q

#ggplot
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_large_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #            subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 1 & 2, 1-5000 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier1_2_large_1q_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier1_2_small_1q <- ssd_fit_dists(tier1_2_small_1q, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier1_2_small_1q) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier1_2_small_1q <- as.data.frame(ssd_gof(dists_tier1_2_small_1q)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier1_2_small_1q <- predict(dists_tier1_2_small_1q, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier1_2_small_1q <- tier1_2_small_1q[order(tier1_2_small_1q$Conc), ]
SSD_tier1_2_small_1q$frac <- ppoints(tier1_2_small_1q$Conc, 0.5)

tier1_2_small_1q_lcl <- c(pred_tier1_2_small_1q$lcl[5]) #CI95
tier1_2_small_1q_hc5 <- c(pred_tier1_2_small_1q$est[5]) #HC5
pred_tier1_2_small_1q$est_format <-format(pred_tier1_2_small_1q$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier1_2_small_1q_specificsurfacearea <- pred_tier1_2_small_1q
SSD_tier1_2_small_1q_specificsurfacearea <- SSD_tier1_2_small_1q

#ggplot
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier1_2_small_1q,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier1_2_small_1q, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #            subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 1 & 2, 1 < 83 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier1_2_small_1q_ggplot)
```

#### Tier 1 & 2 Summary Table

Hazard Confidence Level = HC5  

Endpoints = All  

Levels of Biological Organization = All  

HC Metric = 95% CI (Tier 1); Mean (Tier 2)  

Data Quality = Pass Technical Quality Red Criteria

```{r}
#save unique ID for ERM
tier1_large_ssa <- tier1_2_large_1q_lcl
tier2_large_ssa <- tier1_2_large_1q_hc5
tier1_small_ssa <- tier1_2_small_1q_lcl
tier2_small_ssa <- tier1_2_small_1q_hc5

larges <- list(tier1_2_large_1q_lcl, tier1_2_large_1q_hc5,
              tier1_2_large.obs[[1,1]], #n.obs
              tier1_2_large.obs[[1,2]], #n.studies
              tier1_2_large.obs[[1,3]], #n.taxa
              tier1_2_large.obs[[1,4]]) #n.species

smalls <- list(tier1_2_small_1q_lcl, tier1_2_small_1q_hc5,
              tier1_2_small.obs[[1,1]], #n.obs
              tier1_2_small.obs[[1,2]], #n.studies
              tier1_2_small.obs[[1,3]], #n.taxa
              tier1_2_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_1", "Tier_2", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 1 & 2, 1um-5mm", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 1 & 2, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)

```

### Tiers 3 & 4

Tier 3 and 4 are the same SSD. A different HC metric is selected to differentiate between the tiers. Specifically, Tier 1 uses the 95% CI whereas Tier 2 uses the Mean. See below for more details of specific parameters. 

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria

#### Data Filtering

```{r size category selection7 - tiers 3/4, include=TRUE}

#Food Dilution
tier3_4_large <-  aoc_final_large_marine %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) #%>% 
 # filter(environment == "Marine")

# Count observations and studies
tier3_4_large.obs <- tier3_4_large %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))

#Translocation
tier3_4_small <- aoc_final_small %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) # %>% 
 # filter(environment == "Marine")
  
# Count observations and studies
tier3_4_small.obs <- tier3_4_small %>% summarize(n.obs = n(), n.studies = n_distinct(doi), n.taxa = n_distinct(org_f), n.species = n_distinct(species_f))
```

#### Endpoint Representation

```{r, include = TRUE, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 10}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_large)

#small
endpoints_tier3_4_small <- tier3_4_small %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
  group_by(lvl1_f, lvl1_count, bio_f) %>% 
  summarise() %>%  
  ggplot(aes(x = lvl1_f, y=lvl1_count, fill = bio_f))+
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tier 1 & 2, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
        

plot(endpoints_tier3_4_small)
```

#### SSD Build
#### Data Collapse

```{r, include = TRUE, warning = FALSE, message = FALSE}

#Food Dilution Median
tier3_4_large_median <- tier3_4_large %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

#Translocation Median
tier3_4_small_median <- tier3_4_small %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = median(dose_new))

```

##### Food Dilution (1 - 5000 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_large_median <- ssd_fit_dists(tier3_4_large_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_large_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_large_median <- as.data.frame(ssd_gof(dists_tier3_4_large_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_large_median <- predict(dists_tier3_4_large_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_large_median <- tier3_4_large_median[order(tier3_4_large_median$Conc), ]
SSD_tier3_4_large_median$frac <- ppoints(tier3_4_large_median$Conc, 0.5)

tier3_4_large_median_hc5 <- c(pred_tier3_4_large_median$est[5]) #HC5
tier3_4_large_median_hc10 <- c(pred_tier3_4_large_median$est[10]) #HC10
pred_tier3_4_large_median$est_format <-format(pred_tier3_4_large_median$est, digits = 3, scientific = TRUE)

#add unique ID for multiplot
pred_tier3_4_large_median_specificsurfacearea <- pred_tier3_4_large_median
SSD_tier3_4_large_median_specificsurfacearea <- SSD_tier3_4_large_median

#ggplot
tier3_4_large_median_ggplot <- ggplot(pred_tier3_4_large_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_large_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_large_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Microplastics Species Sensitivity Distribution",
             subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 3/4, 1-5000 µm", x = "particles/L") +
      fill.type + 
      color.type + 
      theme.type 

plot(tier3_4_large_median_ggplot)
```

##### Translocation (1 - 83 µm) 

```{r include = TRUE, warning = FALSE, fig.height = 6, fig.width = 10}
###### Modelling ####
dists_tier3_4_small_median <- ssd_fit_dists(tier3_4_small_median, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 

#autoplot(dists_tier3_4_small_median) #plots the distribution in ggplotier2
#### Goodness of fit ####
gof_tier3_4_small_median <- as.data.frame(ssd_gof(dists_tier3_4_small_median)) %>% mutate_if(is.numeric, ~ signif(., 3))

#### Prediction ####
set.seed(99)
pred_tier3_4_small_median <- predict(dists_tier3_4_small_median, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 

#order data
SSD_tier3_4_small_median <- tier3_4_small_median[order(tier3_4_small_median$Conc), ]
SSD_tier3_4_small_median$frac <- ppoints(tier3_4_small_median$Conc, 0.5)

tier3_4_small_median_hc5 <- c(pred_tier3_4_small_median$est[5]) #HC5
tier3_4_small_median_hc10 <- c(pred_tier3_4_small_median$est[10]) #HC10
pred_tier3_4_small_median$est_format <-format(pred_tier3_4_small_median$est, digits = 3, scientific = TRUE)


#add unique ID for multiplot
pred_tier3_4_small_median_specificsurfacearea <- pred_tier3_4_small_median
SSD_tier3_4_small_median_specificsurfacearea <- SSD_tier3_4_small_median

#ggplot
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "lightgrey") +
      geom_line(aes_string(y = "percent/100"), color = "gray") +
      geom_point(data = SSD_tier3_4_small_median,aes(x = Conc, y =frac, color = Group)) + 
      geom_text_repel(data = SSD_tier3_4_small_median, aes(x = Conc, y = frac, label = Species), color = "black", size = 3.5, segment.alpha = 0.5, min.segment.length = 0, direction = "x", hjust = -1) + 
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
      # labs(title = "Microplastics Species Sensitivity Distribution",
      #            subtitle = paste("(ERM = ",ERM,")")) +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
      labs(
          # title = "Tier 3/4, 1 < 83 µm", 
          x = "particles/L") +
          fill.type + 
          color.type + 
          theme.type 

plot(tier3_4_small_median_ggplot)
```

##### Tier 3 & 4 Summary Table

Hazard Confidence Level = HC5(Tier3)/HC10(Tier4)  

Endpoints = All  

Levels of Biological Organization = Organism and Population  

HC Metric = Mean  

Data Quality = Pass Technical & Risk Quality Red Criteria  

```{r include=TRUE}
#save unique ID for ERM
tier3_large_ssa <- tier3_4_large_median_hc5
tier4_large_ssa <- tier3_4_large_median_hc10
tier3_small_ssa <- tier3_4_small_median_hc5
tier4_small_ssa <- tier3_4_small_median_hc10

larges <- list(tier3_4_large_median_hc5, tier3_4_large_median_hc10,
              tier3_4_large.obs[[1,1]], #n.obs
              tier3_4_large.obs[[1,2]], #n.studies
              tier3_4_large.obs[[1,3]], #n.taxa
              tier3_4_large.obs[[1,4]]) #n.species

smalls <- list(tier3_4_small_median_hc5, tier3_4_small_median_hc10,
              tier3_4_small.obs[[1,1]], #n.obs
              tier3_4_small.obs[[1,2]], #n.studies
              tier3_4_small.obs[[1,3]], #n.taxa
              tier3_4_small.obs[[1,4]]) #n.species

# Convert to data frames
largesdf <- data.frame(matrix(unlist(larges), ncol = 6))
smallsdf <- data.frame(matrix(unlist(smalls), ncol = 6))

# Rename columns
colnames(largesdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")
colnames(smallsdf) <- c("Tier_3", "Tier_4", "N_Obs.", "N_Doi", "N_Taxa", "N_Species")

# Make tables
gt(largesdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-5000 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3 & 4, 1-83um ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

```

#Study List & Figure Export for Manuscript

```{r, fig.height = 12, fig.width = 14}
#Study Lists
list_of_datasets <- list("Thresholds 1_2,1-5000um,Vol" = study_list_fooddilution_1_2, 
                         "Thresholds 3_4,1-5000um,Vol" = study_list_fooddilution_3_4, 
                         "Thresholds 1_2,1-83um,SA" = study_list_translocation_1_2, 
                         "Thresholds 3_4, 1-83um,SA" = study_list_translocation_3_4)


write.xlsx(list_of_datasets, file = "./Concentration data/Threshold_Manuscript_Figs/SSD_rawdata_102121.xlsx")

#Data Collapse

Data_Collapse_Figure <- ggarrange(FoodDilution_Tier1_2_DataCollapse, FoodDilution_Tier3_4_DataCollapse,
                                  Translocation_Tier1_2_DataCollapse, Translocation_Tier3_4_DataCollapse,
                        labels = c("A", "B", "C", "D"), common.legend = TRUE, legend = "bottom",
                        ncol = 2, nrow = 2)

plot(Data_Collapse_Figure)

ggsave(filename = "Data_Collapse_Figure_SI2_10521.jpg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       plot = Data_Collapse_Figure, width = 12, height = 10, units = "in")

#SSDs

SSD_Figure <- ggarrange(FoodDilution_Tier1_2_SSD, FoodDilution_Tier3_4_SSD,
                        Translocation_Tier1_2_SSD,Translocation_Tier3_4_SSD,
                        labels = c("A", "B", "C", "D"), common.legend = TRUE, legend = "bottom", 
                        ncol = 2, nrow = 2)

plot(SSD_Figure)

ggsave(filename = "SSD_Figure_Fig2_10821.jpeg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       plot = SSD_Figure, width = 12, height = 10, units = "in",
       dpi = 300, bg = "white")

```


## Multi plots
### Larges
#### Tiers 1 & 2
```{r}
# Build dataframe with all SSDs so we can get a color scale
p_t1_l_p <- pred_tier1_2_large_1q_particles %>% mutate(alignment = "particle")
p_t1_l_u <- pred_tier1_2_large_1q_unaligned %>% mutate(alignment = "unaligned")
p_t1_l_v <- pred_tier1_2_large_1q_volume %>% mutate(alignment = "volume") %>% select(-est_format)
p_t1_l_sa <- pred_tier1_2_large_1q_surfacearea %>% mutate(alignment = "surface_area") %>% select(-est_format)
p_t1_l_m <- pred_tier1_2_large_1q_mass %>% mutate(alignment = "mass") %>% select(-est_format)
p_t1_l_ssa <- pred_tier1_2_large_1q_specificsurfacearea %>% mutate(alignment = "specific_surface_area") %>% select(-est_format)


predictions_t1_large <- rbind.data.frame(p_t1_l_p, 
                                p_t1_l_u,
                                p_t1_l_v,
                                p_t1_l_sa,
                                p_t1_l_m,
                                p_t1_l_ssa) %>% 
  mutate(alignment = factor(alignment))
predictions_t1_large

#do the same for individual points
s_t1_l_p <- SSD_tier1_2_large_1q_particles %>% mutate(alignment = "particle")
s_t1_l_u <- SSD_tier1_2_large_1q_unaligned %>% mutate(alignment = "unaligned")
s_t1_l_v <- SSD_tier1_2_large_1q_volume %>% mutate(alignment = "volume")
s_t1_l_sa <- SSD_tier1_2_large_1q_surfacearea %>% mutate(alignment = "surface_area")
s_t1_l_m <- SSD_tier1_2_large_1q_mass %>% mutate(alignment = "mass")
s_t1_l_ssa <- SSD_tier1_2_large_1q_specificsurfacearea %>% mutate(alignment = "specific_surface_area")

points_t1_large <- rbind.data.frame(s_t1_l_p, 
                                s_t1_l_u,
                                s_t1_l_v,
                                s_t1_l_sa,
                                s_t1_l_m,
                                s_t1_l_ssa)# %>% 
  #mutate(alignment = as.factor(alignment))
#convert alignment to factor
points_t1_large$alignment <- as.factor(points_t1_large$alignment)
```
##### All
```{r}
compare_ERMs_t1_2_large <- predictions_t1_large %>% 
ggplot(aes(x = est, y = percent/100, color = alignment)) +
  geom_line(size = 1) +
  #geom_xribbon(aes(xmin = lcl, xmax = ucl, y = percent/100, fill = alignment),color = "black", alpha = 0.15) +
  geom_point(data = points_t1_large,aes(x = Conc, y =frac, color = alignment)) + 
  scale_color_npg(name = "Alignment") +
  scale_fill_npg(name = "Alignment") +
  guides(fill = FALSE) + # suppress different legend for colors and lines
  coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 10),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Tiers 1 & 2, 1-5000 µm", 
       x = "particles/L (aligned to 1-5,000 µm)") +
      theme.type #user theme

compare_ERMs_t1_2_large
```

#### Tiers 3 & 4
```{r}
# Build dataframe with all SSDs so we can get a color scale
p_t3_l_p <- pred_tier3_4_large_median_particles %>% mutate(alignment = "particle") %>% select(-est_format)
p_t3_l_u <- pred_tier3_4_large_median_unaligned %>% mutate(alignment = "unaligned")
p_t3_l_v <- pred_tier3_4_large_median_volume %>% mutate(alignment = "volume") %>% select(-est_format)
p_t3_l_sa <- pred_tier3_4_large_median_surfacearea %>% mutate(alignment = "surface_area") %>% select(-est_format)
p_t3_l_m <- pred_tier3_4_large_median_mass %>% mutate(alignment = "mass") %>% select(-est_format)
p_t3_l_ssa <- pred_tier3_4_large_median_specificsurfacearea %>% mutate(alignment = "specific_surface_area") %>% select(-est_format)


predictions_t3_large <- rbind.data.frame(p_t3_l_p, 
                                p_t3_l_u,
                                p_t3_l_v,
                                p_t3_l_sa,
                                p_t3_l_m,
                                p_t3_l_ssa) %>% 
  mutate(alignment = factor(alignment))
predictions_t3_large

#do the same for individual points
s_t3_l_p <- SSD_tier3_4_large_median_particles %>% mutate(alignment = "particle")
s_t3_l_u <- SSD_tier3_4_large_median_unaligned %>% mutate(alignment = "unaligned")
s_t3_l_v <- SSD_tier3_4_large_median_volume %>% mutate(alignment = "volume")
s_t3_l_sa <- SSD_tier3_4_large_median_surfacearea %>% mutate(alignment = "surface_area")
s_t3_l_m <- SSD_tier3_4_large_median_mass %>% mutate(alignment = "mass")
s_t3_l_ssa <- SSD_tier3_4_large_median_specificsurfacearea %>% mutate(alignment = "specific_surface_area")

points_t3_large <- rbind.data.frame(s_t3_l_p, 
                                s_t3_l_u,
                                s_t3_l_v,
                                s_t3_l_sa,
                                s_t3_l_m,
                                s_t3_l_ssa)# %>% 
  #mutate(alignment = as.factor(alignment))
#convert alignment to factor
points_t3_large$alignment <- as.factor(points_t3_large$alignment)
```
##### All
```{r}
compare_ERMs_t3_4 <- predictions_t3_large %>% 
ggplot(aes(x = est, y = percent/100, color = alignment)) +
  geom_line(size = 1) +
  #geom_xribbon(aes(xmin = lcl, xmax = ucl, y = percent/100, fill = alignment),color = "black", alpha = 0.15) +
  geom_point(data = points_t3_large,aes(x = Conc, y =frac, color = alignment)) + 
  scale_color_npg(name = "Alignment") +
  scale_fill_npg(name = "Alignment") +
  guides(fill = FALSE) + # suppress different legend for colors and lines
  coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 10),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Tiers 1 & 2, 1-5000 µm", 
       x = "particles/L (aligned to 1-5,000 µm)") +
      theme.type #user theme

compare_ERMs_t3_4
```

#### Simple aligned vs unaligned
```{r}
## Particles, Mass, Volume for Tier 1
ggplot(pred_tier1_2_large_1q_particles,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, fill = "green", color = "green") +
      geom_line(aes_string(y = "percent/100"), color = "green") +
      geom_point(data = SSD_tier1_2_large_1q_particles,aes(x = Conc, y =frac), color = "green") + 
     
  # Unailgned
  geom_xribbon(data =pred_tier1_2_large_1q_unaligned, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "red", fill = "red") +
  geom_line(data = pred_tier1_2_large_1q_unaligned,aes_string(y = "percent/100"), color = "red") +
  geom_point(data = SSD_tier1_2_large_1q_unaligned,aes(x = Conc, y =frac), color = "red") + 
  
  #Volume
 # geom_point(data = SSD_tier1_2_large_1q_volume,aes(x = Conc, y =frac, color = Group)) + 
  #geom_xribbon(data =pred_tier1_2_large_1q_volume, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "red", fill = "red") +
  #Mass
 # geom_point(data = SSD_tier1_2_large_1q_mass,aes(x = Conc, y =frac, color = Group)) + 
  #geom_xribbon(data =pred_tier1_2_large_1q_mass, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "blue") +
  #surface area
 # geom_point(data = SSD_tier1_2_large_1q_surfacearea,aes(x = Conc, y =frac, color = Group)) + 
  #geom_xribbon(data =pred_tier1_2_large_1q_surfacearea, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "yellow", fill = "yellow") +
  #aesthetics
  # scale_color_manual(name = "ERM",
  #                      breaks = c("green", "red", "blue", "yellow"),
  #                      labels = c("particles","volume", "mass", "surface area"),
  #                      guide = "legend")+
      scale_color_manual(name = "alignment",
                         values = c(1,1),
                         guide = guide_legend(override.aes = list(color = c("red", "green")))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Microplastics Species Sensitivity Distribution",
             subtitle = "Red = Unaligned, Green = Particles") +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 1 & 2, 1-5000 µm", x = "particles/L") +
     # fill.type + #user-selected
    #  color.type + #user-selected
      theme.type #user theme

```
#### all ERMs
```{r}
## Particles, Mass, Volume for Tier 1
ggplot(pred_tier1_2_large_1q_particles,aes_string(x = "est")) +
      geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, fill = "green", color = "green") +
      geom_line(aes_string(y = "percent/100"), color = "green") +
      geom_point(data = SSD_tier1_2_large_1q_particles,aes(x = Conc, y =frac), color = "green") + 
  
  #unalighed
  geom_point(data = SSD_tier1_2_large_1q_unaligned,aes(x = Conc, y =frac), color = "white") + 
  geom_line(data = pred_tier1_2_large_1q_unaligned, aes_string(y = "percent/100"), color = "white") +
  geom_xribbon(data = pred_tier1_2_large_1q_unaligned, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "grey", fill = "white") +
  
  #Volume
  geom_point(data = SSD_tier1_2_large_1q_volume,aes(x = Conc, y =frac), color = "red") + 
   geom_line(data = pred_tier1_2_large_1q_volume, aes_string(y = "percent/100"), color = "red") +
  geom_xribbon(data =pred_tier1_2_large_1q_volume, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "red", fill = "red") +
  
  #Mass
  geom_point(data = SSD_tier1_2_large_1q_mass,aes(x = Conc, y =frac), color = "blue") + 
  geom_line(data = pred_tier1_2_large_1q_mass, aes_string(y = "percent/100"), color = "blue") +
  geom_xribbon(data =pred_tier1_2_large_1q_mass, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "blue") +
  
  #surface area
  geom_point(data = SSD_tier1_2_large_1q_surfacearea,aes(x = Conc, y =frac), color = "yellow") + 
  geom_line(data = pred_tier1_2_large_1q_surfacearea, aes_string(y = "percent/100"), color = "yellow") +
  geom_xribbon(data =pred_tier1_2_large_1q_surfacearea, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "yellow", fill = "yellow") +
  
  #aesthetics
  scale_color_manual(name = "ERM",
                       breaks = c("green", "red", "blue", "yellow"),
                       labels = c("particles","volume", "mass", "surface area"),
                       guide = "legend")+
      scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Microplastics Species Sensitivity Distribution",
             subtitle = "White = Unaligned, Green = Particles, Red = Volume, Yellow = surface area, blue = mass") +
      coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 1 & 2, 1-5000 µm", x = "particles/L") +
      fill.type + #user-selected
      color.type + #user-selected
      theme.type #user theme

```
### Smalls
#### Tier 1_2
```{r include=FALSE}
# Build dataframe with all SSDs so we can get a color scale
p_t1_s_p <- pred_tier1_2_small_1q_particles %>% mutate(alignment = "particle")
p_t1_s_u <- pred_tier1_2_small_1q_unaligned %>% mutate(alignment = "unaligned")
p_t1_s_v <- pred_tier1_2_small_1q_volume %>% mutate(alignment = "volume")# %>% select(-est_format)
p_t1_s_sa <- pred_tier1_2_small_1q_surfacearea %>% mutate(alignment = "surface area")# %>% select(-est_format)
p_t1_s_m <- pred_tier1_2_small_1q_mass %>% mutate(alignment = "mass")# %>% select(-est_format)
p_t1_s_ssa <- pred_tier1_2_small_1q_specificsurfacearea %>% mutate(alignment = "specific surface area")# %>% select(-est_format)

predictions_t1_small <- rbind.data.frame(p_t1_s_p, 
                                p_t1_s_u,
                                p_t1_s_v,
                                p_t1_s_sa,
                                p_t1_s_m,
                                p_t1_s_ssa) %>% 
  mutate(alignment = factor(alignment))
predictions_t1_small

#do the same for individual points
s_t1_s_p <- SSD_tier1_2_small_1q_particles %>% mutate(alignment = "particle")
s_t1_s_u <- SSD_tier1_2_small_1q_unaligned %>% mutate(alignment = "unaligned")
s_t1_s_v <- SSD_tier1_2_small_1q_volume %>% mutate(alignment = "volume")
s_t1_s_sa <- SSD_tier1_2_small_1q_surfacearea %>% mutate(alignment = "surface area")
s_t1_s_m <- SSD_tier1_2_small_1q_mass %>% mutate(alignment = "mass")
s_t1_s_ssa <- SSD_tier1_2_small_1q_specificsurfacearea %>% mutate(alignment = "specific surface area")

points_t1_small <- rbind.data.frame(s_t1_s_p, 
                                s_t1_s_u,
                                s_t1_s_v,
                                s_t1_s_sa,
                                s_t1_s_m,
                                s_t1_s_ssa)# %>% 
  #mutate(alignment = as.factor(alignment))
#convert alignment to factor
points_t1_small$alignment <- as.factor(points_t1_small$alignment)
```
##### All
```{r}
compare_ERMs_t1_2_small <- predictions_t1_small %>% 
ggplot(aes(x = est, y = percent/100, color = alignment)) +
  geom_line() +
 # geom_xribbon(aes(xmin = lcl, xmax = ucl, y = percent/100, fill = alignment),color = "black", alpha = 0.15) +
  geom_point(data = points_t1_small,aes(x = Conc, y =frac, color = alignment)) + 
  scale_color_npg(name = "Alignment") +
  scale_color_npg(name = "Alignment") +
  guides(fill = FALSE) + # suppress different legend for colors and lines
  coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 10),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Tiers 1 & 2, 1-5000 µm",
    x = "particles/L (aligned to 1-5,000 µm)") +
      coord_trans(x = "log10") +
      theme.type #user theme

compare_ERMs_t1_2_small
```

#### Tier 3_4
```{r eval=FALSE, include=FALSE}
# Build dataframe with all SSDs so we can get a color scale
p_t3_s_p <- pred_tier3_4_small_median_particles %>% mutate(alignment = "particle")
p_t3_s_u <- pred_tier3_4_small_median_unaligned %>% mutate(alignment = "unaligned")
p_t3_s_v <- pred_tier3_4_small_median_volume %>% mutate(alignment = "volume")# %>% select(-est_format)
p_t3_s_sa <- pred_tier3_4_small_median_surfacearea %>% mutate(alignment = "surface area")# %>% select(-est_format)
p_t3_s_m <- pred_tier3_4_small_median_mass %>% mutate(alignment = "mass")# %>% select(-est_format)
p_t3_s_ssa <- pred_tier3_4_small_median_specificsurfacearea %>% mutate(alignment = "specific surface area")# %>% select(-est_format)

predictions_t3_small <- rbind.data.frame(p_t3_s_p, 
                                p_t3_s_u,
                                p_t3_s_v,
                                p_t3_s_sa,
                                p_t3_s_m,
                                p_t3_s_ssa) %>% 
  mutate(alignment = factor(alignment))
predictions_t3_small

#do the same for individual points
s_t3_s_p <- SSD_tier3_4_small_median_particles %>% mutate(alignment = "particle")
s_t3_s_u <- SSD_tier3_4_small_median_unaligned %>% mutate(alignment = "unaligned")
s_t3_s_v <- SSD_tier3_4_small_median_volume %>% mutate(alignment = "volume")
s_t3_s_sa <- SSD_tier3_4_small_median_surfacearea %>% mutate(alignment = "surface area")
s_t3_s_m <- SSD_tier3_4_small_median_mass %>% mutate(alignment = "mass")
s_t3_s_ssa <- SSD_tier3_4_small_median_specificsurfacearea %>% mutate(alignment = "specific surface area")

points_t3_small <- rbind.data.frame(s_t3_s_p, 
                                s_t3_s_u,
                                s_t3_s_v,
                                s_t3_s_sa,
                                s_t3_s_m,
                                s_t3_s_ssa)# %>% 
  #mutate(alignment = as.factor(alignment))
#convert alignment to factor
points_t3_small$alignment <- as.factor(points_t3_small$alignment)
```
##### All
```{r eval=FALSE, include=FALSE}
compare_ERMs_t3_4_small <- predictions_t3_small %>% 
ggplot(aes(x = est, y = percent/100, color = alignment)) +
  geom_line() +
 # geom_xribbon(aes(xmin = lcl, xmax = ucl, y = percent/100, fill = alignment),color = "black", alpha = 0.15) +
  geom_point(data = points_t3_small,aes(x = Conc, y =frac, color = alignment)) + 
  scale_fill_npg(name = "Alignment") +
  scale_color_npg(name = "Alignment") +
  guides(fill = FALSE) + # suppress different legend for colors and lines
  coord_trans(x = "log10") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 10),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Tiers 3 & 4, 1-5000 µm", 
    x = "particles/L") +
      coord_trans(x = "log10") +
      theme.type #user theme

compare_ERMs_t3_4_small
```

##### Manuscript Figure

```{r}
ERM_comparisons <- ggarrange(compare_ERMs_t1_2_small, compare_ERMs_t1_2_large,
          labels = c("A", "B", "C", "D"), common.legend = TRUE,
                        legend = "bottom",
                        ncol = 2, nrow = 1)

ERM_comparisons

ggsave(plot = ERM_comparisons,
       filename = "compare_ERMs_SI-Fig4.jpeg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       width = 12, height = 8, units = "in",
       dpi = 600)
```

## Grand Summary
```{r}
ERM_small <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Unaligned" = c(tier1_small_u, tier2_small_u, tier3_small_u, tier4_small_u),
  "Particle" = c(tier1_small_p, tier2_small_p, tier3_small_p, tier4_small_p), 
  "Volume" = c(tier1_small_v, tier2_small_v, tier3_small_v, tier4_small_v), 
  "Surface Area" = c(tier1_small_sa, tier2_small_sa, tier3_small_sa, tier4_small_sa),
  "Mass" = c(tier1_small_m, tier2_small_m, tier3_small_m, tier4_small_m),
  "Specific Surface Area" = c(tier1_small_ssa, tier2_small_ssa, tier3_small_ssa, tier4_small_ssa)
                    )


# build gt table
full_val_range <- ERM_small %>% 
    ungroup %>% 
    select_if(is.numeric) %>% 
    range

gt(ERM_small) %>% 
  tab_header(title = paste0("1-", upper.tissue.trans.size.um, "µm Thresholds (Aligned to 1 - 5000 µm)"), subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Unaligned, Particle, Volume,'Surface Area', Mass, 'Specific Surface Area'), 
             n_sigfig = 2,
             use_seps = TRUE) %>% 
    data_color(
    columns = names(ERM_small)[2:7],
    colors = scales::col_numeric(
      palette = paletteer::paletteer_d(palette = "ggsci::teal_material") %>% as.character(),
      domain = full_val_range),
    alpha = 0.75) %>% 
    tab_options(column_labels.hidden = FALSE) #%>% 
    #as_raw_html() # return as html
```

```{r}
ERM_large <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Unaligned" = c(tier1_large_u, tier2_large_u, tier3_large_u, tier4_large_u),
  "Particle" = c(tier1_large_p, tier2_large_p, tier3_large_p, tier4_large_p), 
  "Volume" = c(tier1_large_v, tier2_large_v, tier3_large_v, tier4_large_v), 
  "Surface Area" = c(tier1_large_sa, tier2_large_sa, tier3_large_sa, tier4_large_sa),
  "Mass" = c(tier1_large_m, tier2_large_m, tier3_large_m, tier4_large_m),
  "Specific Surface Area" = c(tier1_large_ssa, tier2_large_ssa, tier3_large_ssa, tier4_large_ssa)
                    )

full_val_range <- ERM_large %>% 
    ungroup %>% 
    select_if(is.numeric) %>% 
    range

gt(ERM_large) %>% 
  tab_header(title = "1-5,000 µm Thresholds (Aligned to 1-5,000 µm)", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Unaligned, Particle, Volume,'Surface Area', Mass, 'Specific Surface Area'), 
             n_sigfig = 3,
             use_seps = TRUE) %>% 
    data_color(
    columns = names(ERM_large)[2:7],
    colors = scales::col_numeric(
      palette = paletteer::paletteer_d(palette = "ggsci::teal_material") %>% as.character(),
      domain = full_val_range),
    alpha = 0.75) %>% 
    tab_options(column_labels.hidden = FALSE) #%>% 
    #as_raw_html() # return as html
```
## Compartment-specific SSDs
Alignments are compartment-specific.
```{r eval=FALSE, include=FALSE}
#### define function for SSD generation for tier 1 ####
SSD_function_t1 <- function(filtered.data, hcxlcl){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#
ssd_frame <- collapsed[order(collapsed$Conc), ]
ssd_frame$frac <- ppoints(collapsed$Conc, 0.5)
#report HC metrics of interest
hc5lcl <- c(preds$lcl[hcxlcl]) #CI95
#values to extract
#print(hc5lcl)
return(list(preds, ssd_frame))
}
```

### Oxidative Stress
#### Marine vs. Marine + Freshwater
Note that there's only 5 freshwater species that meet criteria for oxidative stress thresholds, so it's impossible to compare them.
```{r eval=FALSE, include=FALSE}
### Marine
#data filtering
Marine_t1_ox <- aoc_final_small %>% 
         mutate(dose_new = 1000 * EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) %>% 
  filter(environment == "Marine")

## SSD build
marine_ox.t1.ssd <- SSD_function_t1(filtered.data = Marine_t1_ox, hcxlcl = 5)

#extract data frames from object
marine_ox.t1.preds <- as.data.frame(marine_ox.t1.ssd[1])
marine_ox.t1.points <- as.data.frame(marine_ox.t1.ssd[2])

### Freshwater
#data filtering
Freshwater_t1_ox <- aoc_final_small %>% 
         mutate(dose_new = 1000 * EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) %>% 
  filter(environment == "Freshwater")

## SSD build
freshwater_ox.t1.ssd <- SSD_function_t1(filtered.data = Freshwater_t1_ox, hcxlcl = 5)

#extract data frames from object
freshwater_ox.t1.preds <- as.data.frame(Freshwater_ox.t1.ssd[1])
freshwater_ox.t1.points <- as.data.frame(Freshwater_ox.t1.ssd[2])



# give uniq id
marine_preds <- marine_ox.t1.preds %>% 
  mutate(type = "Marine")
marine_points <-  marine_ox.t1.points %>% 
  mutate(type = "Marine")
freshwater_points <- freshwater_ox.t1.points %>% 
  mutate(type = "Freshwater") 
freshwater_preds <-  freshwater_ox.t1.preds %>% 
  mutate(type = "Freshwater")
#join dataframes
t1.ox.preds <- rbind(marine_preds, freshwater_preds)
t1.ox.points <- rbind(marine_points, freshwater_points)


#ggplot
ggplot() +#, aes_string(x = "est")) +
  geom_xribbon(data = t1.ox.preds, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", fill = "type"), alpha = 0.4) +
  geom_line(data = t1.ox.preds, aes_string(x = "est", y = "percent/100", fill = "type")) +
  geom_point(data = t1.ox.points, aes(x = Conc, y =frac, color = Group), alpha = 0.7) + 
  #geom_text_repel(data = ox.t1.emp.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Predicted vs. Empirical Microplastics Species Sensitivity Distribution (polymer/shape - agnostic)",
       subtitle = "Tier 1/2, 1 - 83 µm; ERM = Surface Area",
       x = "particles/L (aligned to 1-5,000 um; translocation limited)") +
  coord_trans(x = "log10") +
  scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                     labels = trans_format("log10", scales::math_format(10^.x))) + 
  #scale_fill_manual(name = "SSD Data") +
  #fill.type + #user-selected
  #color.type + #user-selected
  theme.type #user theme
```
### Food Dilution
#### Marine vs. Freshwater vs. Marine + Freshwater
```{r eval=FALSE, include=FALSE}
### Marine
#data filtering
Marine_t1_food <- aoc_final_large %>% 
         mutate(dose_new =EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(environment == "Marine")

## SSD build
marine_food.t1.ssd <- SSD_function_t1(filtered.data = Marine_t1_food, hcxlcl = 5)

#extract data frames from object
marine_food.t1.preds <- as.data.frame(marine_food.t1.ssd[1])
marine_food.t1.points <- as.data.frame(marine_food.t1.ssd[2])

### Freshwater
#data filtering
Freshwater_t1_food <- aoc_final_large %>% 
         mutate(dose_new =  EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(environment == "Freshwater")

## SSD build
freshwater_food.t1.ssd <- SSD_function_t1(filtered.data = Freshwater_t1_food, hcxlcl = 5)

#extract data frames from object
freshwater_food.t1.preds <- as.data.frame(freshwater_food.t1.ssd[1])
freshwater_food.t1.points <- as.data.frame(freshwater_food.t1.ssd[2])

### Marine + freshwater
#data filtering
both_t1_food <- aoc_final_large %>% 
         mutate(dose_new =  EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))

## SSD build
both_food.t1.ssd <- SSD_function_t1(filtered.data = both_t1_food, hcxlcl = 5)

#extract data frames from object
both_food.t1.preds <- as.data.frame(both_food.t1.ssd[1])
both_food.t1.points <- as.data.frame(both_food.t1.ssd[2])


# give uniq id
marine_preds <- marine_food.t1.preds %>% 
  mutate(type = "Marine")
marine_points <-  marine_food.t1.points %>% 
  mutate(type = "Marine")
freshwater_points <- freshwater_food.t1.points %>% 
  mutate(type = "Freshwater") 
freshwater_preds <-  freshwater_food.t1.preds %>% 
  mutate(type = "Freshwater")
both_preds <- both_food.t1.preds %>%
  mutate(type = "Marine + Freshwater")
both_points <-  both_food.t1.points %>%
 mutate(type = "Marine + Freshwater")

#### join dataframes
t1.food.preds <- rbind(marine_preds, freshwater_preds, both_preds)
t1.food.points <- rbind(marine_points, freshwater_points, both_points)


#ggplot
ggplot() +#, aes_string(x = "est")) +
  geom_xribbon(data = t1.food.preds, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", fill = "type"), alpha = 0.15) +
  geom_line(data = t1.food.preds, aes_string(x = "est", y = "percent/100", fill = "type", color = "type")) +
  geom_point(data = t1.food.points, aes(x = Conc, y =frac, color = type), alpha = 0.3) + 
  #geom_text_repel(data = food.t1.emp.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Freshwater vs. Marine SSDs",
       subtitle = "Tier 2, 1um - Ingestible limit; ERM = Food Dilution",
       x = "particles/L (aligned to 1-5,000 um; ingestion limited)") +
  coord_trans(x = "log10") +
  scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                     labels = trans_format("log10", scales::math_format(10^.x))) + 
  scale_fill_aaas(name = "Environment") +
  scale_color_aaas(name = "Environment") +
  #fill.type + #user-selected
  #color.type + #user-selected
  theme.type #user theme
```
#### Kolmogorov-Smirnov Test for Differences
K-S tests make no assumption about the underlying distributions, and should be used for empirical distributions only. 

```{r eval=FALSE, include=FALSE}
ks.test(x = freshwater_food.t1.points[,3],
        y = marine_food.t1.points[,3],
        exact = TRUE)

# ks.test(x = freshwater_food.t1.points[,3],
#         y = both_food.t1.points[,3],
#         exact = TRUE)
# 
# ks.test(x = marine_food.t1.points[,3],
#         y = both_food.t1.points[,3],
#         exact = TRUE)
```

### Publication Ready
```{r eval=FALSE, include=FALSE}
preds <- t1.food.preds %>% 
  filter(type != "Marine + Freshwater")
points <- t1.food.points %>% 
  filter(type != "Marine + Freshwater")

#ggplot
food_environment_ssds <- ggplot() +#, aes_string(x = "est")) +
  geom_xribbon(data = preds, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", fill = "type"), alpha = 0.15) +
  geom_line(data = preds, aes_string(x = "est", y = "percent/100", color = "type")) +
  geom_point(data = points, aes(x = Conc, y =frac, color = Group), alpha = 0.5) + 
  geom_text_repel(data = points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Freshwater vs. Marine SSDs",
       #subtitle = "Tier 2, 1um - Ingestible limit; ERM = Food Dilution",
       x = "particles/L (aligned to 1-5,000 um; ingestion limited)") +
  coord_trans(x = "log10") +
  scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 7),
                     labels = trans_format("log10", scales::math_format(10^.x))) + 
  scale_fill_manual(name = "Environment",
                  values = c("#EE0000FF", "#3B4992FF")) +
  scale_color_aaas(name = "Species") +
  #fill.type + #user-selected
  #color.type + #user-selected
  theme.type #user theme

food_environment_ssds

ggsave(plot = food_environment_ssds,
       filename = "food_environment_ssds_SI-Fig4.jpeg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       width = 8, height = 6, units = "in",
       dpi = 300)
```

## SIMPLE Summary Table
Thresholds are presented in multiple exposure metrics, aligned to ERMs of interest (surface area for translocatable particles, volume for ingestible particles). Data are aligned using the methods described in Kooi et al 2021. 

The environmentally-relevant, ERM-aligned threshold in particles/L is converted to surface area (um2/L) and mass (ug/L) using equations 5 and 4 from Kooi et al 2021.

$EC_{env,ERM, x} = \mu_{x,poly} * EC_{env}$

Where  $\mu_{x,poly}$ is derived using the following equation:
$\mu_{x,poly} = \frac{1 - a_{x}}{2 - a_{x}}  \frac{X^{2-a_x}_{UL} - X^{2-a_x}_{LL}}{X^{1-a_x}_{UL} - X^{1-a_x}_{LL}}$

In this case, the limits LL and UL in  $\mu_{x,poly}$ relate to the values of the ERM at the 1 and 5,000 um size limits, respectively, rather than to bioavailability limits. Compartment-specific alpha values are used based on Table S4 of Kooi et al 2021. Marine surface water values are used. For converting to surface area, an alpha value of 1.50 is used, 1.48 for volume, and 1.32 for mass.

### Marine 
```{r}
# Steve wants thresholds reported in the units they're aligned to (um3/L and um2/L)
oxidative.stress.tiers <- ERM_small %>% 
  mutate(EC_env_sa_um2.L =  `Surface Area` * mux.polyfnx(a.x = 1.50, #alpha for marine surface water for surface area
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.SA =  `Surface Area` * mux.polyfnx(a.x = 1.32, #alpha for marine surface water for mass
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  dplyr::select(EC_env_sa_um2.L, EC_env_mass_ug.L.SA, `Surface Area`)

# calculate food dilution tiers with surface area and in particles/L
food.dilution.tiers <- ERM_large %>% 
  mutate(EC_env_volume_um3.L =  Volume * mux.polyfnx(a.x = 1.48, #alpha for marine surface water for surface area
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.V =  Volume * mux.polyfnx(a.x = 1.32, #alpha for marine surface water for mass
                                       x_UL = x2D_set, #upper limit of default range
                                       x_LL = x1D_set # lower limit of default range
                                       )) %>% 
  dplyr::select(EC_env_volume_um3.L, EC_env_mass_ug.L.V, Volume)

simple.summary <- bind_cols(oxidative.stress.tiers, food.dilution.tiers)

simple.summary <- simple.summary %>% 
  mutate("Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4')) %>% 
  rename("um2/L" = EC_env_sa_um2.L,
         "surface area (particles/L)" = 'Surface Area',
         "ug/L (SA)" = EC_env_mass_ug.L.SA,
         "um3/L" = EC_env_volume_um3.L,
        "volume (particles/L)" =  Volume,
        "ug/L (V)" = EC_env_mass_ug.L.V,) %>% 
  relocate(Tier)

full_val_range <- simple.summary %>% 
    ungroup %>% 
    select_if(is.numeric) %>% 
    range

marine_thresholds_table <- gt(simple.summary) %>% 
  tab_header(title = "Marine Microplastics Thresholds (Aligned to 1-5,000 µm)") %>% 
  fmt_number(columns = c(names(simple.summary)[2:7]), 
             decimals = 1,
             use_seps = TRUE) %>% 
    tab_options(column_labels.hidden = FALSE) %>% 
  tab_spanner(label = "Surface Area ERM",
              columns = c(names(simple.summary)[2:4])) %>% 
  tab_spanner(label = "Volume ERM",
              columns = c(names(simple.summary)[5:7]))

#save as html
gtsave(marine_thresholds_table, 
       filename = "./Concentration data/Threshold_Manuscript_Figs/marine_thresholds_table.html")

#save as csv
write.csv(simple.summary,
          "./Concentration data/Threshold_Manuscript_Figs/marine_thresholds_table.csv")

marine_thresholds_table
```


Note that insufficient data are available to build translocation-based thresholds for freshwater species.
```{r eval=FALSE, include=FALSE}
SSD_function_t2 <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.25))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hcx <- c(preds$est[hcx]) #HC5
#values to extract
return(list(preds, ssd_frame, hcx))
}

SSD_function_t3_4 <- function(filtered.data, hcx){
  set.seed(99)
  #data collapse
collapsed <- filtered.data %>% 
  group_by(Species, Group) %>% 
  summarize(Conc = quantile(dose_new, 0.5))
#fit distributions
dists <- ssd_fit_dists(collapsed, left = "Conc", dists = c("weibull", "llogis", "lnorm", "gamma", "lgumbel"), computable = FALSE, silent = FALSE) 
#use average distribution with weighthing based on AICC
preds <- predict(dists, average = TRUE, ic = "aicc", nboot = 10, ci= TRUE) 
#report HC metrics of interest
hcx <- c(preds$est[hcx]) #HC5
#values to extract
return(list(preds, ssd_frame, hcx))
}
```
### Freshwater
#### Calculations
```{r eval=FALSE, include=FALSE}
##### TIERs 1 & 2 ####
#data filtering
Freshwater_t1_food <- aoc_final_large %>% 
         mutate(dose_new =  EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(environment == "Freshwater")

## SSD build
freshwater_food.t1.ssd <- SSD_function_t1(filtered.data = Freshwater_t1_food, hcxlcl = 5)

#extract data frames from object
freshwater_food.t1.preds <- as.data.frame(freshwater_food.t1.ssd[1])
freshwater_food.t1.points <- as.data.frame(freshwater_food.t1.ssd[2])
# extract HC5LCL
freshwater_food_t1 <- freshwater_food.t1.preds$lcl[5]

##### TIER 2 ####
#note that the dataset for tiers 1 and 2 are identical, and just the HC metric changes

freshwater_food_t2 <- freshwater_food.t1.preds$est[5]

##### TIER 3 ####
#data filtering
Freshwater_t3_food <- aoc_final_large %>% 
         mutate(dose_new =  EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
  filter(environment == "Freshwater")

#### SSD build
#freshwater_food.t3.ssd <- SSD_function_t3_4(filtered.data = Freshwater_t3_food, hcx = 5)
## Won't run due to 5 species

##extract data frames from object
#freshwater_food.t3.preds <- as.data.frame(freshwater_food.t3.ssd[1])
#freshwater_food.t3.points <- as.data.frame(freshwater_food.t3.ssd[2])

##### TIER 4 ####
#note that tiers 3 and 4 use the same data but different HC metric
## SSD build
#freshwater_food.t4.ssd <- SSD_function_t3_4(filtered.data = Freshwater_t3_food, hcx = 10)

##extract data frames from object
#freshwater_food.t4.preds <- as.data.frame(freshwater_food.t4.ssd[1])
#freshwater_food.t4.points <- as.data.frame(freshwater_food.t4.ssd[2])
```

#### Table
```{r eval=FALSE, include=FALSE}
ERM_small_freshwater <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Unaligned" = c(NA, NA, NA, NA),
  "Particle" = c(NA, NA, NA, NA), 
  "Volume" = c(NA, NA, NA, NA), 
  "Surface Area" = c(NA, NA, NA, NA),
  "Mass" = c(NA, NA, NA, NA),
  "Specific Surface Area" = c(NA, NA, NA, NA)
                    )

ERM_large_freshwater <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Unaligned" = c(NA, NA, NA, NA),
  "Particle" = c(NA, NA, NA, NA), 
  "Volume" = c(freshwater_food_t1, freshwater_food_t2, NA, NA), 
  "Surface Area" = c(NA, NA, NA, NA),
  "Mass" = c(NA, NA, NA, NA),
  "Specific Surface Area" = c(NA, NA, NA, NA)
                    )


# Steve wants thresholds reported in the units they're aligned to (um3/L and um2/L)
oxidative.stress.tiers_freshwater <- ERM_small_freshwater %>% 
  mutate(EC_env_sa_um2.L =  `Surface Area` * mux.polyfnx(a.x = 1.50, #alpha for marine surface water for surface area
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.SA =  `Surface Area` * mux.polyfnx(a.x = 1.32, #alpha for marine surface water for mass
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>% 
  dplyr::select(EC_env_sa_um2.L, EC_env_mass_ug.L.SA, `Surface Area`)

# calculate food dilution tiers with surface area and in particles/L
food.dilution.tiers_freshwater <- ERM_large_freshwater %>% 
 # drop_na(Volume) %>% 
  mutate(EC_env_volume_um3.L =  Volume * mux.polyfnx(a.x = a.v.freshwater, #alpha for surface area
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>% 
  mutate(EC_env_mass_ug.L.V =  Volume * mux.polyfnx(a.x = a.m.freshwater, #alpha for  mass
                                       x_UL = 5000, #upper limit of default range
                                       x_LL = 1 # lower limit of default range
                                       )) %>%
  dplyr::select(EC_env_volume_um3.L, EC_env_mass_ug.L.V, Volume)

simple.summary_freshwater <- bind_cols(oxidative.stress.tiers_freshwater, food.dilution.tiers_freshwater)

simple.summary_freshwater <- simple.summary_freshwater%>% 
  mutate("Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4')) %>% 
  rename("um2/L" = EC_env_sa_um2.L,
         "surface area (particles/L)" = 'Surface Area',
         "ug/L (SA)" = EC_env_mass_ug.L.SA,
         "um3/L" = EC_env_volume_um3.L,
        "volume (particles/L)" =  Volume,
        "ug/L (V)" = EC_env_mass_ug.L.V,) %>% 
  relocate(Tier)

full_val_range <- simple.summary_freshwater %>% 
    ungroup %>% 
    select_if(is.numeric) %>% 
    range

gt(simple.summary_freshwater) %>% 
  tab_header(title = "Freshwater Microplastics Thresholds (Aligned to 1-5,000 µm)") %>% 
  # fmt_number(columns = c(names(simple.summary)[2:7]), 
  #            decimals = 1,
  #            use_seps = TRUE) %>% 
    tab_options(column_labels.hidden = FALSE) %>% 
  tab_spanner(label = "Surface Area ERM",
              columns = c(names(simple.summary_freshwater)[2:4])) %>% 
  tab_spanner(label = "Volume ERM",
              columns = c(names(simple.summary_freshwater)[5:7]))
```



# Sensitivity Analysis

## Effects
The thresholds above are calculated based on all effect types, including everything from changes in behavior to mortality. There is a gradient of effect severities which correlates with the concentrations at which they occur. For instance, mortality occurs typically at higher concentrations than developmental changes. The sensitivity of those data selections are examined below by restricting the effects data to be only fitness-level endpoints vs. all data.

### Calculate Thresholds
```{r include=FALSE}
####---- Food Dilution ------#####
#### ALL Endpoints ###
filtered.data.large.all_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.all_t3.4 <- filtered.data.large.all_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.all.t1 <- SSD_function_t1(filtered.data = filtered.data.large.all_t1.2, hcxlcl = 5)
large.all.t2 <- SSD_function_t2(filtered.data = filtered.data.large.all_t1.2, hcx = 5)
large.all.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.all_t3.4, hcx = 5)
large.all.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.all_t3.4, hcx = 10)

#### fitness endpoints only ###
filtered.data.large.fitness_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set),
         lvl1 == "fitness")

filtered.data.large.fitness_t3.4 <- filtered.data.large.fitness_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.fitness.t1 <- SSD_function_t1(filtered.data = filtered.data.large.fitness_t1.2, hcxlcl = 5)
large.fitness.t2 <- SSD_function_t2(filtered.data = filtered.data.large.fitness_t1.2, hcx = 5)
large.fitness.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.fitness_t3.4, hcx = 5)
large.fitness.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.fitness_t3.4, hcx = 10)

#### Mortality  only ###
filtered.data.large.mortality_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set),
         lvl3 == "mortality")

filtered.data.large.mortality_t3.4 <- filtered.data.large.mortality_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.mortality.t1 <- SSD_function_t1(filtered.data = filtered.data.large.mortality_t1.2, hcxlcl = 5)
large.mortality.t2 <- SSD_function_t2(filtered.data = filtered.data.large.mortality_t1.2, hcx = 5)
large.mortality.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.mortality_t3.4, hcx = 5)
large.mortality.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.mortality_t3.4, hcx = 10)


####---- Tissue Translocation ------#####
#### All endpoints ###
filtered.data.small.all_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.all_t3.4 <- filtered.data.small.all_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.all.t1 <- SSD_function_t1(filtered.data = filtered.data.small.all_t1.2, hcxlcl = 5)
small.all.t2 <- SSD_function_t2(filtered.data = filtered.data.small.all_t1.2, hcx = 5)
small.all.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.all_t3.4, hcx = 5)
small.all.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.all_t3.4, hcx = 10)

#### fitness endpoints only ###
filtered.data.small.fitness_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um),
         lvl1 == "fitness")

filtered.data.small.fitness_t3.4 <- filtered.data.small.fitness_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.fitness.t1 <- SSD_function_t1(filtered.data = filtered.data.small.fitness_t1.2, hcxlcl = 5)
small.fitness.t2 <- SSD_function_t2(filtered.data = filtered.data.small.fitness_t1.2, hcx = 5)
small.fitness.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.fitness_t3.4, hcx = 5)
small.fitness.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.fitness_t3.4, hcx = 10)

#### mortality  only ###
filtered.data.small.mortality_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um),
         lvl3 == "mortality")

filtered.data.small.mortality_t3.4 <- filtered.data.small.mortality_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.mortality.t1 <- SSD_function_t1(filtered.data = filtered.data.small.mortality_t1.2, hcxlcl = 5)
small.mortality.t2 <- SSD_function_t2(filtered.data = filtered.data.small.mortality_t1.2, hcx = 5)
small.mortality.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.mortality_t3.4, hcx = 5)
small.mortality.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.mortality_t3.4, hcx = 10)
```
### Summary Table

```{r}
# arrange
endpoints_sensitivity_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Food Dilution (All Endpoints)" = c(large.all.t1, large.all.t2, large.all.t3, large.all.t4),
  "Food Dilution (Fitness Endpoints)" = c(large.fitness.t1, large.fitness.t2, large.fitness.t3, large.fitness.t4),
  "Food Dilution (Mortality)" = c(large.mortality.t1, large.mortality.t2, large.mortality.t3, large.mortality.t4),
  "Tissue Translocation (All Endpoints)" = c(small.all.t1, small.all.t2, small.all.t3, small.all.t4),
  "Tissue Translocation (Fitness Endpoints)" = c(small.fitness.t1, small.fitness.t2, small.fitness.t3, small.fitness.t4),
  "Tissue Translocation (Mortality)" = c(small.mortality.t1, small.mortality.t2, small.mortality.t3, small.mortality.t4))

#save
write.csv(endpoints_sensitivity_thresholds,
          "./Concentration data/Threshold_Manuscript_Figs/endpoints_sensitivity.csv")

#print
endpoints_sensitivity_thresholds
```

## Biological Level of Organization

The first two tiers are based on molecular to population levels, and the upper tiers are based on organism to population levels. Letr's test the sensitivity based on the least restrictive (all levels) to most restrictive (population).

### Calculate Thresholds
```{r include=FALSE}
####---- Food Dilution ------#####
#### ALL Levels ###
filtered.data.large.all_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.all_t3.4 <- filtered.data.large.all_t1.2 %>% 
 filter(risk.13 != 1)

# get thresholds
large.all.t1 <- SSD_function_t1(filtered.data = filtered.data.large.all_t1.2, hcxlcl = 5)
large.all.t2 <- SSD_function_t2(filtered.data = filtered.data.large.all_t1.2, hcx = 5)
large.all.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.all_t3.4, hcx = 5)
large.all.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.all_t3.4, hcx = 10)

#### Organism and population levels only for all tiers ###
filtered.data.large.organism_population_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set),
         lvl1 == "fitness",
          bio_f %in% c("Organism", "Population"))

filtered.data.large.organism_population_t3.4 <- filtered.data.large.organism_population_t1.2 %>% 
  filter(risk.13 != 1)

# get thresholds
large.organism_population.t1 <- SSD_function_t1(filtered.data = filtered.data.large.organism_population_t1.2, hcxlcl = 5)
large.organism_population.t2 <- SSD_function_t2(filtered.data = filtered.data.large.organism_population_t1.2, hcx = 5)
large.organism_population.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.organism_population_t3.4, hcx = 5)
large.organism_population.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.organism_population_t3.4, hcx = 10)

####---- Tissue Translocation ------#####
#### All levels ###
filtered.data.small.all_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.all_t3.4 <- filtered.data.small.all_t1.2 %>% 
 filter(risk.13 != 1)

# get thresholds
small.all.t1 <- SSD_function_t1(filtered.data = filtered.data.small.all_t1.2, hcxlcl = 5)
small.all.t2 <- SSD_function_t2(filtered.data = filtered.data.small.all_t1.2, hcx = 5)
small.all.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.all_t3.4, hcx = 5)
small.all.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.all_t3.4, hcx = 10)

#### organism and population levels only ###
filtered.data.small.organism_population_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um),
         lvl1 == "fitness",
         bio_f %in% c("Organism", "Population"))

filtered.data.small.organism_population_t3.4 <- filtered.data.small.organism_population_t1.2 %>% 
 filter(risk.13 != 1)

# get thresholds
small.organism_population.t1 <- SSD_function_t1(filtered.data = filtered.data.small.organism_population_t1.2, hcxlcl = 5)
small.organism_population.t2 <- SSD_function_t2(filtered.data = filtered.data.small.organism_population_t1.2, hcx = 5)
small.organism_population.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.organism_population_t3.4, hcx = 5)
small.organism_population.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.organism_population_t3.4, hcx = 10)
```

### Summary Table

```{r}
# arrange
biological_level_sensitivity_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Food Dilution (All biological org)" = c(large.all.t1, large.all.t2, large.all.t3, large.all.t4),
  "Food Dilution (Organism + biological org)" = c(large.organism_population.t1, large.organism_population.t2, large.organism_population.t3, large.organism_population.t4),
  "Tissue Translocation (All biological org)" = c(small.all.t1, small.all.t2, small.all.t3, small.all.t4),
  "Tissue Translocation (Organism + biological org)" = c(small.organism_population.t1, small.organism_population.t2, small.organism_population.t3, small.organism_population.t4))

#save
write.csv(biological_level_sensitivity_thresholds,
          "./Concentration data/Threshold_Manuscript_Figs/biological_level_sensitivity.csv")

#print
biological_level_sensitivity_thresholds
```


## Bioaccessibility Alignment Sensitivity Analysis
In determining the ingestible (bioaccessible) particles, several decisions must be made. The bioaccessible limit of the particle may be determined by the width or length of the particle restricted by the mouth size opening of the organism. The most conservative assumption is that fibers are ingestible, with lengths equivalent to the upper default size (e.g. 5,000 um) and the length equivalent to the mouth size opening. A less conservative assumption is that the fibers of such length are unlikely to be ingested, so both the width and length could be defined as the bioaccessibility limit, approximated by a sphere (spheres do occur in the environment). The least conservative assumption would be to estimate the bioaccessible width based on the average width to length ratio in the environment (e.g. 0.67 for marine surface water) and use the length as the bioaccessible limit (equivalent to mouth size opening). Each of these scenarios are evaluated in the following sensitivity analysis.

Note that in all cases, the lower size limit is always calculated using the equation for a sphere with length, width, and height defined as the lower size limit (e.g. 1 micron). This is due to the inability to extrapolate below 1 micron based on the available alignment data.
### Alignments
#### Food Dilution
##### Fibers as Ingestible Fractions
```{r}
# calculate ERM for each species
aoc_final_large_fibers <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2D_set, #fiber length (5,000 um)
                         b = 0.5 * x2M, # width-limited
                         c = 0.5 * x2M)) %>%   #height same as width
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2D_set, # #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #width-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible width
                               length = x2D_set, #upper definition for fibers (# CONSERVATIVE)
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```



##### Spheres as Ingestible Fraction
Here, width height and length are all set to the mouth size opening for calculating ingesible sizes.
```{r}
# calculate ERM for each species
aoc_final_large_spheres <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2M, # shpere
                         b = 0.5 * x2M, # sphere
                         c = 0.5 * x2M)) %>%   #height same as width
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, #sphere
                                 width = x2M)) %>% #width-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible width
                               length = x2M, #sphere
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

##### Fragments as Ingestible Fraction
Here, length is set to mouth size opening, while width is set to the average ratio of length:width and height is set to the average ratio of heigh:width.
```{r}
# calculate ERM for each species
aoc_final_large_fragments <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2M, # ingestible length
                         b = 0.5 * R.ave * x2M, #width of fragment
                         c = 0.5 * R.ave * 0.67 * x2M #height of fragment
                         )) %>%   
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, #length-limited
                                 width = R.ave * x2M)) %>% #width of fragment
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = R.ave * x2M, #ingestible width of fragment
                               length = x2M, #ingestible length
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

#### Tissue Translocation
##### Fibers as Ingestible Fractions
```{r}
# calculate ERM for each species
aoc_final_small_fibers <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>% 
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2D_set, #fiber length (5,000 um)
                         b = 0.5 * x2M, # width-limited
                         c = 0.5 * x2M)) %>%   #height same as width
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2D_set, # #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #width-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible width
                               length = x2D_set, #upper definition for fibers (# CONSERVATIVE)
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```



##### Spheres as Ingestible Fraction
Here, width height and length are all set to the mouth size opening for calculating ingesible sizes.
```{r}
# calculate ERM for each species
aoc_final_small_spheres <- aoc_intermediate_alphas  %>% 
 # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>% 
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2M, # shpere
                         b = 0.5 * x2M, # sphere
                         c = 0.5 * x2M)) %>%   #height same as width
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, #sphere
                                 width = x2M)) %>% #width-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible width
                               length = x2M, #sphere
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

##### Fragments as Ingestible Fraction
Here, length is set to mouth size opening, while width is set to the average ratio of length:width and height is set to the average ratio of heigh:width.
```{r}
# calculate ERM for each species
aoc_final_small_fragments <- aoc_intermediate_alphas  %>% 
 # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>% 
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible width
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,
                         c = 0.5 * x1D_set)) %>% 
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
                         a = 0.5 * x2M, # ingestible length
                         b = 0.5 * R.ave * x2M, #width of fragment
                         c = 0.5 * R.ave * 0.67 * x2M #height of fragment
                         )) %>%   
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, #length-limited
                                 width = R.ave * x2M)) %>% #width of fragment
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = R.ave * x2M, #ingestible width of fragment
                               length = x2M, #ingestible length
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

### Calculate Thresholds
```{r include=FALSE}
####---- Food Dilution ------#####
#### SPHERES ###
filtered.data.large.spheres_t1.2 <- aoc_final_large_spheres %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.spheres_t3.4 <- filtered.data.large.spheres_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.spheres.t1 <- SSD_function_t1(filtered.data = filtered.data.large.spheres_t1.2, hcxlcl = 5)
large.spheres.t2 <- SSD_function_t2(filtered.data = filtered.data.large.spheres_t1.2, hcx = 5)
large.spheres.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.spheres_t3.4, hcx = 5)
large.spheres.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.spheres_t3.4, hcx = 10)

#### fibers ###
filtered.data.large.fibers_t1.2 <- aoc_final_large_fibers %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.fibers_t3.4 <- filtered.data.large.fibers_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.fibers.t1 <- SSD_function_t1(filtered.data = filtered.data.large.fibers_t1.2, hcxlcl = 5)
large.fibers.t2 <- SSD_function_t2(filtered.data = filtered.data.large.fibers_t1.2, hcx = 5)
large.fibers.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.fibers_t3.4, hcx = 5)
large.fibers.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.fibers_t3.4, hcx = 10)

#### fragments ###
filtered.data.large.fragments_t1.2 <- aoc_final_large_fragments %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.fragments_t3.4 <- filtered.data.large.fragments_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.fragments.t1 <- SSD_function_t1(filtered.data = filtered.data.large.fragments_t1.2, hcxlcl = 5)
large.fragments.t2 <- SSD_function_t2(filtered.data = filtered.data.large.fragments_t1.2, hcx = 5)
large.fragments.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.fragments_t3.4, hcx = 5)
large.fragments.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.fragments_t3.4, hcx = 10)

####---- Tissue Translocation ------#####
#### SPHERES ###
filtered.data.small.spheres_t1.2 <- aoc_final_small_spheres %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.spheres_t3.4 <- filtered.data.small.spheres_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.spheres.t1 <- SSD_function_t1(filtered.data = filtered.data.small.spheres_t1.2, hcxlcl = 5)
small.spheres.t2 <- SSD_function_t2(filtered.data = filtered.data.small.spheres_t1.2, hcx = 5)
small.spheres.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.spheres_t3.4, hcx = 5)
small.spheres.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.spheres_t3.4, hcx = 10)

#### fibers ###
filtered.data.small.fibers_t1.2 <- aoc_final_small_fibers %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.fibers_t3.4 <- filtered.data.small.fibers_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.fibers.t1 <- SSD_function_t1(filtered.data = filtered.data.small.fibers_t1.2, hcxlcl = 5)
small.fibers.t2 <- SSD_function_t2(filtered.data = filtered.data.small.fibers_t1.2, hcx = 5)
small.fibers.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.fibers_t3.4, hcx = 5)
small.fibers.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.fibers_t3.4, hcx = 10)

#### fragments ###
filtered.data.small.fragments_t1.2 <- aoc_final_small_fragments %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, upper.tissue.trans.size.um))

filtered.data.small.fragments_t3.4 <- filtered.data.small.fragments_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.fragments.t1 <- SSD_function_t1(filtered.data = filtered.data.small.fragments_t1.2, hcxlcl = 5)
small.fragments.t2 <- SSD_function_t2(filtered.data = filtered.data.small.fragments_t1.2, hcx = 5)
small.fragments.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.fragments_t3.4, hcx = 5)
small.fragments.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.fragments_t3.4, hcx = 10)
```
### Summary Table

```{r}
# arrange
bioaccessibility_sensitivity_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Food Dilution (Spheres)" = c(large.spheres.t1, large.spheres.t2, large.spheres.t3, large.spheres.t4),
  "Food Dilution (Fibers)" = c(large.fibers.t1, large.fibers.t2, large.fibers.t3, large.fibers.t4),
  "Food Dilution (Fragments)" = c(large.fragments.t1, large.fragments.t2, large.fragments.t3, large.fragments.t4),
  "Tissue Translocation (Spheres)" = c(small.spheres.t1, small.spheres.t2, small.spheres.t3, small.spheres.t4),
  "Tissue Translocation (Fibers)" = c(small.fibers.t1, small.fibers.t2, small.fibers.t3, small.fibers.t4),
  "Tissue Translocation (Fragments)" = c(small.fragments.t1, small.fragments.t2, small.fragments.t3, small.fragments.t4))

#save
write.csv(bioaccessibility_sensitivity_thresholds,
          "./Concentration data/Threshold_Manuscript_Figs/bioaccessibility_sensitivity.csv")

#print
bioaccessibility_sensitivity_thresholds
```


## Polydisperse Lab Exposure Alpha Distributions
A portion of tests (n = 10 out of a total of 40) used particles having a certain size range (e.g. authors specify 20 – 200 µm, etc.) and were considered polydisperse. We verified the effect of the uncertainty in the exponent values between 2 and 3 for these 10 lab-studies using a sensitivity analysis, which turned out to be marginal (i.e. only YY%). 

Since alpha distributions were not extracted from the studies, we varied the alpha values at arbitrary values above, at, and below the expected distribution in the marine environment.A table describing the sensitivity values tested is below:

### How many studies are polydisperse?
```{r}
#total studies
aoc_z %>% 
  group_by(polydispersity) %>% 
  summarize(n = n_distinct(doi))

#total studies passing minimum criteria
aoc_intermediate %>% 
  group_by(polydispersity) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  summarize(n = n_distinct(doi))

#tissue translocation lower tier
tier1_2_small %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) %>% 
  group_by(polydispersity) %>%
  dplyr::summarize(n = n_distinct(doi))

#tissue translocation upper tier
tier3_4_small %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) %>% 
   filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
  group_by(polydispersity) %>%
  dplyr::summarize(n = n_distinct(doi))

#food dilution lower tier
tier1_2_large %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(Group != "Algae") %>%
  group_by(polydispersity) %>% 
  summarize(n = n_distinct(doi))

#food dilution upper tier
tier3_4_large %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) %>% 
  filter(Group != "Algae") %>%
  filter(risk.13 != 1, #Drop studies that only scored a 1 for endpoints criteria
         bio_f %in% c("Organism", "Population")) %>% #limit to endpoints measured at the organism or population level
  group_by(polydispersity) %>% 
  summarize(n = n_distinct(doi))
```

### Parameter Assignment
 The upper and lower interquartile ranges for length alpha distributions in marine surface waters were extracted from Figure 5 in Kooi et al (2021) using an online tool (WebPlotDigitizer). The upper and lower values are 2.66 and 2.09, respectively, with a median value of 2.2, thus corresponding to a percentage difference of +21% and -6%, respectively. These same relative upper and lower bounds (i.e., +21% and –6%)  were applied to the surface area, volume, and mass distribution values to estimate the sensitivity of such assumptions in deriving μlabx,poly𝜇x,polylab  for thresholds derived in the main manuscript. 
```{r}
# default values
alpha.marine = 2.07 #table s4 for marine surface water. length
a.sa.marine = 1.5 #marine surface area power law
a.v.marine = 1.48 #a_V for marine surface water volume
a.m.marine = 1.32 # upper limit fora_m for mass for marine surface water in table S4
a.ssa.marine = 1.98 # A_S1/SA for marine surface water
R.ave.marine = 0.77 #average width to length ratio for microplastics in marine enviornment
p.ave.marine = 1.10 #average density in marine surface water

#upper values
alpha.upper = 2.5 #table s4 for upper surface water. length
a.sa.upper = 1.8 #upper surface area power law
a.v.upper = 1.4 #a_V for upper surface water volume
a.m.upper = 1.24 # upper limit fora_m for mass for upper surface water in table S4
a.ssa.upper = 2.5 # A_S1/SA for upper surface water
R.ave.upper = 0.8 #average width to length ratio for microplastics in upper enviornment
p.ave.upper = 1.20 #average density in upper surface water

#lower values
alpha.lower = 1.95 #table s4 for lower surface water. length
a.sa.lower = 1.4 #lower surface area power law
a.v.lower = 1.39 #a_V for lower surface water volume
a.m.lower = 1.24 # upper limit fora_m for mass for lower surface water in table S4
a.ssa.lower = 1.5 # A_S1/SA for lower surface water
R.ave.lower = 0.67 #average width to length ratio for microplastics in lower enviornment
p.ave.lower = 1.0 #average density in lower surface water

alpha_sensitivity <- tibble(
  parameter = c("length", "surface area", "volume", "mass", "specific surface area", "width-length ratio", "density"),
  default = c(alpha.marine, a.sa.marine, a.v.marine, a.m.marine, a.ssa.marine, R.ave.marine, p.ave.marine),
  upper = c(alpha.upper, a.sa.upper, a.v.upper, a.m.upper, a.ssa.upper, R.ave.upper, p.ave.upper),
  lower = c(alpha.lower, a.sa.lower, a.v.lower, a.m.lower, a.ssa.lower, R.ave.lower, p.ave.lower))

write.csv(alpha_sensitivity,
          file="alpha_sensitivity_table.csv")
alpha_sensitivity
```
#### Alignments
##### Lower parameters
###### Tissue Translocation 
```{r}
##### SMALL TIER DATA ####
### Set size limits for tier
x2D_set = small_tier_upper_size #upper size range (default)
x1D_set = small_tier_lower_size #lower size range (default)
x1M_set = small_tier_lower_size #lower size range for measured


# calculate ERM for each species
aoc_final_small_lower <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>%  #set to 10um for upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper translocatable
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set, #width #0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set #height #0.5 * R.ave * 0.67 * x1D_set
                         )) %>%  
  #calculate upper translocatable surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, 
                         b = 0.5 * x2M, #width #0.5 * R.ave * x2M, 
                         c = 0.5 * x2M #height #0.5 * R.ave * 0.67 * x2M
                         )) %>%  
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa.lower,  #use alpha = 1 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
  
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable volume 
  mutate(x_LL_v = volumefnx_poly(width = x1D_set, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.lower, #use alpha = 1 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
##--- environmental calculations ---###
  #calculate lower translocatable mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               length = x2M, # upper width
                               p = p.ave)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.lower, #use alpha = 1 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
  ##### specific surface area ERM ####
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower translocatable 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper translocatable SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

###### Ingestion
```{r}
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large_lower <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa.lower, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.lower, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.lower, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

##### Upper parameters
###### Tissue Translocation 
```{r}
##### SMALL TIER DATA ####
### Set size limits for tier
x2D_set = small_tier_upper_size #upper size range (default)
x1D_set = small_tier_lower_size #lower size range (default)
x1M_set = small_tier_lower_size #lower size range for measured


# calculate ERM for each species
aoc_final_small_upper <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>%  #set to 10um for upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper translocatable
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set, 
                         c = 0.5 * x1D_set
                         )) %>%  
  #calculate upper translocatable surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, 
                         b = 0.5 * x2M, 
                         c = 0.5 * x2M
                         )) %>%  
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa.upper, #use alpha = 2 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
  
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable volume 
  mutate(x_LL_v = volumefnx_poly(width = x1D_set, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.upper, #use alpha = 2 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
##--- environmental calculations ---###
  #calculate lower translocatable mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               length = x2M, # upper width
                               p = p.ave)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.upper, #use alpha = 2 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
  ##### specific surface area ERM ####
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower translocatable 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper translocatable SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

###### Ingestion
```{r}
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large_upper <- aoc_intermediate_alphas  %>% 
  #filter(lvl1 == "fitness") %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa.upper, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.upper, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.upper, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

### Calculate Thresholds
```{r}
####---- TISSUE TRANSLOCATION ------#####
#### DEFAULT ####
filtered.data.small.default_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.default_t3.4 <- filtered.data.small.default_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.default.t1 <- SSD_function_t1(filtered.data = filtered.data.small.default_t1.2, hcxlcl = 5)
small.default.t2 <- SSD_function_t2(filtered.data = filtered.data.small.default_t1.2, hcx = 5)
small.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.default_t3.4, hcx = 5)
small.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.default_t3.4, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.small.lower_t1.2 <- aoc_final_small_lower %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.lower_t3.4 <- filtered.data.small.lower_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.small.lower_t1.2, hcxlcl = 5)
small.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.small.lower_t1.2, hcx = 5)
small.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower_t3.4, hcx = 5)
small.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower_t3.4, hcx = 10)

#### upper ####
#data filtering
filtered.data.small.upper_t1.2 <- aoc_final_small_upper %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.upper_t3.4 <- filtered.data.small.upper_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.small.upper_t1.2, hcxlcl = 5)
small.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.small.upper_t1.2, hcx = 5)
small.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper_t3.4, hcx = 5)
small.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper_t3.4, hcx = 10)

####---- Food Dilution ------#####
#### DEFAULT ####
filtered.data.large.default_t1.2 <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.default_t3.4 <- filtered.data.large.default_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.default.t1 <- SSD_function_t1(filtered.data = filtered.data.large.default_t1.2, hcxlcl = 5)
large.default.t2 <- SSD_function_t2(filtered.data = filtered.data.large.default_t1.2, hcx = 5)
large.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.default_t3.4, hcx = 5)
large.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.default_t3.4, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.large.lower_t1.2 <- aoc_final_large_lower %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.lower_t3.4 <- filtered.data.large.lower_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.large.lower_t1.2, hcxlcl = 5)
large.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.large.lower_t1.2, hcx = 5)
large.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower_t3.4, hcx = 5)
large.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower_t3.4, hcx = 10)

#### upper ####
#data filtering
filtered.data.large.upper_t1.2 <- aoc_final_large_upper %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.upper_t3.4 <- filtered.data.large.upper_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.large.upper_t1.2, hcxlcl = 5)
large.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.large.upper_t1.2, hcx = 5)
large.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper_t3.4, hcx = 5)
large.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper_t3.4, hcx = 10)

# arrange
polydispersity_sensitivity_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Tissue Translocation (Default)" = c(small.default.t1, small.default.t2, small.default.t3, small.default.t4),
  "Tissue Translocation (lower)" = c(small.lower.t1, small.lower.t2, small.lower.t3, small.lower.t4),
  "Tissue Translocation (upper)" = c(small.upper.t1, small.upper.t2, small.upper.t3, small.upper.t4),
  "Food Dilution (Default)" = c(large.default.t1, large.default.t2, large.default.t3, large.default.t4),
  "Food Dilution (lower)" = c(large.lower.t1, large.lower.t2, large.lower.t3, large.lower.t4),
  "Food Dilution (upper)" = c(large.upper.t1, large.upper.t2, large.upper.t3, large.upper.t4))
  
write.csv(polydispersity_sensitivity_thresholds,
          "./Concentration data/Threshold_Manuscript_Figs/polydispersity_sensitivity.csv")

polydispersity_sensitivity_thresholds
```

## Alignments (all parameters)
Whereas the previous section tested the effects of alignments on the polydisperse effect studies, the following Section assess the sensitivity of the alignments on everything else.

##### Default
###### Ingestion
```{r}
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large_default_alignment <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

##### Lower parameters
###### Tissue Translocation 
```{r}
##### SMALL TIER DATA ####
### Set size limits for tier
x2D_set = small_tier_upper_size #upper size range (default)
x1D_set = small_tier_lower_size #lower size range (default)
x1M_set = small_tier_lower_size #lower size range for measured


# calculate ERM for each species
aoc_final_small_lower_alignment <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>%  #set to 10um for upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha.lower, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper translocatable
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default
                        a = alpha.lower)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set, #width #0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set #height #0.5 * R.ave * 0.67 * x1D_set
                         )) %>%  
  #calculate upper translocatable surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, 
                         b = 0.5 * x2M, #width #0.5 * R.ave * x2M, 
                         c = 0.5 * x2M #height #0.5 * R.ave * 0.67 * x2M
                         )) %>%  
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa.lower, x_UL_sa, x_LL_sa)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa,  #use alpha = 1 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
  
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable volume 
  mutate(x_LL_v = volumefnx_poly(width = x1D_set, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v.lower, x_UL_v, x_LL_v)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, #use alpha = 1 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
##--- environmental calculations ---###
  #calculate lower translocatable mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave.lower)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               length = x2M, # upper width
                               p = p.ave.lower)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m.lower, x_UL_m, x_LL_m)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, #use alpha = 1 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
  ##### specific surface area ERM ####
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower translocatable 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper translocatable SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa.lower, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

###### Ingestion
```{r}
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large_lower_alignment <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha.lower, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha.lower)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa.lower, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v.lower, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.lower, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave.lower)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave.lower)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m.lower, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.lower, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa.lower, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

##### Upper parameters
###### Tissue Translocation 
```{r}
##### SMALL TIER DATA ####
### Set size limits for tier
x2D_set = small_tier_upper_size #upper size range (default)
x1D_set = small_tier_lower_size #lower size range (default)
x1M_set = small_tier_lower_size #lower size range for measured


# calculate ERM for each species
aoc_final_small_upper_alignment <- aoc_intermediate_alphas  %>% 
   # define upper size length for Translocation
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ upper.tissue.trans.size.um, 
                         max.size.ingest.um  < upper.tissue.trans.size.um ~  max.size.ingest.um,
                         max.size.ingest.um  > upper.tissue.trans.size.um ~ upper.tissue.trans.size.um)) %>%  #set to 10um for upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha.upper, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit (width of particle)
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper translocatable
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default
                        a = alpha.upper)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set, #width #0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set #height #0.5 * R.ave * 0.67 * x1D_set
                         )) %>%  
  #calculate upper translocatable surface area
  mutate(x_UL_sa = SAfnx(a = 0.5 * x2M, 
                         b = 0.5 * x2M, #width #0.5 * R.ave * x2M, 
                         c = 0.5 * x2M #height #0.5 * R.ave * 0.67 * x2M
                         )) %>%  
  #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa.upper, x_UL_sa, x_LL_sa)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa,  #use alpha = 1 eq.
                                  x_LL = particle.surface.area.um2.min,
                                  x_UL = particle.surface.area.um2.max))) %>% 
  
   #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower translocatable volume 
  mutate(x_LL_v = volumefnx_poly(width = x1D_set, # lower width
                                 length = x1D_set)) %>%
  #calculate maximum translocatable volume 
  mutate(x_UL_v = volumefnx_poly(width = x2M, #upper translocatable width
                                 length = x2M)) %>% 
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v.upper, x_UL_v, x_LL_v)) %>% 
  
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v, #use alpha = 1 eq.
                                  x_LL = particle.volume.um3.min,
                                  x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
##--- environmental calculations ---###
  #calculate lower translocatable mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set, # lower width
                               length = x1D_set, # lower size limit
                               p = p.ave.upper)) %>%  #average density
  #calculate upper translocatable mass
  mutate(x_UL_m = massfnx_poly(width = x2M, # upper width
                               length = x2M, # upper width
                               p = p.ave.upper)) %>%  #average density
  # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m.upper, x_UL_m, x_LL_m)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m, #use alpha = 1 eq.
                                  x_UL = mass.per.particle.mg.max * 1000,
                                  x_LL = mass.per.particle.mg.min * 1000))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
  ##### specific surface area ERM ####
#note that mu were calcaulted for polydisperse particles before, so not special case needed here
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower translocatable 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper translocatable SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa.upper, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

###### Ingestion
```{r}
x2D_set <- large_tier_upper_size #upper size range (default)
x1D_set <- large_tier_lower_size #lower size range (default)
x1M_set <- large_tier_lower_size #lower size range for measured
# calculate ERM for each species

aoc_final_large_upper_alignment <- aoc_intermediate_alphas  %>% 
  # remove algae, as food dilution MOE doesn't make sense for algae
  filter(Group != "Algae") %>% 
  # define upper size length for ingestion 
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ x2D_set, 
                         max.size.ingest.um < x2D_set ~ max.size.ingest.um,
                         max.size.ingest.um > x2D_set ~ x2D_set
                         )) %>%  #set to 5,000 as upper limit or max size ingest, whichever is smaller
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
  mutate(mu.p.poly = mux.polyfnx(a.x = alpha.upper, #alpha for particles
                                 x_UL= x2M, #upper ingestible size limit
                                 x_LL = x1M_set)) %>% 
  # polydisperse effect threshold for particles
  mutate(EC_poly_p.particles.mL = (EC_mono_p.particles.mL * mu.p.mono)/mu.p.poly) %>% 
   #calculate CF_bio for all conversions
  mutate(CF_bio = CFfnx(x1M = x1M_set,#lower size bin
                        x2M = x2M, #upper ingestible length
                        x1D = x1D_set, #default
                        x2D = x2D_set,  #default upper size range
                        a = alpha.upper)) %>%  
  ## Calculate environmentally relevant effect threshold for particles
  mutate(EC_env_p.particles.mL = EC_poly_p.particles.mL * CF_bio) %>%  #aligned particle effect concentraiton (1-5000 um)
  
  #### Surface area ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible surface area
  mutate(x_LL_sa = SAfnx(a = 0.5 * x1D_set, 
                         b = 0.5 * x1D_set,# 0.5 * R.ave * x1D_set, 
                         c = 0.5 * x1D_set)) %>% #0.5 * R.ave * 0.67 * x1D_set)) %>%  
  #calculate upper ingestible surface area
  mutate(x_UL_sa = SAfnx( 
    #CONSERVATIVE ASSUMPTION #a = 0.5 * x2D_set, #length is upper default limit by definition (accounts for fibers)
                         a = 0.5 * x2M, #LENGTH-limited (less conservative assumption)
                         b = 0.5 * x2M,#0.5 * R.ave * x2M, #length-limited
                         c = 0.5 * x2M)) %>%   #0.5 * R.ave * 0.67 * x2M)) %>%  #height based on ingestible length
 #calculate mu_x_poly (env) for surface area
  mutate(mu.sa.poly = mux.polyfnx(a.sa.upper, x_UL_sa, x_LL_sa)) %>% 
  
   ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.sa.mono = case_when(
    polydispersity == "monodisperse" ~ particle.surface.area.um2, # use reported surface area in monodisperse
    polydispersity == "polydisperse" ~  mux.polyfnx(a.x = a.sa, 
                                                    x_LL = particle.surface.area.um2.min,
                                                    x_UL = particle.surface.area.um2.max))) %>% 
  #calculate polydisperse effect concentration for surface area (particles/mL)
  mutate(EC_poly_sa.particles.mL = (EC_mono_p.particles.mL * mu.sa.mono)/mu.sa.poly) %>%  
  #calculate environmentally realistic effect threshold
  mutate(EC_env_sa.particles.mL = EC_poly_sa.particles.mL * CF_bio) %>% 
  
  #### volume ERM ####
##--- environmental calculations ---###
  #calculate lower ingestible volume 
  mutate(x_LL_v = volumefnx_poly(length = x1D_set,
                                 width = x1D_set)) %>% 
  #calculate maximum ingestible volume 
  mutate(x_UL_v = volumefnx_poly(length = x2M, # length-limited
                                 #x2D_set, #upper definiton (accouunts for fibers) CONSERVATIVE
                                 width = x2M)) %>% #ingestion-limited
  # calculate mu.v.poly
  mutate(mu.v.poly = mux.polyfnx(a.v.upper, x_UL_v, x_LL_v)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.v.mono = case_when(
    polydispersity == "monodisperse" ~ particle.volume.um3, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.v.upper, 
                                                   x_LL = particle.volume.um3.min,
                                                   x_UL = particle.volume.um3.max))) %>% 
  
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_v.particles.mL = (EC_mono_p.particles.mL * mu.v.mono)/mu.v.poly) %>%  
    #calculate environmentally realistic effect threshold
  mutate(EC_env_v.particles.mL = EC_poly_v.particles.mL * CF_bio) %>% 
  
  #### mass ERM ###
  ##--- environmental calculations ---###
  #calculate lower ingestible mass
  mutate(x_LL_m = massfnx_poly(width = x1D_set,
                               length = x1D_set,
                               p = p.ave.upper)) %>% 
  #calculate upper ingestible mass
  mutate(x_UL_m = massfnx_poly(width = x2M, #ingestible
                               length = x2M, #length-limited
                               # CONSERVATIVE length = x2D_set, #upper definition
                               p = p.ave.upper)) %>% #average density
 # calculate mu.m.poly
  mutate(mu.m.poly = mux.polyfnx(a.m.upper, x_UL_m, x_LL_m)) %>% 
  ##--- laboratory calculations ---###
  ## define mu_x_mono OR mu_x_poly (lab) for alignment to ERM  #
  #(note that if mixed particles were used, a different equation must be used)
  mutate(mu.m.mono = case_when(
    polydispersity == "monodisperse" ~  mass.per.particle.mg * 1000, # use reported volume in monodisperse
    polydispersity == "polydisperse" ~ mux.polyfnx(a.x = a.m.upper, 
                                                   x_UL = mass.per.particle.mg.max * 1000,
                                                   x_LL = mass.per.particle.mg.min * 1000))) %>% 
  #calculate polydisperse effect concentration for volume (particles/mL)
  mutate(EC_poly_m.particles.mL = (EC_env_p.particles.mL * mu.m.mono)/mu.m.poly) %>%
  #calculate environmentally realistic effect threshold
  mutate(EC_env_m.particles.mL = EC_poly_m.particles.mL * CF_bio) %>% 
  
 ##### specific surface area ERM ####
  mutate(mu.ssa.mono = mu.sa.mono/mu.m.mono) %>% #define mu_x_mono for alignment to ERM (um^2/ug)
  #calculate lower ingestible 1/SSA
  mutate(x_LL_ssa = SSA.inversefnx(sa = x_LL_sa, #surface area
                           m = x_LL_m) #mass
         ) %>% 
  #calculate upper ingestible SSA  (um^2/ug)
  mutate(x_UL_ssa = SSA.inversefnx(sa = x_UL_sa, #surface area
                           m = x_UL_m) #mass
         ) %>% 
  #calculate mu_x_poly for specific surface area
  #note that mu were calcaulted for polydisperse particles before, so not special case needed here
   mutate(mu.ssa.inverse.poly = mux.polyfnx(a.ssa.upper, x_UL_ssa, x_LL_ssa)) %>% 
  #calculate polydisperse effect concentration for specific surface area (particles/mL)
  mutate(mu.ssa.poly = 1 / mu.ssa.inverse.poly) %>%  #calculate mu_SSA from inverse
  mutate(EC_poly_ssa.particles.mL = (EC_env_p.particles.mL * mu.ssa.mono)/mu.ssa.poly) %>% 
  #calculate environmentally realistic effect threshold
  mutate(EC_env_ssa.particles.mL = EC_poly_ssa.particles.mL * CF_bio)
```

### Calculate Thresholds
```{r}
####---- TISSUE TRANSLOCATION ------#####
#### DEFAULT ####
filtered.data.small.default_t1.2 <- aoc_final_small %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.default_t3.4 <- filtered.data.small.default_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.default.t1 <- SSD_function_t1(filtered.data = filtered.data.small.default_t1.2, hcxlcl = 5)
small.default.t2 <- SSD_function_t2(filtered.data = filtered.data.small.default_t1.2, hcx = 5)
small.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.default_t3.4, hcx = 5)
small.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.default_t3.4, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.small.lower_t1.2 <- aoc_final_small_lower_alignment %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.lower_t3.4 <- filtered.data.small.lower_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.small.lower_t1.2, hcxlcl = 5)
small.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.small.lower_t1.2, hcx = 5)
small.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower_t3.4, hcx = 5)
small.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.lower_t3.4, hcx = 10)

#### upper ####
#data filtering
filtered.data.small.upper_t1.2 <- aoc_final_small_upper_alignment %>% 
         mutate(dose_new = EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))

filtered.data.small.upper_t3.4 <- filtered.data.small.upper_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
small.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.small.upper_t1.2, hcxlcl = 5)
small.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.small.upper_t1.2, hcx = 5)
small.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper_t3.4, hcx = 5)
small.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.small.upper_t3.4, hcx = 10)

####---- Food Dilution ------#####
#### DEFAULT ####
filtered.data.large.default_t1.2 <- aoc_final_large_default_alignment %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.default_t3.4 <- filtered.data.large.default_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.default.t1 <- SSD_function_t1(filtered.data = filtered.data.large.default_t1.2, hcxlcl = 5)
large.default.t2 <- SSD_function_t2(filtered.data = filtered.data.large.default_t1.2, hcx = 5)
large.default.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.default_t3.4, hcx = 5)
large.default.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.default_t3.4, hcx = 10)

#### LOWER ####
#data filtering
filtered.data.large.lower_t1.2 <- aoc_final_large_lower_alignment %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.lower_t3.4 <- filtered.data.large.lower_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.lower.t1 <- SSD_function_t1(filtered.data = filtered.data.large.lower_t1.2, hcxlcl = 5)
large.lower.t2 <- SSD_function_t2(filtered.data = filtered.data.large.lower_t1.2, hcx = 5)
large.lower.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower_t3.4, hcx = 5)
large.lower.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.lower_t3.4, hcx = 10)

#### upper ####
#data filtering
filtered.data.large.upper_t1.2 <- aoc_final_large_upper_alignment %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, x1D_set, x2D_set))

filtered.data.large.upper_t3.4 <- filtered.data.large.upper_t1.2 %>% 
 filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population"))

# get thresholds
large.upper.t1 <- SSD_function_t1(filtered.data = filtered.data.large.upper_t1.2, hcxlcl = 5)
large.upper.t2 <- SSD_function_t2(filtered.data = filtered.data.large.upper_t1.2, hcx = 5)
large.upper.t3 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper_t3.4, hcx = 5)
large.upper.t4 <- SSD_function_t3_4(filtered.data = filtered.data.large.upper_t3.4, hcx = 10)

# arrange
alignment_sensitivity_thresholds <- tibble(
  "Tier" = c('Tier1', 'Tier2', 'Tier3', 'Tier4'),
  "Tissue Translocation (Default)" = c(small.default.t1, small.default.t2, small.default.t3, small.default.t4),
  "Tissue Translocation (lower)" = c(small.lower.t1, small.lower.t2, small.lower.t3, small.lower.t4),
  "Tissue Translocation (upper)" = c(small.upper.t1, small.upper.t2, small.upper.t3, small.upper.t4),
  "Food Dilution (Default)" = c(large.default.t1, large.default.t2, large.default.t3, large.default.t4),
  "Food Dilution (lower)" = c(large.lower.t1, large.lower.t2, large.lower.t3, large.lower.t4),
  "Food Dilution (upper)" = c(large.upper.t1, large.upper.t2, large.upper.t3, large.upper.t4))
  
write.csv(alignment_sensitivity_thresholds,
          "./Concentration data/Threshold_Manuscript_Figs/alignment_sensitivity.csv")

alignment_sensitivity_thresholds
```

## Leave-one-out method
These techniques are based on the Leave-One-Out-method, in which we recalculate the results of our meta-analysis K − 1  times, each times leaving out one study. This way, we can more easily detect studies which influence the overall estimate of our meta-analysis the most, and this lets us better assess if this influence may distort our pooled effect (Viechtbauer and Cheung 2010).

### SSD and Sensitivity analysis Functions
```{r}
# Leave-one-out by study function
sensitivity <- Vectorize(function(x, y) {
  train <- filtered.data[!filtered.data$doi %in% x,]
   SSD_function_t2(train, hcx = y)
})

#tier 3 & 4 sensitivity funciton
sensitivity_t3_4 <- Vectorize(function(x, y) {
  train <- filtered.data[!filtered.data$doi %in% x,]
   SSD_function_t3_4(train, hcx = y)
})
```

###  Tier 1 - Translocation
```{r warning=FALSE}
#data filtering
filtered.data <- aoc_final_small %>% 
         mutate(dose_new = 1000 * EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))# %>% 
#  filter(environment == "Marine")

#HC metrics to calculate
y <- list(hcxlcl = 5)

# get HC5 for all studies
aoc_final_smalllcl <- SSD_function_t1(filtered.data = filtered.data, hcxlcl = 5)

#unique study names
doi.u <- unique(filtered.data$doi)


# generate matrix
o2 <- 'rownames<-'(outer(doi.u, y, sensitivity_t1), doi.u)

#bind DOI's to other parameters of interest for analysis
o2_tbl <- rownames_to_column(as.data.frame(o2), var = "doi")
  
tier1_small_SA <- left_join(o2_tbl, filtered.data, by = "doi") %>% 
  dplyr::select(authors,
                year,
                Species, 
                dose_new,
                EC_env_sa.particles.mL,
                hcxlcl,
                acute.chronic_f, 
                bio_f,
               lvl2_f,
                size.length.um.used.for.conversions,
                doi) 
tier1_small_SA
```
Save
```{r}
t1_2_SA <- tier1_small_SA %>%  
  distinct(doi, authors, year, hcxlcl) 
```

#### Analytics
```{r}
#study
tier1_small_SA %>% 
  distinct(doi, authors, year, hcxlcl, Species) %>% 
  mutate(authoryear = paste(authors, "et. al ", year)) %>% 
   mutate(diff = (hcxlcl - aoc_final_smalllcl)/aoc_final_smalllcl) %>% 
  group_by(doi, authoryear, diff) %>% 
  summarise(species_multiple = paste(Species, collapse = ", ")) %>%
  ungroup() %>% 
  mutate(doi = fct_reorder(doi, diff)) %>% 
  ggplot(aes(x = diff, y = doi)) +
  geom_point() +
  geom_text_repel(aes(label = species_multiple),
                  size= 3,
                 # position = position_nudge(x = 0.05),
                  ylim = c(NA,NA),
                 hjust = 0.8,
                  nudge_x = 0.25,
                  force = 2,
                   color = "black",
                   direction = "x",
                 #  point.padding = 1,
                 # segment.shape = 1,
                 # segment.inflect = TRUE,
                 # box.padding = 1,
                 fontface = "italic") +
  geom_line() +
  geom_vline(xintercept = 0, alpha = 0.5, color = "grey60") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  xlab("% Difference") +
  ylab("Study") +
  labs(title = "Sensitivity Analysis (Leave-one-study-out-method)",
       subtitle = "95% lower CI:HC5 - Tier 1(Tissue Translocation)") +
  theme_minimal() +
  theme(legend.position = 'none')
```

###  Tier 2 - Translocation
```{r warning=FALSE}
#data filtering
Marine_t1_ox <- aoc_final_small %>% 
         mutate(dose_new = 1000 * EC_env_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um))# %>% 
#  filter(environment == "Marine")

#HC metrics to calculate
y <- list(hc5 = 5)

# get HC5 for all studies
aoc_final_smallhc5 <- SSD_function_t2(filtered.data = Marine_t1_ox, hcx = 5)

#unique study names
doi.u <- unique(filtered.data$doi)


# generate matrix
o2 <- 'rownames<-'(outer(doi.u, y, sensitivity), doi.u)

#bind DOI's to other parameters of interest for analysis
o2_tbl <- rownames_to_column(as.data.frame(o2), var = "doi")
  
tier2_small_SA <- left_join(o2_tbl, filtered.data, by = "doi") %>% 
  dplyr::select(authors,
                year,
                Species, 
                dose_new,
                EC_env_sa.particles.mL,
                hc5,
                acute.chronic_f, 
                bio_f,
               lvl2_f,
                size.length.um.used.for.conversions,
                doi) 
tier2_small_SA
```
Save
```{r}
t2_SA <- tier2_small_SA %>%  
  distinct(doi, authors, year, hc5) 
```

#### Analytics
```{r}
#study
tier1_2_small_SA_sens <- tier2_small_SA %>% 
   distinct(doi, authors, year, hc5, Species) %>% 
  mutate(authoryear = paste(authors, "et. al ", year)) %>% 
   mutate(authoryear = fct_reorder(authoryear, hc5)) %>% 
  mutate(diff = (hc5 - aoc_final_smallhc5)/aoc_final_smallhc5) %>% 
  group_by(doi, authoryear, diff) %>% 
  summarise(species_multiple = paste(Species, collapse = ", ")) %>% 
#plot
  ggplot(aes(x = diff, y = authoryear)) + 
  geom_point(color = "black") +
  geom_text_repel(aes(label = species_multiple),
                  size= 3,
                 # position = position_nudge(x = 0.05),
                  ylim = c(NA,NA),
                 hjust = 0.8,
                  nudge_x = 0.25,
                  force = 2,
                   color = "black",
                   direction = "x",
                 #  point.padding = 1,
                 # segment.shape = 1,
                 # segment.inflect = TRUE,
                 # box.padding = 1,
                 fontface = "italic") +
  geom_line() +
  geom_vline(xintercept = 0, alpha = 0.5, color = "grey60") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  xlab("% Difference") +
  ylab("Study") +
  # labs(title = "Sensitivity Analysis",
  #      subtitle = "Tissue Translocation : Tiers 1 & 2") +
  theme.type +
  theme(legend.position = 'none')
tier1_2_small_SA_sens
```

###  Tiers 3 and 4 - Translocation
```{r warning=FALSE}
#data filtering
filtered.data <- aoc_final_small %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_poly_sa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, upper.tissue.trans.size.um)) # %>% 
 # filter(environment == "Marine")


#HC metrics to calculate
y <- list(hc5 = 5,hc10 = 10)

# get HC5 for all studies
tier3_4_small_SAhc5 <- SSD_function_t3_4(filtered.data, hcx = 5)

#unique study names
doi.u <- unique(filtered.data$doi)


# generate matrix
o2 <- 'rownames<-'(outer(doi.u, y, sensitivity_t3_4), doi.u)

#bind DOI's to other parameters of interest for analysis
tier3_4_small_SA <- cbind(filtered.data, o2[match(filtered.data$doi, rownames(o2)), ]) %>% 
  dplyr::select(authors,
                year,
                Species, 
                dose_new,
                EC_env_sa.particles.mL,
                hc5, 
                hc10, 
                acute.chronic_f, 
                bio_f,
                lvl2_f,
                size.length.um.used.for.conversions,
                doi) 
tier3_4_small_SA
```
Save
```{r}
t3_4_SA <- tier3_4_small_SA %>%  
  distinct(doi, authors, year, hc5, hc10) 
```

#### Analytics
```{r}
#group species into one column if study has multiple
tier3_4_small_SA_sens <- tier3_4_small_SA %>%
  distinct(doi, authors, year, hc5, Species) %>% 
  mutate(authoryear = paste(authors, "et. al ", year)) %>% 
   mutate(authoryear = fct_reorder(authoryear, hc5)) %>% 
  mutate(diff = (hc5 - tier3_4_small_SAhc5)/tier3_4_small_SAhc5) %>% 
  group_by(doi, authoryear, diff) %>% 
  summarise(species_multiple = paste(Species, collapse = ", ")) %>% 
#plot
  ggplot(aes(x = diff, y = authoryear)) + 
  geom_point(color = "black") +
  geom_text_repel(aes(label = species_multiple),
                  size= 3,
                 # position = position_nudge(x = 0.05),
                  ylim = c(NA,NA),
                 hjust = 0.8,
                  nudge_x = 0.25,
                  force = 2,
                   color = "black",
                   direction = "x",
                 #  point.padding = 1,
                 # segment.shape = 1,
                 # segment.inflect = TRUE,
                 # box.padding = 1,
                 fontface = "italic") +
  geom_line() +
  geom_vline(xintercept = 0, alpha = 0.5, color = "grey60") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  xlab("% Difference") +
  ylab("Study") +
  # labs(title = "Sensitivity Analysis (Leave-one-study-out-method)",
  #      subtitle = "Tissue Translocation: Tiers 3 & 4") +
  theme.type +
  #theme_minimal() +
  theme(legend.position = 'none')

tier3_4_small_SA_sens
```


###  Tiers 1 and 2 - Food Dilution
```{r warning=FALSE}
#data filtering
filtered.data <- aoc_final_large %>% 
         mutate(dose_new = EC_env_v.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new * 1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) #%>% 
#  filter(environment == "Marine")

#HC metrics to calculate
y <- list(hc5 = 5,hc10 = 10)

# get HC5 for all studies
tier1_2_large_Vhc5 <- SSD_function_t2(filtered.data, hcx = 5)

#unique study names
doi.u <- unique(filtered.data$doi)


# generate matrix
o2 <- 'rownames<-'(outer(doi.u, y, sensitivity), doi.u)

#bind DOI's to other parameters of interest for analysis
tier1_2_large_V <- cbind(filtered.data, o2[match(filtered.data$doi, rownames(o2)), ]) %>% 
  dplyr::select(authors,
                year,
                Species, 
                dose_new,
                EC_env_sa.particles.mL,
                hc5, 
                hc10, 
                acute.chronic_f, 
                bio_f,
                lvl2_f,
                size.length.um.used.for.conversions,
                doi) 
tier1_2_large_V
```
Save
```{r}
t1_2_volume <- tier1_2_large_V %>%  
  distinct(doi, authors, year, hc5, hc10) 
```

#### Analytics
```{r}
#study
tier1_2_large_V_sens <- tier1_2_large_V %>%
  distinct(doi, authors, year, hc5, Species) %>% 
  mutate(authoryear = paste(authors, "et. al ", year)) %>% 
   mutate(authoryear = fct_reorder(authoryear, hc5)) %>% 
  mutate(diff = (hc5 - tier1_2_large_Vhc5)/tier1_2_large_Vhc5) %>% 
  group_by(doi, authoryear, diff) %>% 
  summarise(species_multiple = paste(Species, collapse = ", ")) %>% 
#plot
  ggplot(aes(x = diff, y = authoryear)) + 
  geom_point(color = "black") +
  geom_text_repel(aes(label = species_multiple),
                  size= 3,
                 # position = position_nudge(x = 0.05),
                  ylim = c(NA,NA),
                 hjust = 0.8,
                  nudge_x = 0.25,
                  force = 2,
                   color = "black",
                   direction = "x",
                 #  point.padding = 1,
                 # segment.shape = 1,
                 # segment.inflect = TRUE,
                 # box.padding = 1,
                 fontface = "italic") +
  geom_line() +
  geom_vline(xintercept = 0, alpha = 0.5, color = "grey60") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  xlab("% Difference") +
  ylab("Study") +
  # labs(title = "Sensitivity Analysis",
  #      subtitle = "Food Dilution: Tiers 1 & 2") +
  theme.type +
  #theme_minimal() +
  theme(legend.position = 'none')

tier1_2_large_V_sens
```

###  Tiers 3 and 4 - Food Dilution
```{r warning=FALSE}
#data filtering
filtered.data <- aoc_final_large %>% 
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population")) %>% 
         mutate(dose_new = EC_poly_ssa.particles.mL / (af.time * af.noec)) %>%  
         drop_na(dose_new) %>% 
         mutate(dose_new = dose_new*1000) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000)) # %>% 
 # filter(environment == "Marine")

#HC metrics to calculate
y <- list(hc5 = 5,hc10 = 10)

# get HC5 for all studies
tier3_4_large_Vhc5 <- SSD_function_t3_4(filtered.data, hcx = 5)

#unique study names
doi.u <- unique(filtered.data$doi)


# generate matrix
o2 <- 'rownames<-'(outer(doi.u, y, sensitivity_t3_4), doi.u)

#bind DOI's to other parameters of interest for analysis
tier3_4_large_V <- cbind(filtered.data, o2[match(filtered.data$doi, rownames(o2)), ]) %>% 
  dplyr::select(authors,
                year,
                Species, 
                dose_new,
                EC_env_sa.particles.mL,
                hc5, 
                hc10, 
                acute.chronic_f, 
                bio_f,
                lvl2_f,
                size.length.um.used.for.conversions,
                doi) 

tier3_4_large_V %>% 
  datatable()
```
Save
```{r}
t3_4_volume <- tier3_4_large_V %>%  
  distinct(doi, authors, year, hc5, hc10) 
```

#### Analytics
```{r}
#study
tier3_4_large_V_sens <- tier3_4_large_V %>% 
  distinct(doi, authors, year, hc5, Species) %>% 
  mutate(authoryear = paste(authors, "et. al ", year)) %>% 
   mutate(authoryear = fct_reorder(authoryear, hc5)) %>% 
  mutate(diff = (hc5 - tier3_4_large_Vhc5)/tier3_4_large_Vhc5) %>% 
  group_by(doi, authoryear, diff) %>% 
  summarise(species_multiple = paste(Species, collapse = ", ")) %>% 
#plot
  ggplot(aes(x = diff, y = authoryear)) + 
  geom_point(color = "black") +
  geom_text_repel(aes(label = species_multiple),
                  size= 3,
                 # position = position_nudge(x = 0.05),
                  ylim = c(NA,NA),
                 hjust = 0.8,
                  nudge_x = 0.25,
                  force = 2,
                   color = "black",
                   direction = "x",
                 #  point.padding = 1,
                 # segment.shape = 1,
                 # segment.inflect = TRUE,
                 # box.padding = 1,
                 fontface = "italic") +

  geom_line() +
  geom_vline(xintercept = 0, alpha = 0.5, color = "grey60") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  xlab("% Difference") +
  ylab("Study") +
  # labs(title = "Sensitivity Analysis (Leave-one-study-out-method)",
  #      subtitle = "HC5 - Tier 3 & 4 (Volume)") +
  theme.type +
  #theme_minimal() +
  theme(legend.position = 'none')
#print
tier3_4_large_V_sens
```

## Export to Excel
```{r eval=FALSE, include=FALSE}
library(openxlsx)
write.xlsx(t1_2_SA, file="sensitivityAnalysis_10-5-2021.xlsx", sheetName="Tier 1&2_translocation", row.names=TRUE)
write.xlsx(t3_4_SA, file="sensitivityAnalysis_10-5-2021.xlsx", sheetName="Tier 3&4_translocation", row.names=TRUE, append = TRUE)
write.xlsx(t1_2_volume, file="sensitivityAnalysis_10-5-2021.xlsx", sheetName="Tier 1&2_food dilution", row.names=TRUE, append = TRUE)
write.xlsx(t3_4_volume, file="sensitivityAnalysi_10-5-2021s.xlsx", sheetName="Tier 1&2_food dilution", row.names=TRUE, append = TRUE)

## doesn't seem to work, so do manual export to .csv and append to excel file

write.csv(t1_2_SA, file="Tier 1&2_translocation.csv")
write.csv(t3_4_SA, file="Tier 3&4_translocation.csv")
write.csv(t1_2_volume, file="Tier 1&2_food_dilution.csv")
write.csv(t3_4_volume, file="Tier 3&4_food_dilution.csv")
```

## Manuscript Figure
```{r}
sensitivity_Figure <- ggarrange(tier1_2_small_SA_sens, tier3_4_small_SA_sens,
                        tier1_2_large_V_sens,tier3_4_large_V_sens,
                        labels = c("A", "B", "C", "D"), common.legend = TRUE,
                        legend = "bottom",
                        ncol = 2, nrow = 2)

plot(sensitivity_Figure)

ggsave(plot = sensitivity_Figure,
       filename = "sensitivity_Figure_SI-Fig3.jpeg",
       path = "./Concentration data/Threshold_Manuscript_Figs/", 
       width = 12, height = 10, units = "in",
       dpi = 300)
```
# Probabilistic Species Sensitivity Distribution Plus

The below code will not run with an open environment due to memory constraints.
```{r eval=FALSE, include=FALSE}
#library(gdata)
#keep(tier1_2_large, sure = TRUE)
write.csv(tier1_2_large, "tier1_2_large.csv")
#rm(list = ls())
#gc()
tier1_2_large <- read.csv("tier1_2_large.csv")
```

```{r eval=FALSE, include=FALSE}
# -------------------------------------------------------------------------------------------------
# Code based on PSSD and PNEC calculations using the tool PSSD+ developed by Wigger et al. (2019)
# -------------------------------------------------------------------------------------------------
# 
# Associated publication: Systematic consideration of parameter uncertainty and variability in
#                         probabilistic species sensitivity distributions
# 
# Authors: Henning Wigger, Delphine Kawecki, Bernd Nowack and V?ronique Adam
# 
# Institute: Empa, Swiss Federal Laboratories for Materials Science and Technology,
#            Technology and Society Laboratory, Lerchenfeldstrasse 5, 9014 St. Gallen, Switzerland
# 
# submitted to Integrated Environmental Assessment and Management in December 2018
# 
# -------------------------------------------------------------------------------------------------

##### Build functions ####
source("PSSD/rmore.r")
source("PSSD/do.pssd.r")
#source("PSSD/do.pssd.troph.r")
#source("PSSD/do.pssd.ag.r")


###################################################################################################
##### MAIN PARAMETERS FOR THE CALCULATION #########################################################
###################################################################################################

# number of simulations for the triangular distributions of the data points and uncertainty factors
SIM <- 10 #10 is minimum, 10,000 is recommended

# coefficient of variation for the data point distributions
CV.DP <- 0.3

# coefficient of variation for the uncertainty factor distributions
CV.UF <- 0.5
```
## Data preparation
```{r eval=FALSE, include=FALSE}
memory.limit(size=56000) #force R to use more memory than what's phyiscally available
library(reshape2)
require(tidyverse)
require(Matrix)

t2 <- tier1_2_large#%>% 
 # filter(species == "magna")

# prep dataframe for tox datapoints
DP_1_large_volume <-  t2 %>% 
  #mutate(tier = "A") %>% 
  mutate(id = row_number()) %>% 
  mutate(particles_L = EC_env_v.particles.mL * 1000) %>% #must *10 to eliminate sub-zero values, can't go above 10 b/c R can't handle integers above certain value
  mutate(particles_L_round = as.integer(round(particles_L,digits = 1))) %>%  #must make integer for below code to work
  select(id, species, particles_L_round) %>% 
  acast(id ~ species, value.var = "particles_L_round") %>% 
  Matrix(sparse = TRUE) # makes a smaller object than using as.matrix()

#prep uncertainty factors for conversion from acute to chronic values
UFt_1_large_volume <- t2 %>% 
  mutate(id = row_number()) %>% 
  select(id, species, af.time) %>% 
  acast(id ~ species, value.var = "af.time")

# uncertainty factors for conversion of dose descriptors to NOEC
UFdd_1_large_volume <- t2 %>% 
  mutate(id = row_number()) %>% 
  select(id, species, af.noec) %>% 
  acast(id ~ species, value.var = "af.noec")
#rm(tier1_2_large)
```

## Tier 1_2_Large_volume
```{r eval=FALSE, include=FALSE}
# read toxicity data
# data points from the literature
DP   <- DP_1_large_volume
# uncertainty factors for conversion from acute to chronic values
UFt  <- UFt_1_large_volume
# uncertainty factors for conversion of dose descriptors to NOEC
UFdd <- UFdd_1_large_volume


###################################################################################################
##### CALCULATION OF NOECs and PSSDs ##############################################################
###################################################################################################

# estimate the NOEC distributions for each species
NOEC_t1_volume <- do.pSSD(DP, UFt, UFdd, SIM, CV.DP, CV.UF)

# calculate the deterministic (modal) values of the NOEC
NOEC.det_t1_volume <- DP/(UFdd*UFt)

###convert to particles/L
NOEC2_t1_volume = NOEC_t1_volume * 1000 #correction factor from getting into integer values (above)
NOEC.det2_t1_volume = NOEC.det_t1_volume * 1000
```

```{r eval=FALSE, include=FALSE}
###################################################################################################
##### EXAMPLES OF RESULTS #########################################################################
###################################################################################################

### Plot the probability densities of some species

## For a species with 1 endpoint (T. thermophila)
# histogram
hist(log10(NOEC2_t1_volume[3,]), freq = F, main = "Ceriodaphnia dupia - 14 endpoints", xlab = "NOEC log[particles/L]")
# probability density
lines(density(log10(NOEC2_t1_volume[3,])), col = "red", lwd = 2) 
# Deterministic NOEC
abline(v = median(log10(NOEC.det2_t1_volume[,3]), na.rm = TRUE), col = "green", lty = 2, lwd = 2)
```

```{r eval=FALSE, include=FALSE}
## For a species with 1 endpoints (Danio rerio)
hist(NOEC_t1_volume[11,], freq = F, main = "Danio rero - 1 endpoints", xlab = "NOEC [ug/l]")
lines(density(NOEC_t1_volume[11,]), col = "red", lwd = 2)
abline(v = median(NOEC.det_t1_volume[,11], na.rm = TRUE), col = "green", lty = 2, lwd = 2)

# for(i in 1:length(which(!is.na(NOEC.det[,11])))){
#   abline(v = NOEC.det[i,11], col = "green", lty = 2, lwd = 2)
#}

```

```{r eval=FALSE, include=FALSE}
### Plot PSSDs

# calculate all PSSD curves
xvalues <- seq(-6,7,0.001)
PSSD <- matrix(NA, SIM, length(xvalues))
# calculate the ecdf function
for(i in 1:SIM){
  the.ecdf.f <- ecdf(log10(NOEC_t1_volume[,i], base = 10))
  PSSD[i,] <- the.ecdf.f(xvalues)
}

```
### Plot
```{r eval=FALSE, include=FALSE}
# Plot the mean PSSD
plot(c(-2,8), c(0,1),
     #main = "Microplastics Probabilistic SSD (1-5,000 um; volumetric)",
     xlab = "NOEC (particles/L; aligned to 1-5,000 um by volume)",
     ylab = "Proportion of Species Affected",
     type = "n",
     axes = F)
axis(1, at = c(-2,0,2,4,6,8), labels = expression(10^-2, 10^0, 10^2, 10^4, 10^6, 10^8), lwd = 0.5)
axis(2, lwd = 0.5, las = 1)
abline(h = seq(0,1,0.2), lty = 1, lwd = 0.5, col = "gray90")
abline(v = seq(-6,8,1), lty = 1, lwd = 0.5, col = "gray90")
box(lwd = 0.5)
lines(xvalues, apply(PSSD,2,function(x) {mean(x)}), col = "red", lwd = 1.5)
legend("topleft", "Mean PSSD", col = "red", lwd = 1.5)
```

### GGplot
```{r}
ggplot() +
  geom_line(aes(xvalues, apply(PSSD,2,function(x) {mean(x)})),
            color = "red") +
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^ x, n = 10),
                         labels = trans_format("log10", scales::math_format(10^.x))) +
scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(#title = "Tiers 1 & 2, 1-5000 µm",
    x = "particles/L (aligned to 1-5,000 µm)") +
  coord_trans(x = "log10") +
      theme.type #user theme

```


## Tier 1_2_Small_SurfaceArea
```{r eval=FALSE, include=FALSE}
library(csvread) #64-digit integer

t2_s <- tier1_2_small# %>% 
 # filter(EC_env_sa.particles.mL < 1E4) #have to remove points above integer limit

# prep dataframe for tox datapoints
DP_1_small_surfaceArea <-  t2_s %>% 
  #mutate(tier = "A") %>% 
  mutate(id = row_number()) %>% 
  mutate(particles_L = EC_env_sa.particles.mL * 10000) %>% #must *10 to eliminate sub-zero values, can't go above 10 b/c R can't handle integers above certain value
  mutate(particles_L_round = as.integer(round(particles_L,digits = 1))) %>%  #must make integer for below code to work
  select(id, Species, particles_L_round) %>% 
  acast(id ~ Species, value.var = "particles_L_round") %>% 
  Matrix(sparse = TRUE) # makes a smaller object than using as.matrix()

#prep uncertainty factors for conversion from acute to chronic values
UFt_1_small_surfaceArea <- t2_s %>% 
  mutate(id = row_number()) %>% 
  select(id, Species, af.time) %>% 
  acast(id ~ Species, value.var = "af.time")

# uncertainty factors for conversion of dose descriptors to NOEC
UFdd_1_small_surfaceArea <- t2_s %>% 
  mutate(id = row_number()) %>% 
  select(id, species, af.noec) %>% 
  acast(id ~ species, value.var = "af.noec")

# read toxicity data
# data points from the literature
DP   <- DP_1_small_surfaceArea
# uncertainty factors for conversion from acute to chronic values
UFt  <- UFt_1_small_surfaceArea
# uncertainty factors for conversion of dose descriptors to NOEC
UFdd <- UFdd_1_small_surfaceArea


###################################################################################################
##### CALCULATION OF NOECs and PSSDs ##############################################################
###################################################################################################

# estimate the NOEC distributions for each species
NOEC_t1_SA <- do.pSSD(DP, UFt, UFdd, SIM, CV.DP, CV.UF)

# calculate the deterministic (modal) values of the NOEC
NOEC.det_t1_SA <- DP/(UFdd*UFt)

###convert to particles/L
NOEC2_t1_SA = NOEC_t1_SA * 1000 #correction factor from getting into integer values (above)
NOEC.det2_t1_SA = NOEC.det_t1_SA * 1000
```

## Plotting
```{r eval=FALSE, include=FALSE}
# calculate PNEC (HC5) for volume and surface area
PNEC_t1_SA <- apply(NOEC_t1_SA,2,function(x){quantile(x, probs = 0.05, na.rm = TRUE) })

PNEC_t1_volume <- apply(NOEC_t1_SA,2,function(x){quantile(x, probs = 0.05, na.rm = TRUE) })

#make into dataframe
PNECs <- data.frame( = PNEC_t1_SA,
                    

PNEC %>% 
  data.frame(PNEC) %>% 
  ggplot(aes(x = PNEC))
```


