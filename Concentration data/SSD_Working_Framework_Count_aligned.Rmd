...
![Agreed-upon framework](www/treshhold_framework_42121.png)
...
#       scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
#                          labels = trans_format("log10", scales::math_format(10^.x))) + 
#   labs(title = "Tier 1/2, 10-83 µm", x = "particles/L") +
#       fill.type + #user-selected
#       color.type + #user-selected
#       theme.type #user theme
# 
...
tier1_2_small_1q_ggplot <- ggplot(pred_tier1_2_small_1q,aes_string(x = "est")) +
  ...
  labs(title = "Microplastics Species Sensitivity Distribution",
      ...
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 1/2, 1 < 83 µm", x = "particles/L") +
      fill.type + #user-selected
      color.type + #user-selected
      theme.type #user theme
...
gt(largesdf) %>% 
  tab_header(title = "Tier 1/2, 1µm - 5mm", paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
...
gt(smallsdf) %>% 
  tab_header(title = "Tier 1/2, 1 < 83 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)
             ) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
```
...
endpoints_tier3_4_large <- tier3_4_large %>% 
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tiers 3 & 4, Endpoint Representation", subtitle = "1µm - 5mm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
...
#   geom_bar(stat = "identity") +
#   scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
#   labs(title = "Tier 3 & 4, Endpoint Representation", subtitle = "10µm < 100µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
#   theme_test() +
#   scale_x_discrete(labels = wrap_format(10)) 
#         
# 
...
endpoints_tier3_4_small <- tier3_4_small %>% 
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual(values = wes_palette("Zissou1", 5, type = "continuous")) +
  labs(title = "Tiers 3 & 4, Endpoint Representation", subtitle = "1µm < 83µm", x = "Endpoint Category", y = "Number of Endpoints", fill = "Biological Level\nof Organization", caption = "Number of endpoints is prior to data collapse") +
  theme_test() +
  scale_x_discrete(labels = wrap_format(10)) 
...
gt(largesdf) %>% 
  tab_header(title = "Tier 3/4, 1µm - 5mm", paste("Thresholds presented in particles/L; ERM = ",ERM)) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

gt(smallsdf) %>% 
  tab_header(title = "Tier 3/4, 1 < 83 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)
             ) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)
```
...
```{r endpoint plots2, message=FALSE}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r endpoint plots3 - tiers 3/4, message=FALSE}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse3 - tiers 3/4, include=TRUE}
...
#       scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
#                          labels = trans_format("log10", scales::math_format(10^.x))) + 
#   labs(title = "Tier 3/4, 10-83 µm", x = "particles/L") +
#       fill.type + #user-selected
#       color.type + #user-selected
#       theme.type #user theme
# 
...
tier3_4_small_median_ggplot <- ggplot(pred_tier3_4_small_median,aes_string(x = "est")) +
  ...
  labs(title = "Microplastics Species Sensitivity Distribution",
      ...
      scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                         labels = trans_format("log10", scales::math_format(10^.x))) + 
  labs(title = "Tier 3/4, 1 < 83 µm", x = "particles/L") +
      fill.type + #user-selected
      color.type + #user-selected
      theme.type #user theme
...
gt(largesdf) %>% 
  tab_header(title = "Tier 3/4, 1-5000 µm", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE) 

             
# gt(mediumsdf) %>% 
#   tab_header(title = "Tier 3/4, 10-100 um", subtitle = "Thresholds presented in particles/L") %>% 
#   fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)

gt(smallsdf) %>% 
  tab_header(title = "Tier 3/4, 1-10µm ", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)
...
```{r endpoint plots4, message=FALSE}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
Is body size related to effect threshold for Voluime?
```{r}
...
gt(largesdf) %>% 
  tab_header(title = "Tier 1/2, 1µm - 5mm", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE) 
...
gt(smallsdf) %>% 
  tab_header(title = "Tier 1/2, 1-10µm ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)
             ) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
...
```{r size category selection5 - tiers 3/4, include=TRUE}
...
```{r endpoint plots5 - tiers 3/4, message=FALSE}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse5 - tiers 3/4, include=TRUE}
...
#Raw Data Export for Evaluation - Large Particles, Volume

```{r tables for data review}

a <- tier1_2_large %>% 
  select(doi, authors, risk.13, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, shape_f, poly_f, size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_v.particles.mL, dose_new) %>% 
  rename("Endpoint_Score" = risk.13,
         "First_Author" = authors,
         "Particle_Shape" = shape_f,
         "Effect Metric" = effect.metric,
         "Polymer_Type" = poly_f,
         "Raw_Dose_Particles_mL" = dose.particles.mL.master, 
         "Raw_Dose_Particles_mL_Reported_Converted" = dose.particles.mL.master.converted.reported,
         "Raw_Dose_mg_L" = dose.mg.L.master,
         "Raw_Dose_mg_L_Reported_Converted" = dose.mg.L.master.converted.reported,
         "Organism Group" = org_f,
         "Species" = species_f,
         "Broad_Endpoint_Category" = lvl1_f,
         "Measured_Endpoint" = lvl3_f,
         "Biological_Level" = bio_f,
         "Particle_Length_um" = size.length.um.used.for.conversions, 
         "Dose_Volume_Aligned_particles_mL" = EC_env_v.particles.mL, 
         "Aligned_Dose_AF_Applied_particles_L" = dose_new)  
...
```{r endpoint plots6, message=FALSE}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r endpoint plots7 - tiers 3/4, message=FALSE}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse7 - tiers 3/4, include=TRUE}
...
#Raw Data Export for Evaluation - Small Particles, Surface Area

```{r tables for data review 2}

c <- tier1_2_small %>% 
  select(doi, authors, risk.13, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, shape_f, poly_f, size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_sa.particles.mL, dose_new) %>% 
  rename("Endpoint_Score" = risk.13,
         "First_Author" = authors,
         "Particle_Shape" = shape_f,
         "Effect Metric" = effect.metric,
         ...
         "Biological_Level" = bio_f,
         "Particle_Length_um" = size.length.um.used.for.conversions, 
         "Dose_SurfaceArea_Aligned_particles_mL" = EC_env_sa.particles.mL, 
         "Aligned_Dose_AF_Applied_particles_L" = dose_new) 
...
list_of_datasets <- list("Tier 1_2, 1-5000um, Volume Aligned" = a, "Tier 3_4, 1-5000um, Volume Aligned" = b, "Tier 1_2, 1-83um, Surface Area Aligned" = c, "Tier 3_4, 1-83um, Surface Area Aligned" = d)
write.xlsx(list_of_datasets, file = "SSD_rawdata.xlsx")
...
```{r endpoint plots8, message=FALSE}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r endpoint plots9 - tiers 3/4, message=FALSE}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse9 - tiers 3/4, include=TRUE}
...
```{r size category selection10, include=TRUE}
tier1_2_large <- aoc_final_large %>% 
...
```{r endpoint plots10, message=FALSE}
#large
endpoints_tier1_2_large <- tier1_2_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse10, include=TRUE}
...
```{r size category selection11 - tiers 3/4, include=TRUE}
...
```{r endpoint plots11 - tiers 3/4, message=FALSE}
#large
endpoints_tier3_4_large <- tier3_4_large %>% 
  group_by(lvl1_f, bio_f) %>% 
  mutate(lvl1_count = n()) %>% 
...
```{r data collapse11 - tiers 3/4, include=TRUE}
...
DP_1_large_volume <-  t2 %>% 
  ...
  mutate(particles_L = EC_env_v.particles.mL*10) %>% #must *10 to eliminate sub-zero values, can't go above 10 b/c R can't handle integers above certain value
...
# histogram
hist(log10(NOEC2[3,]), freq = F, main = "Ceriodaphnia dupia - 14 endpoints", xlab = "NOEC log[particles/L]")
# probability density
lines(density(log10(NOEC2[3,])), col = "red", lwd = 2) 
# Deterministic NOEC
abline(v = median(log10(NOEC.det2[,3]), na.rm = TRUE), col = "green", lty = 2, lwd = 2)
```

```{R}
## For a species with 1 endpoints (Danio rerio)
hist(NOEC[11,], freq = F, main = "Danio rero - 1 endpoints", xlab = "NOEC [ug/l]")
lines(density(NOEC[11,]), col = "red", lwd = 2)
abline(v = median(NOEC.det[,11], na.rm = TRUE), col = "green", lty = 2, lwd = 2)
...
hist(PNEC, xlab = "HC5's [particles/l]", main = "Probabilistic HC5's", sub = "1-5,000 um Microplastics; tier 3; volumetric alignment")
...
SSD_function_t1 <- function(filtered.data){
  set.seed(99)
  #data collapse
...
aoc_final_smallhc5 <- SSD_function(filtered.data, hcx = 5)
...
tier1_2_small_SA <- cbind(filtered.data, o2[match(filtered.data$doi, rownames(o2)), ]) %>% 
...
tier1_2_small_SA %>% 
  ...
  labs(title = "Sensitivity Analysis",
       subtitle = "HC5 for Tier 2(Tissue Translocation) by Leave-One-Out (Study)")
```
