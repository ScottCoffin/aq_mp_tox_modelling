...
aoc_final_small <- aoc_intermediate  %>% 
  # define upper size length for ingestion
   # define upper size length for ingestion
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ 10,
                         max.size.ingest.um < 10 ~ max.size.ingest.um,
                         max.size.ingest.um > 10 ~ 10
...
aoc_final_large <- aoc_intermediate  %>% 
  # define upper size length for ingestion
  mutate(x2M = case_when(is.na(max.size.ingest.um) ~ 5000,
                         max.size.ingest.um < 5000 ~ max.size.ingest.um,
                         max.size.ingest.um > 5000 ~ 5000
                         )) %>% 
 # calculate effect threshold for particles
  mutate(EC_mono_p.particles.mL = dose.particles.mL.master) %>% 
  mutate(mu.p.mono = 1) %>% #mu_x_mono is always 1 for particles to particles
...
gt(smallsdf) %>% 
  tab_header(title = "Tier 3/4, 1 < 100 um", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)
             ) %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)
```
...
gt(ERM_small) %>% 
  tab_header(title = "1-10 µm Thresholds (Aligned to 1 - 5000 µm)", subtitle = "Thresholds presented in particles/L") %>% 
