...
aoc_intermediate <- aoc_z %>% 
  ...
  #Remove all temperature treatment LC50s from Jaimukar et al. 2018 besides 22C
  filter(!(article == 42 & media.temp == 18)) %>% 
...
a <- tier1_2_large %>% 
  select(doi, authors, risk.13, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_v.particles.mL, dose_new, sol.rinse) %>% 
  rename("Endpoint_Score" = risk.13,
         "First_Author" = authors,
         "Particle_Shape" = shape_f,
         "Effect Metric" = effect.metric,
         "Acute_Chronic" = acute.chronic_f,
         "Polymer_Type" = poly_f,
...
c <- tier1_2_small %>% 
  select(doi, authors, risk.13, org_f, species_f, lvl1_f, lvl3_f, bio_f, effect.metric, acute.chronic_f, shape_f, poly_f, size.length.um.used.for.conversions, dose.particles.mL.master, dose.particles.mL.master.converted.reported, dose.mg.L.master, dose.mg.L.master.converted.reported, EC_env_sa.particles.mL, dose_new, sol.rinse) %>% 
  rename("Endpoint_Score" = risk.13,
         "First_Author" = authors,
         "Particle_Shape" = shape_f,
         "Effect Metric" = effect.metric,
