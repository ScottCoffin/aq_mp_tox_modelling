...
gt(smallsdf) %>% 
  tab_header(title = "Tier 3/4, 1-83µm ", subtitle = "Thresholds presented in particles/L") %>% 
  fmt_number(columns = vars(Tier_3, Tier_4),decimals = 2, use_seps = TRUE)
...
gt(smallsdf) %>% 
  tab_header(title = "Tier 1/2, 1-83µm ", subtitle = paste("Thresholds presented in particles/L; ERM = ",ERM)
             ) %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),decimals = 2, use_seps = TRUE)
...
oxidative.stress.tiers <- ERM_small %>% 
  ...
  dplyr::select(EC_env_sa_um2.L, `Surface Area`)
...
food.dilution.tiers <- ERM_large %>% 
  ...
  dplyr::select(EC_env_volume_um3.L, Volume)
...
gt(simple.summary) %>% 
  ...
  fmt_number(columns = vars(names(simple.summary)[2:5]), 
             decimals = 0,
             use_seps = TRUE) %>% 
    tab_options(column_labels.hidden = FALSE) %>% 
  tab_spanner(label = "Surface Area ERM",
              columns = vars(names(simple.summary)[2:3])) %>% 
  tab_spanner(label = "Volume ERM",
              columns = vars(names(simple.summary)[4:5]))
```
