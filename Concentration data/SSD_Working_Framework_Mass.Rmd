...
Data Collapsing Method = 1st Quartile
...
tier1_2_large_1q_ggplot <- ggplot(pred_tier1_2_large_1q,aes_string(x = "est")) +
      ...
      geom_text_repel(data = SSD_tier1_2_large_1q, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 1, segment.alpha = 0.5) + 
  ...
  labs(title = "Tier 1/2, 100-5000 µm", x = "µg/L") +
...
gt(largesdf) %>% 
  tab_header(title = "Tier 1/2, 100-5000 um", subtitle = "Thresholds presented in ug/L") %>% 
  fmt_number(columns = vars(Tier_1, Tier_2),n_sigfig = 2, use_seps = TRUE) 
