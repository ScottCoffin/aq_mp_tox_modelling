...
adam <- read.csv("Concentration Data/Datasets/adam2019.csv", na.strings = "N.A.", fileEncoding="UTF-8-BOM") %>% 
  mutate(x1M = Min.particle.size.um,
         x2M = Max.particle.size.um) %>% 
  mutate(Sample.Type = "sample")

#SF bay data from Zhu et al (2021)
sfBay <- read.csv("Concentration Data/Datasets/SFBayData.csv", na.strings = "NA", fileEncoding="UTF-8-BOM", stringsAsFactors = TRUE) %>% 
...
globalData %>% 
  ...
  scale_fill_manual(name = "Location", labels = c("San Francisco Bay", "Global (no CA data)"),
                    values = c("#FF410DFF", "6EE2FF99")) +
...
### Risk Characterization Histogram
```{r}
#generate plot
dfADAM_exceedance %>% 
  filter(Conc > 0) %>% 
