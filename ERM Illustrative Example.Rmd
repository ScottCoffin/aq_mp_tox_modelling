---
title: "ERM Illustrative Example"
author: "Scott Coffin"
date: "7/30/2021"
output:
...
upper.ingestible.width.um = 115
lower.ingestible.width.um = 1
...
```{r}
# calculate lower
x_LL_sa = SAfnx(a = 0.5 * x1D_set, #length
                b = 0.5 * x1D_set, #width
                c = 0.5 * 0.67 * x1D_set) #height = 0.67 * width
...
# calculate lower
x_UL_sa = SAfnx(a = 0.5 * x2D_set, #upper default length (5,000 um)
                b = 0.5 * upper.ingestible.width.um, #()
                c = 0.5 * 0.67 * upper.ingestible.width.um)
...
```{r}
EC_env_sa_ingestion <- EC_poly_sa * CF_bio
paste("EC_env_sa = ",signif(EC_env_sa_ingestion,4), "Particles/L (1-5,000 um; marine surface water )")
```
...
x_LL_m = massfnx_poly(width = lower.ingestible.width.um, #smallest ingestible width
               length = x1D_set, # smallest length 
               p = p.ave) #average density
#upper limit
x_UL_m = massfnx_poly(width = upper.ingestible.width.um,
               length = x2D_set, #upper default length
               p = p.ave) #average density
