...
##++++++++++++++++++++++++++++++++++++++++
Data <- synthetic_data_builder(count = 10000, 
                               volume_l = 1, 
...
for(loop in volumes){
    n = n + 1
    list[[n]] <- synthetic_data_builder(count = 10000, 
                                        volume_l = loop, 
                                        addedfactors = possible_variables[488,]) %>%
                  group_by(size.category) %>% 
                  summarize(particles.L = sum(dose.particles.mL.master) * 1000) %>% 
                  mutate(volume.L = factor(as.character(loop)))
}

summary_df <- bind_rows(list)
...
for(loop in volumes){
    n = n + 1
    list[[n]] <- add_prediction(synthetic_data_builder, count = 10000, 
                                        volume_l = loop, 
                                        addedfactors = possible_variables[488,]
                                )
}

df_pred <- bind_rows(list)
...
## Generate Particles
### Datasets with different numbers of particles in 1 ml
```{r, include=FALSE}

count = c(5,10,20,40,80,160,320,480,640,1000,1280)
...
for(loop in count){
    n = n + 1
    list[[n]] <- add_prediction(synthetic_data_builder, count = loop, 
                                        volume_l = 0.001, 
                                        addedfactors = possible_variables[488,]
                                )
}
...
df <- add_prediction(synthetic_data_builder, count = 640, 
                                        volume_l = 0.001, 
                                        addedfactors = possible_variables[488,]
                                )

summary(glm(ToxicityProbability ~ Size * Shape * polymer * um3, data = df))
p1 <- df %>% ggplot(aes(x = 10^Size, y = ToxicityProbability, color = polymer)) + geom_point()

p2 <- df %>% ggplot(aes(x = ToxicityProbability, fill = size.category)) + geom_histogram()
grid.arrange(p1,p2)
```
