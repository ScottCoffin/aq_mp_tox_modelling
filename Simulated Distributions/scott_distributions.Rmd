...
```{r}
Data %>%
  mutate(measured = factor(case_when(
    Size < 20 ~ "Not Measured",
    Size < 212 & Size >= 20 ~ "Measured",
    Size >= 212 ~ "Not Measured"))) %>% 
  ggplot(aes(x = Size)) +
  geom_histogram(aes(fill = measured),
               position = "identity",
               bins = 200) +
  # geom_density( +
  #              # fill = "deepskyblue2",
  ...
  dark_theme_minimal(base_size = 20) +
