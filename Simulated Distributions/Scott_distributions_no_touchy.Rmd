...
#set to 83um for upper limit or max size ingest, whichever is smaller
  ...
  mutate(x2M_ingest = case_when(is.na(max.size.ingest.um) ~ R.ave * x2D_set, #all calculations below occur for length. Width is R.ave * length, so correcting here makes width the max size ingest below
...
glm_fiber <- plot(pdp_classif_glm_fiber$agr_profiles) +
  ggtitle("GLM: Partial Dependence Profiles by Fiber Fraction and Effect Metric", "") +
  xlab("Fiber Fraction") +
  ylab("Average Prediction for Exposure Concentration (particles/mL)") +
...
composition_agnostic <- df_oxidative.stress %>% 
  ...
  distinct(species_f, max.size.ingest.mm, life.stage, bio.org, lvl1_f, lvl2_f, 
           #lvl3_f, 
           exposure.duration.d, organism.group,
...
ox1.summary <- ox.1 %>%
  ...
  group_by(Species, Group) %>% 
  summarise(dose_new = quantile(10 ^ particles.mL.ox.stress, 0.50)) #convert from log10
...
# predict NOEC
ox.1.real$particles.mL.ox.stress <- predict(classif_rf_oxidative.stress, ox.1.real, type = "raw")
...
#Data collapse
ox3.summary <- ox.3 %>%
  mutate(particles.mL.ox.stress = 10 ^particles.mL.ox.stress) %>% 
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
...
ggplot(ox.t3.preds, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = ox.t3.points,aes(x = Conc, y =frac, color = Group)) + 
  geom_text_repel(data = ox.t3.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Model-Predicted Microplastics Species Sensitivity Distribution",
...
```{r}
#assign groups
food.1 <- composition_agnostic %>% 
 filter(bio.org %in% c("tissue", "organism", "population")) %>% 
  left_join(groups)
  
# predict NOEC
food.1$particles.mL.food.dilution <- predict(classif_rf_food.dilution, food.1, type = "raw") 

#Data collapse
food1.summary <- food.1 %>%
  mutate(particles.mL.food.dilution = 10 ^particles.mL.food.dilution) %>% #make linear
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
...
#assign groups
food.1.real <- composition_realistic %>% 
   filter(bio.org %in% c("tissue", "organism", "population")) %>% 
  left_join(groups)
  
# predict NOEC
food.1.real$particles.mL.food.dilution <- predict(classif_rf_food.dilution, food.1.real, type = "raw")

#Data collapse
food1.summary.real <- food.1.real %>%
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
  summarise(dose_new = quantile(10 ^ particles.mL.food.dilution, 0.50)) #convert from log10
...
food3.summary <- food.3 %>%
  mutate(particles.mL.food.dilution = 10^particles.mL.food.dilution) %>% 
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
  summarise(dose_new = quantile(particles.mL.food.dilution, 0.50))
...
#assign groups
food.3.real <- composition_realistic %>% 
  filter(bio.org %in% c("organism", "population"),
         lvl1_f %in% c("Fitness")) %>%
  left_join(groups)
  
# predict NOEC
food.3.real$particles.mL.food.dilution <- predict(classif_xgbTree_food.dilution, food.3.real, type = "raw")
...
ggplot() +
  # geom_xribbon(data = t3.food.preds.j.real.agonstic, aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", fill = "type"), alpha = 0.3) +
  geom_line(data = t3.compare, aes_string(x = "est", y = "percent/100", fill = "type", color = "type")) +
...
"Environmentally Realistic Polymer/Shape Predicted" = c(
  ...
  kable(caption = "Food Dilution Thresholds")
