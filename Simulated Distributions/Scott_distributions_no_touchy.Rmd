...
```
```{r}
env_tox_composition <- ggarrange(shape_plot, polymer_plot,
                        labels = c("A", "B"), common.legend = TRUE, legend = "bottom", 
                        ncol = 2, nrow = 1)

plot(env_tox_composition)

ggsave(filename = "env_tox_composition.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       plot = env_tox_composition, width = 10, height = 10, units = "in",
       dpi = 300, bg = "white")
```
...
# RShiny App Prep
## Test Dataset for RShiny App


### Prep dataframe for modelling
```{r GLM build 1}
require(reshape2)
# data preparation
test_data_prediction <- aoc_final %>% 
...
#mutate_if(~is.numeric(.) && (.) > 0, log10) %>% 
  ...
  dplyr::select(-c(rowid,
                   ...
                   #PA,
                   #PC,
                   #PE, PET, PLA, PMMA, PP, PS, PUR, PVC, fiber, fragment, sphere
                   )) %>% 
   drop_na()  #drop missing

skim(test_data_prediction)
```

### Rename variables for interpretability
```{r}
test_data_prediction_renamed <- test_data_prediction %>% 
#Rename columns
       dplyr::rename(c("Species" = species_f, 
                       "Organism Group" = organism.group,
                       "Environment" = environment,
                       "Life Stage" = life.stage,
                       #"In vitro/in vivo" = vivo_f,
                       #"Sex" = sex, 
                       #"Estimated Body Length (cm)" = body.length.cm,
            "Estimated Maximum Ingestible Size (mm)" = max.size.ingest.mm,
             #experimental parameters
            #"Experiment Type" = exp_type_f, 
            "Exposure Route" = exposure.route,
            #"Particle Mix?" = mix,
            #"Negative Control" = negative.control,
             #"Reference Particle" = reference.material, 
            #"Exposure Media" = exposure.media, "Solvent" = solvent, "Detergent" = detergent,
            #"pH" = media.ph, "Salinity (ppt)" = media.sal.ppt, "Temperature (Avg)" = media.temp, "Temperature (Min)"= media.temp.min,
            #"Temperature (Max)" = media.temp.max, 
            ...
            #"Target Cell or Tissue" = target.cell.tissue,
             "Effect Metric" = effect.metric,
            "Effect Score" = effect.score,
            "Empirical Tissue Translocation ERM Conc. (log 10 particles/mL; 1-5,000 um)" = log10.particles.mL.ox.stress,
             "Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)" = log10.particles.mL.food.dilution
             #particle characteristics
            #"Polymer" = poly_f, 
            #"Shape" = shape_f, 
            #"Density (g/cm^3)" = density.g.cm3, "Density, reported or estimated" = density.reported.estimated,
            ...
            #"Particle Mass (mg)" = mass.per.particle.mg, 
            #"Weathered or Biofouled?" = weather.biofoul_f,
             #quality
...
```{r}
set.seed(4)
#remove particles.mL.food.dilution for training
split_ox <- test_data_prediction_renamed %>%
  dplyr::select(-`Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)`) %>% 
  initial_split(prop = 0.75)
test_ox <- testing(split_ox)
train_ox <- training(split_ox)

#remove particles.ml.foodidative.stress
split_food <- test_data_prediction_renamed %>%
  dplyr::select(-`Empirical Tissue Translocation ERM Conc. (log 10 particles/mL; 1-5,000 um)`) %>% 
  initial_split(prop = 0.75)
test_food <- testing(split_food)
train_food <- training(split_food)

# Create a training data set with the training() function
# Pulls from training and testing sets created by initial_split()
split <- test_data_prediction_renamed %>%
  initial_split(prop = 0.75)
test_test_data_prediction <- testing(split)
training_test_data_prediction <- training(split)

#ensure completeness
skim(test_test_data_prediction)

#save test data for app
write.csv(test_test_data_prediction,"ML Models/RShinyPrep/test_data_prediction.csv")

#save training data for app
write.csv(training_test_data_prediction, "ML Models/RShinyPrep/training_data_prediction.csv")


#save list of valid values (levels) for each variable
valid_values <- test_test_data_prediction %>%
  dplyr::select(where(is.factor)) %>%
  purrr::map_dfr(function(f) {
    data.frame(Level = levels(f))
  }, .id = "VarName")

write.csv(valid_values, "ML Models/RShinyPrep/valid_values.csv")
```
##Model Build for RShiny App
```{r}
#ox stress as response
classif_rf_oxidative.stress <- train(`Empirical Tissue Translocation ERM Conc. (log 10 particles/mL; 1-5,000 um)` ~ ., 
                                     data = train_ox, method = "rf", ntree = 100, tuneLength = 1)
#food dilution ERM as response
classif_rf_food.dilution <- train(`Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)` ~ ., data = train_food, method = "rf", ntree = 100, tuneLength = 1)

#save models
#save models
saveRDS(classif_rf_food.dilution, "ML Models/RShinyPrep/randomForest_foodDilution.rds")
saveRDS(classif_rf_oxidative.stress, "ML Models/RShinyPrep/randomForest_oxStress.rds")
```

## Quick feature analysis for powerpoint

Just use polymer and shape as single variables
```{r}
# data preparation
quick_build <- aoc_final %>% 
  filter(poly_f != "Not Reported") %>% 
 #filter data select predictor variables#
  dplyr::select(c(
                 #-rowid,
...
# Create a training data set with the training() function
# Pulls from training and testing sets created by initial_split()
split_quick <- quick_build %>%
  initial_split(prop = 0.75)
quick_test <- testing(split_quick)
quick_train <- training(split_quick)


yTest <- as.numeric(as.character(quick_test$`Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)`))

test_x <- as.data.frame(quick_test %>% dplyr::select(- `Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)`))

quick_rf <- train(`Empirical Food Dilution ERM Conc. (log 10 particles/mL; 1-5,000 um)` ~ ., 
                  data = quick_train, method = "rf", ntree = 100, tuneLength = 1)

explainer_classif_rf_quick <- DALEX::explain(quick_rf,
                                       label = "rf",
                                       data = quick_test, 
                                       y = yTest)

variable_importance_rf <- model_parts(explainer_classif_rf_quick, loss_function = loss_root_mean_square)
...
#ML Model Paper
### Effect score analysis
#### Unaligned
Just visualize effect concentrations by effect metric.
```{r}
unaligned.effect.score <- aoc_final %>% 
   # tier 3/4 filters
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population",
                      Group != "Algae")) %>% 
  filter(between(size.length.um.used.for.conversions, 1, 5000))  %>% 
  mutate(effect.score_f = case_when(
    effect.score == 1 ~ "minor",
    effect.score == 2 ~ "moderate",
    effect.score == 3 ~ "moderate",
    effect.score == 4 ~ "severe",
    effect.score == 6 ~ "lethal",
  )) %>% 
  mutate(effect.score_f = as.factor(effect.score_f),
         effect.score_f = fct_reorder(effect.score_f, effect.score)) %>% 
    filter(!effect.metric %in% c("IC50", 
                               "EC10"#,
                               #"EC50"
                               )) %>% 
  drop_na(effect.metric, effect.score_f) %>% 
   ggplot(aes(x = dose.particles.mL.master * 1000, fill = effect.metric)) +
...
unaligned.effect.score
```

#### Aligned by volume
```{r}
aligned.effect.score <- aoc_final %>% 
   # tier 3/4 filters
  filter(risk.13 != 1,
         bio_f %in% c("Organism", "Population",
                      Group != "Algae")) %>% 
  ...
  drop_na(effect.metric, effect.score_f) %>% 
   ggplot(aes(x = particles.mL.food.dilution * 1000, fill = effect.metric)) +
  geom_boxplot(notch = TRUE) +
  scale_fill_tron(
                    name = "Tox or Occurence Data") +
  scale_x_log10(name = paste("Particles/L (food dilution-aligned)"),
                             breaks = scales::log_breaks(),
                #             labels = comma_signif
                ) +
 # ylab("Normalized Abundance") +
...
aligned.effect.score
```
##### Manuscript Figure
```{r}
effect.score.figures <- ggarrange(unaligned.effect.score, aligned.effect.score,
                        labels = c("A", "B"), common.legend = TRUE,
                        legend = "bottom",
                        ncol = 2, nrow = 1)

plot(effect.score.figures)

ggsave(plot = aligned.effect.score,
       filename = "effect.score.figures.jpeg",
       path = "Simulated Distributions/BigDataManuscript/", 
       width = 8, height = 6, units = "in",
       bg = "white",
       dpi = 300)
```
#### How much data for each effect score for the upper tiers?
```{r}
effect_score_counts <- aoc_final %>% 
   # tier 3/4 filters
  filter(risk.13 != 1,
  ...
  drop_na(effect.metric, effect.score_f) %>% 
  group_by(effect.score_f) %>% 
  summarize(n_species = n_distinct(species), 
            n_doi = n_distinct(doi),
            n_taxa = n_distinct(Group),
            n_obs = n(),
            n_lvl3 = n_distinct(lvl3))
write.csv(effect_score_counts,
          "effect_score_counts.csv")
...
#unique id for saving later for RShiny app
test.ox <- test
...
residuals_oxidative.stress <- resid %>% 
   mutate(label_clean = case_when(
    label == "rf" ~ "Random Forest",
    label == "xgbTree" ~ "Extreme Gradient Boosted Trees",
    label == "glm" ~ "General Linear Model"
  )) %>% 
ggplot(aes(x = diff, y = label_clean, fill = label_clean)) +
  geom_boxplot() +
 # scale_x_log10() +
  xlab("Residuals (Oxidative Stress log10(particles/mL))") +
...
ggsave(plot =residuals_oxidative.stress,
       filename = "residuals_oxstress_regression.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       width = 9, height = 5, units = "in", bg = "white",
       dpi = 300)

residuals_oxidative.stress
```
...
taxa_pdp <- rf_pdp %>%
 # mutate(groups = fct_reorder(`_x_`,`_yhat_`)) %>%
  ggplot(aes(x = `_x_`, y = `_yhat_`, color= `_groups_`)) +
   geom_line() +
  scale_color_discrete(
                        name = "Taxa") +
  scale_x_continuous(name = "Exposure Duration (d)") +
  scale_y_continuous(name = "Average Predicted Effect Concentration (log10(particles/mL))") +
  theme_light()

taxa_pdp


ggsave(plot =taxa_pdp,
       filename = "taxa_pdp_regression.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
...
```{r}
#Partial Dependence by effect metric and PE fraction
pdp_classif_rf_fiber  <- model_profile(explainer_classif_rf_oxidative.stress,
                                       N = 1000, #use all observations
                                       type = "partial",
                                       variable = "fiber_frac", 
                                 groups = "effect.metric")
...
#save unique df for RShiny app later
test_food <- test
...
```{r}
resid_food <- rbind(mp_classif_rf_food.dilution$residuals, mp_classif_glm_food.dilution$residuals, mp_classif_xgbTree_food.dilution$residuals)

residuals_food <- resid_food %>% 
  mutate(label_clean = case_when(
    label == "rf" ~ "Random Forest",
    label == "xgbTrees" ~ "Extreme Gradient Boosted Trees",
    label == "glm" ~ "General Linear Model"
  )) %>% 
ggplot(aes(x = diff, y = label_clean, fill = label_clean)) +
  geom_boxplot() +
 # scale_x_log10() +
  xlab("Residuals (Food Dilution log10(particles/mL))") +
  scale_fill_tron(name = "Model",
                  labels = c("Extreme Gradient Boosting Trees", "Random Forest", "Linear Model")) +
  # labs(title = "Model Performance",
  #      subtitle = "Residuals") +
...
ggsave(plot =residuals_food,
       filename = "residuals_food_regression.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       width = 9, height = 5, units = "in", bg = "white",
       dpi = 300)
...
```{r}
residuals <- ggarrange(residuals_oxidative.stress,residuals_food,
          nrow = 1, labels = c("A", "B"),
          common.legend = TRUE,
          legend = "bottom")

ggsave(plot =residuals,
       filename = "residuals.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
...
```{r}
tibble(mp_classif_glm_food.dilution$measures, mp_classif_rf_food.dilution$measures, mp_classif_xgbTree_food.dilution$measures#,
       #mp_classif_nnet_food.dilution$measures$r2
       )

mp_classif_glm_food.dilution$measures
```
...
composition_agnostic <- df_oxidative.stress %>% 
  ...
  distinct(species_f, max.size.ingest.mm, life.stage, bio.org, lvl1_f, lvl2_f, 
          ...
          PA_frac,
          #PC_frac,
          PE_frac, PET_frac, PLA_frac, PMMA_frac, PP_frac, PS_frac, PUR_frac, PVC_frac, fiber_frac,       fragment_frac, sphere_frac
           ) %>% 
...
#ggplot
empirical.vs.predicted.ox <- ggplot() +#, aes_string(x = "est")) +
...
empirical.vs.predicted.ox_dark <- empirical.vs.predicted.ox + dark_theme_bw()

empirical.vs.predicted.ox
```

###### F Test
```{r}
#line
var.test(t1.x.p$est, t1.x.e$est)

#upper 95% CI
var.test(t1.x.p$lcl, t1.x.e$ucl)
```
...
#t1.ox.preds.j <- rbind(t1.x.e, t1.x.p)
compare <- left_join(t1.x.1.e, t1.x.1.p, by = "Species") %>% 
    mutate(percent.diff = (Conc.e - Conc.y)/Conc.e)
...
compare %>% 
  drop_na() %>% 
  mutate(Species = fct_reorder(Species, percent.diff)) %>% 
  ggplot(aes(x = (percent.diff), y = Species, color = Group.x)) +
  geom_boxplot(fill = "red", color = "black") +
  geom_point(size = 2.5) +
  ...
  xlab("% Difference between Empirical and Predicted NOEC (log10 scale)") +
  ylab("Species") +
  geom_vline(xintercept = 0, linetype = "dashed", size = 1.5, alpha = 0.4) +
 # scale_color_manual(values = rainbow(13)) +
  #scale_color_brewer()
...
ggplot() +
  ...
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Predicted vs. Empirical Microplastics Species Sensitivity Distribution",
       subtitle = "Tier 3/4, 1 - 83 µm; ERM = Surface Area") +
  coord_trans(x = "log10") +
  scale_x_continuous(name = "Particles/mL (1 - 5,000 µm)",
...
ggplot() +
  ...
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Predicted vs. Empirical Microplastics Species Sensitivity Distribution",
       subtitle = "Tier 1/2, 1 - variable (mouth size) µm; ERM = Volume",
...
ggplot(food.t1.preds.agnostic, aes_string(x = "est")) +
  ...
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Model-Predicted Microplastics Species Sensitivity Distribution",
       subtitle = "Tier 1/2, 1 - 5,000 µm; ERM = Volume",
       x = "particles/L (aligned to 1-5,000 um; ingestion limited)") +
  coord_trans(x = "log10") +
  scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
                     labels = trans_format("log10", scales::math_format(10^.x))) + 
...
#ggplot
empirical.SSD <- ggplot(food.t1.emp.preds, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = food.t1.emp.points,aes(x = Conc, y =frac, 
                                           #color = Group
                                           )) + 
  geom_text_repel(data = food.t1.emp.points, aes(x = Conc, y = frac, label = Species, 
                                                 #color = Group
                                                 ), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
...
#ggplot
empirical.vs.predicted.food <- ggplot() +#, aes_string(x = "est")) +
...
empirical.vs.predicted.food_dark <- empirical.vs.predicted.food + dark_theme_bw()

empirical.vs.predicted.food
```
###### F Test
```{r}
var.test(t1.x.p$est, t1.x.e$est)
...
#ggplot
environmental.realistic.SSD <- ggplot(food.t1.preds.real, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = food.t1.points.real,aes(x = Conc, y =frac,
                                            #color = Group
                                            )) + 
  geom_text_repel(data = food.t1.points.real, aes(x = Conc, y = frac, label = Species, 
                                                  #color = Group
                                                  ), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
#  labs(title = "Model-Predicted Microplastics Species Sensitivity Distribution (Realistic Shapes and Polymers)",
 #      subtitle = "Tier 1/2, 1 - variable (mouth size) µm; ERM = Surface Area ",
  #     x = "particles/L (aligned to 1-5,000 um; translocation limited)") +
  xlab("Particles/mL") +
  coord_trans(x = "log10") +
  scale_x_continuous(breaks = scales::trans_breaks("log10", function(x) 10^x, n = 15),
...
environmental.realistic.SSD
```

```{r}
food.dilution.SSDs<- ggarrange(empirical.SSD, environmental.realistic.SSD,
            labels = c("A", "B"), common.legend = TRUE, legend = "bottom", 
                        ncol = 2, nrow = 1)

plot(food.dilution.SSDs)

ggsave(filename = "food.dilution.SSDs.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       plot = food.dilution.SSDs, width = 10, height = 5, units = "in",
       dpi = 300, bg = "white")
```
...
t1.x.1.e <- t1.x.1.e %>% 
  mutate(Conc.e= Conc)
#predicted
t1.x.1.p <-  t1.x.1.p.agnostic %>% 
  mutate(Conc.y = Conc)
###join dataframes
#t1.ox.preds.j <- rbind(t1.x.e, t1.x.p)
compare_food <- left_join(t1.x.1.e, t1.x.1.p, by = "Species") %>% 
    mutate(percent.diff = (Conc.e - Conc.y)/Conc.e)
...
```
##### Is polymer or shape more important for changes in SSD?
To determine what's causing the  shift in the SSD towards the right when using environmentally realistic polymers and shapes, let's just change shapes and again polymers to compare.

```{r}
#define base parameters for all SSDs
composition_realistic_polymers_food <- df_oxidative.stress %>% 
  # make entry for each species
  distinct(species_f, max.size.ingest.mm, life.stage, bio.org, lvl1_f, lvl2_f,
           effect.score,
           #lvl3_f, 
           ...
           translocatable,
           # make synonymous with training data for true comparison
           #EVA_frac, LTX_frac, PA_frac, PC_frac, PE_frac, PET_frac, PLA_frac, PMMA_frac, PP_frac, PS_frac, PUR_frac, PVC_frac, 
           fiber_frac, fragment_frac, sphere_frac # just change polymers
           ) %>% 
  #give summary statistics for typical composition of 1-5,000 um particle distribution in aquatic environment
  mutate(
      #  EVA_frac = 0.06,
...
skim(composition_realistic_polymers_food)
```

```{r}
#define base parameters for all SSDs
composition_realistic_shapes_food <- df_oxidative.stress %>% 
  # make entry for each species
  distinct(species_f, max.size.ingest.mm, life.stage, bio.org, lvl1_f, lvl2_f,
           effect.score,
           #lvl3_f, 
          ...
          # EVA_frac,
           ...
           PA_frac,
          #PC_frac, 
          PE_frac, PET_frac, PLA_frac, PMMA_frac, PP_frac, PS_frac, PUR_frac, PVC_frac, 
          # fiber_frac, fragment_frac, sphere_frac 
           ) %>% 
  #give summary statistics for typical composition of 1-5,000 um particle distribution in aquatic environment
  mutate(
        # EVA_frac = 0.06,
...
skim(composition_realistic_shapes_food)
```


```{r}
#### BUILD SSD FOR REALISTIC POLYMERS ####
#Get organism group for each species
groups <- aoc_z %>% 
  distinct(species_f, Group)

#assign groups
food.1.polymer <- composition_realistic_polymers_food %>% 
   filter(organism.group != "Algae", #all food dilution
         lvl1_f %in% c("Fitness"),
         bio.org %in% c("organism", "population"), #special for tier 3/4
         #environment == "Marine",
...
# predict NOEC
food.1.polymer$particles.mL.food.dilution_log10 <- predict(classif_rf_food.dilution, food.1.polymer, type = "raw")

#Data collapse
food1.summary.polymer <- food.1.polymer %>%
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
   mutate(particles.mL.food.dilution = (10 ^ particles.mL.food.dilution_log10) / (af.time * af.noec)) %>% 
  summarise(dose_new = quantile(particles.mL.food.dilution, 0.50)) #convert from log10
...
## SSD build
food.t1.ssd.polymer <- SSD_function_t1(filtered.data = food1.summary.polymer, hcxlcl = 5)

#extract data frames from object
food.t1.preds.polymer <- as.data.frame(food.t1.ssd.polymer[1]) %>%  mutate(type = "Realistic Polymers")
food.t1.points.polymer <- as.data.frame(food.t1.ssd.polymer[2]) %>%  mutate(type = "Realistic Polymers")

#### BUILD SSD FOR REALISTIC shapeS ####
#Get organism group for each species
groups <- aoc_z %>% 
  distinct(species_f, Group)

#assign groups
food.1.shape <- composition_realistic_shapes_food %>% 
   filter(organism.group != "Algae", #all food dilution
         lvl1_f %in% c("Fitness"),
         bio.org %in% c("organism", "population"), #special for tier 3/4
         #environment == "Marine",
...
# predict NOEC
food.1.shape$particles.mL.food.dilution_log10 <- predict(classif_rf_food.dilution, food.1.shape, type = "raw")

#Data collapse
food1.summary.shape <- food.1.shape %>%
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
   mutate(particles.mL.food.dilution = (10 ^ particles.mL.food.dilution_log10) / (af.time * af.noec)) %>% 
  summarise(dose_new = quantile(particles.mL.food.dilution, 0.50)) #convert from log10
...
## SSD build
food.t1.ssd.shape <- SSD_function_t1(filtered.data = food1.summary.shape, hcxlcl = 5)

#extract data frames from object
food.t1.preds.shape <- as.data.frame(food.t1.ssd.shape[1]) %>%  mutate(type = "Realistic Shapes")
food.t1.points.shape <- as.data.frame(food.t1.ssd.shape[2]) %>%  mutate(type = "Realistic Shapes")

#join dataframes
food.preds.all <- rbind(t1.food.preds.j.real.agonstic, food.t1.preds.shape, food.t1.preds.polymer)
food.points.all <- rbind(t1.food.points.j.real.agonstic, food.t1.points.shape, food.t1.points.polymer)

#ggplot
food.compare.polymer.shape <- food.preds.all %>% 
  filter(!type %in% c("Polymer and Shape Agnostic", "Environmentally Realistic")) %>% 
  ggplot() +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100", fill = "type"), alpha = 0.1) +
  geom_line(aes_string(x = "est", y = "percent/100", fill = "type", color = "type")) +
  #geom_point(data = t1.food.points.j.real.agonstic, aes(x = Conc, y =frac, color = type), alpha = 0.2) + 
  #geom_text_repel(data = food.t1.emp.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  # labs(title = "Realistic Shapes vs. Polymers vs. Empirical Species Sensitivity Distribution",
  #      subtitle = "Tier 1/2, 1 - variable (mouth size) µm; ERM = Volume",
  #      x = "particles/L (aligned to 1-5,000 um; ingestion limited)") +
  coord_trans(x = "log10") +
  xlab("Particles/mL") +
...
ggsave(filename = "food.compare.polymer.shape.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       plot = food.compare.polymer.shape, width = 10, height = 5, units = "in",
       dpi = 300, bg = "white")

food.compare.polymer.shape
```
```{r}

final.comparison <- food.preds.all %>% 
 group_by(type) %>% 
  summarize(HC5 = est[5],
            se = se[5]) #%>% 
...
#write.csv(final.comparison, "Simulated Distributions/BigDataManuscript/")
```
...
```{r}
#dark mode
ssd_comparison_dark <- ggarrange(empirical.vs.predicted.ox_dark, empirical.vs.predicted.food_dark, common.legend = TRUE, legend = "bottom", ncol = 2, nrow = 1)

ssd_comparison_dark

ggsave(filename = "ssd_comparison_dark.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       plot = ssd_comparison_dark, width = 8, height = 3, units = "in",
       dpi = 300, bg = "black")
```

```{r}
#light mode
ssd_comparison <- ggarrange(empirical.vs.predicted.ox, empirical.vs.predicted.food,
            labels = c("A", "B"), common.legend = TRUE, legend = "bottom", 
                        ncol = 2, nrow = 1)

plot(ssd_comparison)

ggsave(filename = "ssd_comparison.jpeg",
       path = "Simulated Distributions/BigDataManuscript", 
       plot = ssd_comparison, width = 10, height = 5, units = "in",
       dpi = 300, bg = "white")
```
