...
# apply LIME
explainer <- lime(train, classif_xgbTree)
...
explainer_classif_xgbTrees_food.dilution <- DALEX::explain(classif_xgbTrees_food.dilution, label = "xgbTrees", 
                                        data = test_x, y = yTest)
...
mp_classif_xgbTrees_food.dilution <- model_performance(explainer_classif_xgbTrees_food.dilution)
...
vi_classif_xgbTrees_food.dilution <- model_parts(explainer_classif_xgbTrees_food.dilution, loss_function = loss_root_mean_square)

plot(vi_classif_rf_food.dilution, vi_classif_glm_food.dilution, vi_classif_xgbTrees_food.dilution)
```
...
composition <- df_oxidative.stress %>% 
  # make entry for each species
  distinct(species_f, max.size.ingest.mm, life.stage, bio.org, lvl1_f, lvl2_f, lvl3_f, exposure.duration.d, organism.group) %>% 
...
#assign groups
ox.1 <- composition %>% 
  filter(#bio.org %in% c("tissue", "organism", "population"),
         lvl1_f %in% c("Fitness")) %>%
  left_join(groups)
  
# predict NOEC
ox.1$particles.mL.ox.stress <- predict(classif_rf_oxidative.stress, ox.1, type = "raw")

#Data collapse
ox1.summary <- ox.1 %>%
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
  summarise(dose_new = quantile(10 ^ particles.mL.ox.stress, 0.25)) #convert from log10

## SSD build
ox.t1.ssd <- SSD_function_t1(filtered.data = ox1.summary, hcxlcl = 5)

#extract data frames from object
ox.t1.preds <- as.data.frame(ox.t1.ssd[1])
ox.t1.points <- as.data.frame(ox.t1.ssd[2])

#ggplot
ggplot(ox.t1.preds, aes_string(x = "est")) +
  ...
  geom_point(data = ox.t1.points,aes(x = Conc, y =frac, color = Group)) + 
  geom_text_repel(data = ox.t1.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
...
##### Empirical alone
```{r}
ox.1.emp <- df_oxidative.stress %>% 
  filter(lvl1_f %in% c("Fitness")) %>% 
  rename(Species = species_f) %>% 
  rename(Group = organism.group) %>% 
  group_by(Species, Group) %>% 
  summarise(dose_new = quantile(10 ^ particles.mL.ox.stress, 0.25)) #convert from log10
...
t1.x.1.e <- ox.t1.points %>% 
  mutate(type = "Empirical") %>% 
  mutate(Conc.e= Conc/1000)
t1.x.1.p <-  ox.t1.emp.points %>% 
  mutate(type = "Predicted")
###join dataframes
#t1.ox.preds.j <- rbind(t1.x.e, t1.x.p)
compare <- left_join(t1.x.1.e, t1.x.1.p, by = "Species") %>% 
    mutate(diff = Conc.e - Conc.x)
...
#assign groups
ox.3 <- composition %>% 
  filter(bio.org %in% c("organism", "population"),
         lvl1_f %in% c("Fitness")) %>%
  left_join(groups)
  
# predict NOEC
ox.3$particles.mL.ox.stress <- predict(classif_xgbTree_oxidative.stress, ox.3, type = "raw")
...
#extract data frames from object
ox.t3.preds <- as.data.frame(ox.t3.ssd[1])
ox.t3.points <- as.data.frame(ox.t3.ssd[2])

#ggplot
ggplot(ox.t3.preds, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = ox.t1.points,aes(x = Conc, y =frac, color = Group)) + 
  geom_text_repel(data = ox.t1.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
...
# predict NOEC
food.1$particles.mL.food.dilution <- predict(classif_xgbTree_food.dilution, food.1, type = "raw")

#Data collapse
food1.summary <- food.1 %>%
    rename(Species = species_f) %>% 
  group_by(Species, Group) %>% 
  summarise(dose_new = quartile(particles.mL.food.dilution, 0.25))

## SSD build
food.t1.ssd <- SSD_function_t1(filtered.data = food1.summary, hcxlcl = 5)

#extract data frames from object
food.t1.preds <- as.data.frame(food.t1.ssd[1])
food.t1.points <- as.data.frame(food.t1.ssd[2])

#ggplot
ggplot(food.t1.preds, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = food.t1.points,aes(x = Conc, y =frac, color = Group)) + 
  geom_text_repel(data = food.t1.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
  scale_y_continuous("Species Affected (%)", labels = scales::percent, limits = c(0,1)) +
  labs(title = "Model-Predicted Microplastics Species Sensitivity Distribution",
...
# predict NOEC
food.3$particles.mL.food.dilution <- predict(classif_xgbTree_food.dilution, food.3, type = "raw")
...
#extract data frames from object
food.t3.preds <- as.data.frame(food.t3.ssd[1])
food.t3.points <- as.data.frame(food.t3.ssd[2])

#ggplot
ggplot(food.t3.preds, aes_string(x = "est")) +
  geom_xribbon(aes_string(xmin = "lcl", xmax = "ucl", y = "percent/100"), alpha = 0.2, color = "blue", fill = "lightblue") +
  geom_line(aes_string(y = "percent/100"), color = "gray") +
  geom_point(data = food.t1.points,aes(x = Conc, y =frac, color = Group)) + 
  geom_text_repel(data = food.t1.points, aes(x = Conc, y = frac, label = Species, color = Group), nudge_x = 0.2, size = 3, segment.alpha = 0.5) + 
